	...
	if(request.getMethod()=="POST")
	{
		...
		byte[] bytes = null;
            ...
            if(Pchlist!=null && Pchlist.length>0)
            {
            	...
            	String cntr=null;
            	query+=" and nvl(a.interunit,'N') in(";
             ...
             parameters.put("RPORTORDR",request.getParameter("RPORT_ORDER"));
             
             
             String rprt=request.getParameter("RPORT_ORDER");
             if(rprt!=null && rprt.equals("T")){
             report = (JasperReport) JRLoader.loadObject(request.getRealPath("/shahiwebpages/MvxExp/reports/PRE/DispatchTruckKRA(T).jasper"));
             }
             else if(rprt!=null && rprt.equals("TP")){
                 report = (JasperReport) JRLoader.loadObject(request.getRealPath("/shahiwebpages/MvxExp/reports/PRE/DispatchTruckKRA(TP).jasper"));
             }
             else if(rprt!=null && rprt.equals("TPS")){
               report = (JasperReport) JRLoader.loadObject(request.getRealPath("/shahiwebpages/MvxExp/reports/PRE/DispatchTruckKRA(TPS).jasper"));  
             }
             else if(rprt!=null && rprt.equals("PT")){
               report = (JasperReport) JRLoader.loadObject(request.getRealPath("/shahiwebpages/MvxExp/reports/PRE/DispatchTruckKRA(PT).jasper"));  
             }
             else{
                report = (JasperReport) JRLoader.loadObject(request.getRealPath("/shahiwebpages/MvxExp/reports/PRE/DispatchTruckKRA(PS).jasper")); 
             }
             filename="DispatchTruckKRA";
...
<body> 
	...
	<form name="create_pdf_form" id="create_pdf_form" action="DispatchTruckKRA.jsp" method="POST">
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
			       ...
			       <td valign="top" class="label-1" style="background-color: white;text-align:left">Port</td>
                               ...
                               <td valign="top" class="label-1" style="background-color: white;text-align:left">TYPE</td>
                                <td valign="top"class="label-1" style="background-color: white;text-align:left">
                                   <s:select theme="simple" name="PCH" id="PCH" multiple="true" list="# {'S':'INTER STATE','L':'LOCAL','U':'INTER UNIT','C':'CONTAINER','N':'NA'}" ondblclick="addPchGrp();" cssClass="texts" cssStyle="font-size:9pt;width:100px;height: 100px">
                                   </s:select>
                               </td>
                                <td valign="top" style="background-color: white;width:85px">
                            ...
                            <tr> 
                                ...
                                <td colspan="10" style="background-color: white;text-align:left;" class="label-1">
                             		<s:radio list="# {'T':'Transporter','TP':'Transporter+Port','TPS':'Transporter+Port+Size','PT':'Port+Transporter','PS':'Port+Size'}" value="%{'T'}" name="RPORT_ORDER" id="RPORT_ORDER" theme="simple" ></s:radio>
                                </td> 
                             ...
                             <tr>
                             	<td colspan="8" class="label-1" style="background-color: white;text-align: center;">
                             		...
                             		<input type="button" class="whitesubmitbutton" value="Reset" onclick='javascript:window.location.href="DispatchTruckKRAA"'>&nbsp;&nbsp;&nbsp;&nbsp;
