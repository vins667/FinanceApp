<%-- 
    ...
    Created on : Jul 07, 2016, 10:52:26 AM
    Author     : Guddu Kumar
--%>
...
<title>Shahi Exports Pvt Ltd</title>
   ...
   <script type="text/javascript" language="javascript" >     
    ...
    function search()
    {
    	if(validate()==true)
    	{
    	document.create_pdf_form.action="RelizationPendingRprt";
...
<%
	...
	if(request.getMethod()=="POST")
	{
		...
		byte[] bytes = null;
            ...
            String SINO=request.getParameter("SI_NO");
            
            if(SINO!=null && SINO.length()>0)
            {  
            	query+=" and nvl(E.dbk_slno,'NA') like '"+SINO+"'";
            } 
           // nvl(E.dbk_slno,'NA') like '62%' and
            ...
            if(Portlist!=null && Portlist.length>0)
            {
             ...
             parameters.put("invtp",request.getParameter("inv_tp"));
             parameters.put("COMPLIST",request.getParameter("COMP_LIST"));
             parameters.put("loc",request.getParameter("LOCT_CODE"));
             parameters.put("BASEDON",request.getParameter("BASED_ON"));
              parameters.put("DBKSTATUS",request.getParameter("DBK_STATUS"));
            
             report = (JasperReport) JRLoader.loadObject(request.getRealPath("/shahiwebpages/MvxExp/reports/GVTINC/RelizationPendingReport.jasper"));
             
             filename="RelizationPendingReport";
...
<body> 
	...
	<form name="create_pdf_form" id="create_pdf_form" action="RelizationPendingReport.jsp" method="POST">
                ...
                <tr>
                    <td align="center">
                        <table border="0" align="center" cellspacing="1" cellpadding="10" style="width: 650px; background-color: #6699CC;">
                            <tr>
                                <td class="label-1" style="background-color: white">Company </td>
                                <td class="label-1" style="background-color: white" colspan="3">
                                <s:select name="COMP_LIST" id="COMP_LIST"
                                      label="Invoice Type" 
                                      cssStyle="width:200px;font-size:9pt;" 
                                      theme="simple"  
                                      list="%{Company_LIST}" /></td>
                                
                                <td class="label-1" style="background-color: white">Bank </td>
                                <td class="label-1" style="background-color: white" colspan="3">
                                <s:select name="BANK_LIST" id="BANK_LIST"
                                      label="Bank_L" 
                                      cssStyle="width:200px;font-size:9pt;" 
                                      theme="simple"  
                                      list="%{Bank_List}" /></td>
                            ...
                            <tr>
                                ...
                                <s:select name="inv_tp" label="Invoice Type" cssStyle="font-size:10pt;width:200px;font-size:9pt;" theme="simple"  value="%{inv_tp}"  list="# {'N':'Normal','S':'Trade Sample','F':'Free Sample','%':'ALL'}" /></td>
			       ...
			       <td valign="top" class="label-1" style="background-color: white;text-align:left">Port</td>
                               ...
                               <td valign="top" class="label-1" style="background-color: white;">SI No.</td>
                                <td class="label-1" style="background-color: white;text-align: left;" valign="top" colspan="7">
                                <s:textfield name="SI_NO" id="SI_NO" cssStyle="text-transform:uppercase;width:150pt" theme="simple" />
                             	</td>
                            </tr> 
                            <tr> 
                                <td class="label-1" style="background-color: white;text-align: left;" valign="top">Dbk Status:
                             	</td>
                                <td colspan="3" style="background-color: white;text-align:left;" class="label-1">
                             		<s:radio list="# {'P':'Pending','D':'OutStanding','R':'Recieved'}" value="%{'P'}" name="DBK_STATUS" id="DBK_STATUS" theme="simple" ></s:radio>
                                </td> 
                             ...
                             <tr>
                             	<td colspan="8" class="label-1" style="background-color: white;text-align: center;">
                             		...
                             		<input type="button" class="whitesubmitbutton" value="Reset" onclick='javascript:window.location.href="RelizationPendingRprt"' >&nbsp;&nbsp;&nbsp;&nbsp;
