...
<html>
  ...
  <script language="javascript">
      ...
      function tax_srh()
      {
                    ...
                    document.frmbuy.action = "AwbviewPostDocsFin.action";
                    document.frmbuy.submit();
                    document.getElementById('prepage').style.visibility = '';
                
      } 
      
      function addUnit(a,b,c,d,e,f)
      {   
          
         indx=document.getElementById("indexname").value;
           
           window.parent.document.getElementById('AWB_NO'+indx).value=document.getElementById(a).value;
           window.parent.document.getElementById('AWB_DATE'+indx).value=document.getElementById(b).value;
           window.parent.document.getElementById('HAWB_NO'+indx).value=document.getElementById(c).value;;
          // window.parent.document.getElementById('APRV_DATE'+APRV_DATE).value=document.getElementById(d).value;
            window.parent.dojo.widget.byId("APRV_DATE"+indx).setValue(document.getElementById(d).value);
            window.parent.document.getElementById('YEAR'+indx).value=document.getElementById(e).value;
            window.parent.document.getElementById('LINK_NO'+indx).value=document.getElementById(f).value;
              
        window.parent.closediv('approveraddid');
           //window.parent.document.getElementById
            //alert(document.getElementById(f).value);
         //   window.parent.getchargesdt_test(document.getElementById(e).value,document.getElementById(f).value);
...
<form action=""  method="post" name="frmbuy" >
 ...
 <table align="center" width="100%"><tr><td valign="middle" style="border-width:1pt;border-color:black;border-style:solid;" >
       ...
       <table width="100%">
           <tr><td class="label-1">Awb No</td><td><s:textfield name="SEARCH_CODE" id="SEARCH_CODE" onKeyPress="tabE(this, event);" theme="simple" value="%{SEARCH_CODE}" cssStyle="width:180px;text-transform:uppercase" cssClass="texts"/></td>
    </tr><tr>
        <td colspan="2">
                ...
                <div  class="div1" style="width:100%;overflow:auto ;height:245px;">
                                ...
                                <tbody> 
                                    <s:iterator value="LicSearch" status="st">
                                        <tr style="color:#000000;background-color: #fff;font-weight: normal;font-size: 10px">
                                                        ...
                                                        <s:hidden name="LIC_PORT" value="%{LIC_PORT}" id="LIC_PORT%{#st.index}"/>
                                                        ...
                                                        <s:hidden name="SALE_AMT" value="%{SALE_AMT}" id="SALE_AMT%{#st.index}"/>
                                                           ...
                                                           <td> 
                                                            <img src="../images/add.png" onclick="addUnit('LIC_NO<s:property value="%{#st.index}"/>','LIC_DATE<s:property value="%{#st.index}"/>','LIC_PORT<s:property value="%{#st.index}"/>','LIC_PRCT<s:property value="%{#st.index}"/>','LIC_AMT<s:property value="%{#st.index}"/>','SALE_AMT<s:property value="%{#st.index}"/>')" />
