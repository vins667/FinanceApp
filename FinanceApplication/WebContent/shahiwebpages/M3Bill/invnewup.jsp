...
<![endif]-->
    ...
    function onsave()
    {
     ...
     document.getElementById('formIdinv').action="newinvmbillentAction?savetest=Yes"  ;
     document.getElementById('formIdinv').submit(); 
        }else{
            return;
        }
            ...
            for(i=0; i<BREAK_CODE.length; i++)
                  { 
                    ...
                    {    if(BREAK_CODE[j].value!=""){
                         ...
                         if(BREAK_CODE[i].value+INV_BUYER[i].value+INV_PCH[i].value==BREAK_CODE[j].value+INV_BUYER[j].value+INV_PCH[j].value)
                             {
