...
<script type="text/javascript">
	  function loadtotal(){
			$("#detailstot").load('Gnrtreftot.jsp');
			}
            function loadallpages(){
            	for(i=0;i<15;i++){
            		$("#details"+i).load('searchRefNoActionAxis.action?textid='+i);
            	}
            }
            function loadsinglerow(a,b,c,d){
            	var d1 = new Date();
            	$("#details"+a).load('searchRefNoActionAxis.action?textid='+a+'&XYEAR='+b+'&XBANK='+c+'&XCHQ_NO='+d+'&tvivek='+$.now());
            ...
            function save()
			{
				if(validateSave()==true)
				{
					if(confirm("Do you want to save!!!")){
						document.frm.action="saveRefNoActionAxis";
					    document.frm.submit();
					}
				}
			}
			...
			{
				$("#details"+a).load('searchRefNoActionAxis.action?textid='+a);
		...
		function openpopdtl(a,b){
			document.getElementById(a).style.display = '';
			document.getElementById('searchdetail').src="searchdtRefNoActionAxis.action?TINDEX="+b;
		}
		
		
		function refno()
		{
			document.frm.action="refnoRefNoActionAxis";
		    document.frm.submit();
		}
		...
		if (pfsct!=null && pfsct!="")
		  {
		 //alert("1"+b);
		     if(k!="" && parseInt(k)<200000 && pfsct!="UTIB" )
		        {
		           if (b=="NE" )
		         {
		         ...
		         if (k!="" && parseInt(k)>200000 && b=="NE")
		           {
		            ...
		            if (b=="FT"){
		                    if (pfsct!="UTIB" )
		                {
		                    alert("Can not be Bank TO Bank--1");
		                    c.value="";  
		                    document.frm.SELECT_TYPE[b].focus();
		                }
		            }
...
<form action="" name="frm" id="frm" method="post" enctype="multipart/form-data">
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
		<td class="hd" valign="middle" style="text-align: center; height: 25px">Payment Ref.No.Creation - Axis (Fbad)
		</td>
		</tr>
	</table>
							...
							<table border="0" cellpadding="0" cellspacing="0" width="100%">
								<tr height="40px">
									...
									<td style="text-align: right; vertical-align: middle;">
										...
										<button class="sexybutton" type="button" onclick="window.location.href='clearRefNoActionAxis'"><span><span><span class="undo">Clear</span></span></span></button>&nbsp;&nbsp;
										...
										<tr bgcolor="" style="font-size: 8pt;background-color: #f0f0f0;color:#A8A8A8;">
											...
											<td style="text-align: left" colspan="6">												
                                                                                          <div id="details<s:property value='%{#st.index}'/>"></div>
                                                                                                                         ...
                                                                                                                         <td style="text-align: left;">
				<a href="#" onclick="openpop('attach<s:property value="%{#st.index}"/>');"><img src="image/attachment.gif" style="border-width:0px;"></a>
					<div id="attach<s:property value="%{#st.index}"/>" style="right: 50px;  display: none" class="root1">
						<table cellpadding="0" cellspacing="0">
							<tr bgcolor="#000080">
								<td width="100%">
									<div id="handle" class="handle1" style="cursor: move">Shahi Exports Pvt. Ltd</div>
								</td>
								<td style="width: 10px"><a href="#" onclick="closediv('attach<s:property value="%{#st.index}"/>')"><img border="0" width="18px"
									height="18px" src="image/deleteFile.gif" /></a>
								</td>
							</tr>
							<tr height="40px" >
								<td colspan="2" style="background-color: #e3eefb;vertical-align: middle;" class="label-1" >&nbsp;Attach
									<s:file name="attachfile" id="attachfile_ID%{#st.index}" theme="simple" cssStyle="width:80%"> </s:file>																	
								</td>
							</tr>
						</table>
					</div>
