...
<script type="text/javascript">
	    function loadtotal(){
				$("#detailstot").load('GnrtreftoticiciNM.jsp');
			}
            function loadallpages(){
            	for(i=0;i<15;i++){
            		$("#details"+i).load('searchRefNoActionHdfcSMANM.action?textid='+i);
            	}
            }
            function loadsinglerow(a,b,c,d,e){
            	var d1 = new Date();
            	$("#details"+a).load('searchRefNoActionHdfcSMANM.action?textid='+a+'&XYEAR='+b+'&XBANK='+c+'&XCHQ_NO='+d+"&COMPID="+e);
            ...
            function save()
			{
				if(validateSave()==true)
				{
					if(confirm("Do you want to save!!!")){
						document.frm.action="saveRefNoActionHdfcSMANM";
					    document.frm.submit();
					}
				}
			}
				...
				YEAR=document.getElementById(id);
				 ...
				 if(YEAR.value!='' && YEAR.value.length>0 && BANK.value!='' && BANK.value.length>0 && CHQ_NO.value!='' && CHQ_NO.value.length>0){
	            	 ...
	            	 if(duplicateCheck() && validateYear(a) && validateChequeNo(a)){
	            		 loadsinglerow(a,abc,BANK.value,CHQ_NO.value,document.getElementById('COMPID').value);
	            	}	            	 
	             }
				 else
        		 {
			...
			{
				$("#details"+a).load('searchRefNoActionHdfcSMANM.action?textid='+a);
		...
		function openpopdtl(a,b){
			document.getElementById(a).style.display = '';
			document.getElementById('searchdetail').src="searchdtRefNoActionHdfcSMANM.action?TINDEX="+b;
		}
		
		
		function refno()
		{
			document.frm.action="refnoRefNoActionHdfcSMANM";
		    document.frm.submit();
		}
		...
		if (pfsct!=null && pfsct!="" )
		  {
		 // alert("1"+b+pfsct);
		     if(k!="" && parseInt(k)<200000 && pfsct!="HDFC" )
		        {
		           if (b=="NE" )
		         {
		         
		         }else{
		         alert("Can be NEFT Only.-2");
		         c.value="";
		         ...
		         if(k!="" && parseInt(k)>200000 && b=="NE")
		           {
		            ...
		            if(k!="" && pfsct=="HDFC" && b!="FT")
		           {
		            alert("Can only be Bank TO Bank.-3");
		            c.value="";
		            document.frm.SELECT_TYPE[b].focus();
		            }
		            else
...
<form action="" name="frm" id="frm" method="post" enctype="multipart/form-data">
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			<td class="hd" valign="middle" style="text-align: center; height: 25px">Payment Ref.No.Creation-SMA(HDFC)
			</td>
		</tr>
	</table>
							...
							<table border="0" cellpadding="0" cellspacing="0" width="100%">
								<tr height="40px">
									...
									<td style=" vertical-align: middle;text-align: right;">&nbsp;
                                                                        ...
                                                                        <td class="label-1" style="vertical-align: middle;"> Company&nbsp;Type&nbsp;
										<s:select list="#{'700':'SMA','710':'HSS'}" name="COMPID" id="COMPID" value="%{COMPID}" theme="simple"></s:select>
									</td>
									<td style="text-align: right; vertical-align: middle;">
										<button class="sexybutton" type="button" onclick="save();"><span><span><span class="save">Save</span></span></span></button>&nbsp;&nbsp;
										<button class="sexybutton" type="button" onclick="window.location.href='clearRefNoActionHdfcSMANM'"><span><span><span class="undo">Clear</span></span></span></button>&nbsp;&nbsp;
                                ...
                                <thead>
	                            	...
	                            	<tr style="height:20px;">
										...
										<th valign="middle" style="text-align: left;width:130px;padding-left: 5px" class="label-1">Vch. No.</th>
                                                                                <th valign="middle" style="text-align: left;width:130px;padding-left: 5px" class="label-1">IFSC</th>
