...
<html>
    <head>
        ...
        <script language="javascript" type="text/javascript">
            ...
            function query()
            {                
                    document.balconf.action="DataTexPurchaseGstSflAction";
            ...
            {           
            	if(confirm('Do you want to upload')){
                    document.balconf.action="saveDataTexPurchaseGstSflAction";
                    document.balconf.submit();
            ...
            function enabledchkbx(a,b){
                ...
                var dropdown=document.getElementById(id).value;
            	if(dropdown=='Y'){
            ...
            function toggleNonTds(rowindex){
            	tds="nontds"+rowindex;
            	id="CHKTDS_id"+rowindex;
            	var dropdown=document.getElementById(id).value;
            	var amount=document.getElementById(tds).value;
            	alert('amount'+amount);
            	if(dropdown=='Y'){
            		document.getElementById(tds).disabled=false;
            	}else{
            ...
            function validatenontds(netamount,nontdsamount,index){
            	var netamt=parseFloat(netamount);
            	var nontdsamt=parseFloat(nontdsamount);
            	if(nontdsamt>netamt){
            		alert('Non Tds amount must be less than Net Amount');
            		//document.getElementById('nontds'+index).focus();
            		return false;
            	}
            	
            	if(nontdsamt<0){
            		alert('Non Tds amount must be greater than Zero');
            		//document.getElementById('nontds'+index).focus();
            		return false;
            	}
            }
            
            function isNumberic(pattern,index){
            	 if (!pattern.match(/^[0-9]\d*(\.\d{0,2})?$/)){
            		alert('Invalid Input, Only Two Decimal numbers Allowed');
            		//document.getElementById('nontds'+index).focus();
            		return false;
            	}
                        ...
                        <table width="100%" border="0">
                            <tr>                                
                                ...
                                <td style="width:120px;">
                                    ...
                                    <s:else>
                                        <s:select name="DIVI" cssClass="txtField" id="DIVI" list="#{'101':'Shimoga','121':'SFL'}" cssStyle="text-transform:uppercase;width:100%" theme="simple"/>
                                    </s:else>
                                </td>
                                ...
                                <td class="label-1" colspan="5" style="text-align:right;">
                                    ...
                                    <input type="reset" class="submitbutton" onclick="window.location.href='makesessionDataTexPurchaseGstSflAction.action'" value="Clear" style="width:55px"/>&nbsp;&nbsp;				    
                            ...
                            <table id="tablea"  cellspacing="1" cellpadding="0" style="width:100%;background-color: gray">
                                ...
                                <tbody>
                                	<s:iterator value="detaillist" status="detailstatus">
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].DIVISIONCODE" id="DIVISIONCODE%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].ACCOUNTINGDATE" id="ACCOUNTINGDATE%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].CUSTOMERSUPPLIERCODE" id="CUSTOMERSUPPLIERCODE%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].INVOICENO" id="INVOICENO%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].INVOICEDATE" id="INVOICEDATE%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].INVAMT" id="INVAMT%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].GLAMT" id="GLAMT%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].NETAMT" id="NETAMT%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].QTY" id="QTY%{#detailstatus.index}"/>
                                	 <s:hidden name="detaillist[%{#detailstatus.index}].NARRATION" id="NARRATION%{#detailstatus.index}"/>
                               		<s:if test="%{DCHK=='OK'}">
                                		...
                                		<s:else>
	                                    	...
	                                    	<td align="left">
	                                    		<s:textfield   id="nontds%{#detailstatus.index}" value="%{nontdsAmount}"  name="detaillist[%{#detailstatus.index}].nontdsAmount" theme="simple"   disabled="%{TCHK==null}"
	                                    		  onblur="validatenontds('%{NETAMT}',this.value,'%{#detailstatus.index}');" />
	                                    	</td>
	                                    	<td> 
	                                    			<select name="CHKTDS" id="CHKTDS_id<s:property value='%{#detailstatus.index}'/>" disabled="disabled" onchange="toggleNonTds(<s:property value='%{#detailstatus.index}'/>)">
	                                    				<s:if test="%{TCHK=='OK'}">
	                                    					<option value="Y">Yes</option>
	                                    					<option value="N">No</option>
	                                    					<%-- <s:hidden name="rowId" value="%{#detailstatus.index}"/> --%>
	                                    				</s:if>
	                                    				<s:else>
		                                    				<option value="N">No</option>
	                                    				</s:else>
	                                    	...
	                                    	<td>
	                                    		...
	                                    		<s:else--%>
	                                            	<s:checkbox name="chkdel"  id="chkdel_id%{#detailstatus.index}" theme="simple" fieldValue="%{CUSTOMERSUPPLIERCODE+'$$'+INVOICENO+'$$'+FINYEAR+'$$'+rowId}" onclick="enabledchkbx('%{#detailstatus.index}',this)" value='Y'/>
                                                <%--/s:else--%>                                                
                                            </td>
                                		</tr>
                                	</s:iterator>
