...
import shahi.Action.M4bill.Beans.M3BillReportBean;
import shahi.Action.M4bill.Beans.M3InvNoBean;
 ...
 * @author Shilpa
 */
public class M3BillNoReport extends ActionSupport implements ServletRequestAware,ServletResponseAware{
		...
		try
		{
			...
			resultSet1 = stat.executeQuery();
                        ...
                        {  
                            ...
                            stat = conn.prepareStatement("select SL_NO,TYPE_CODE,TYPE_DESC,DEPT_SL_NO from m4_bill_type_master where DEPT_SL_NO in (select sl_no from m4_bill_dept_master where DEPT_CODE=? and FLAG='Y') and FLAG='Y' order by TYPE_DESC");
                            stat.setString(1,rightDepartment.get(i).toString());
                            resultSet = stat.executeQuery();
                            while(resultSet.next()){
	...
	public String getbillsubtype() throws Exception{
		...
		try
		{
			...
			{
                         stat = conn.prepareStatement("select SL_NO,TYPE_CODE,TYPE_DESC from m4_bill_type_master where SL_NO=? and FLAG='Y'  order by TYPE_DESC");
                            ...
                            if(resultSet1.next()){   
                                ...
                                stat = conn.prepareStatement("select sl_no,sub_type_code,sub_type_desc from m4_bill_sub_type_master where type_sl_no=? and  FLAG='Y' order by sub_type_desc");
                                stat.setString(1,rightBill.get(i).toString());
                                resultSet = stat.executeQuery();
                                while(resultSet.next()){
		...
		try
		{
			...
			for(int i=0; i<rightBill.size();i++)
			{
                            ...
                            if(resultSet1.next()){   
                                    ...
                                    stat = conn.prepareStatement("select SL_NO,CC_CODE from M4_CC_MASTER where  TYPE_SL_NO=? and  FLAG='Y' order by CC_CODE");
                                    stat.setString(1,rightBill.get(i).toString());

                                    resultSet = stat.executeQuery();
                                    while(resultSet.next()){
                                        String CC_DESC="";
                                                stat=connbi.prepareStatement("select EAAITM,EATX40 from prodbi.fchacc  where EACONO=111 and  EAAITP=3 and eaat12=0 and EAAITM=? ");
	...
	public String getproducttype() throws Exception{
		...
		try
		{
			...
			{
                                stat = conn.prepareStatement("select sl_no,sub_type_code,sub_type_desc from m4_bill_sub_type_master where sl_no=? and FLAG='Y'");
                                ...
                                stat = conn.prepareStatement("select SL_NO ,PRODUCT_CODE,PRODUCT_DESC from M4_BILL_PRODUCT_MASTER where SUB_TYPE_SL_NO=?  and FLAG='Y' order by PRODUCT_DESC");
                                stat.setString(1, rightsubbill.get(i).toString());
                                resultSet = stat.executeQuery();
                                while(resultSet.next()){
	...
	public String printpdf() throws Exception {
     ...
     //  System.out.println(warehouse +" ::: "+BILL_DATE1+"  ::: "+BILL_DATE2);
         ...
         try
        {
          ...
          String billnosql=null;
           if(rightbillno!=null && rightbillno.size()>0)
          {
              for(int i=0; i<rightbillno.size(); i++)
             {
             if(billnosql==null)
             {
             billnosql="'"+rightbillno.get(i).toString()+"'";
             }else{
              billnosql=billnosql+",'"+rightbillno.get(i).toString()+"'";
             }
             
          }
          }
           // System.out.println("shilpa "+billnosql);
           
          if(billnosql!=null && billnosql.length()>0)
          {
             
            billnosql=" and BILL_No in("+billnosql+")";  
          }else{
          billnosql=" ";
          }
         ...
         // System.out.println(rightproductsql);
          
          if(BILL_DATE1!=null && BILL_DATE1.length()>0 && BILL_DATE2!=null && BILL_DATE2.length()>0)
          {
              if(monthtype!=null && monthtype.equals("M"))
              {
               datesql ="and (a.sl_no) in (select bill_sl_no from M4_BILL_month_DETAIL where to_date(bill_month,'dd/yyyy')  between to_date('"+BILL_DATE1+"','dd/yyyy') and to_date('"+BILL_DATE2+"','dd/yyyy')) ";
              }else
              {
               // datesql =" and to_char(a.bill_date,'mm/yyyy')  between '"+BILL_DATE1+"' and '"+BILL_DATE2+"'";
                datesql =" and to_date(to_char(a.bill_date,'mm/yyyy'),'mm/yyyy')  between to_date('"+BILL_DATE1+"','mm/yyyy') and to_date('"+BILL_DATE2+"','mm/yyyy')";
          ...
          {
            qry+=" select  a.BILL_NO,a.DEPT_SL_NO,c.DEPT_DESC,0 type_sl_no,'All' TYPE_DESC, " +
           ...
           qry+="'All' CC_CODE_DESC, " +
            ...
            " m4_cc_master f,prodbi.fchacc@movex.world@movex g,M4_BILL_PRODUCT_MASTER h " +
            ...
            " and a.DEPT_SL_NO in(select  SL_NO from m4_bill_dept_master where DEPT_CODE in("+deptin+")) "+wareshouesql+" "+rightpchsql+" "+datesql+" "
              +billnosql+ "  group by a.BILL_NO,a.DEPT_SL_NO,c.DEPT_DESC ";
              if(rightpchsql.trim().length()==0)
                {
                 qry+=" ";
                }else{
            ...
            qry+=" order by c.DEPT_DESC ";
                  ...
                  {
                   qry+=" select  a.BILL_NO,a.DEPT_SL_NO,c.DEPT_DESC,b.type_sl_no,d.TYPE_DESC ||'-'|| d.TYPE_CODE TYPE_DESC, " +
            " 0 SUB_TYPE_SL_NO,'All' SUB_TYPE_DESC, ";
               if(rightpchsql.trim().length()==0)
                    {
                     qry+=" 'All' PCH, ";
            ...
            " "+rightpchsql+" "+rightcostsql+" "+datesql+" "+billnosql +" group by a.BILL_NO,"
                     + "a.DEPT_SL_NO,c.DEPT_DESC ,b.type_sl_no,d.TYPE_DESC ||'-'|| d.TYPE_CODE " ;
              if(rightpchsql.trim().length()==0)
                {
                 qry+=" ";
                }else{
            ...
            else if (rightBill!=null && rightBill.size()>0 && rightsubbill!=null && rightsubbill.size()>0)
            {
                      ...
                      qry += " sum(round(b.product_amount/(months_between(to_date(bill_date2,'mm/yyyy'),to_date(bill_date1,'mm/yyyy'))+1),2)) "
                              ...
                              + " where a.sl_no=b.bill_sl_no and a.DEPT_SL_NO=c.SL_NO and b.type_sl_no=d.SL_NO AND b.SUB_TYPE_SL_NO=e.SL_NO "
                              + " and b.CC_CODE=f.sl_no and f.cc_code=g.EAAITM and g.EAAITP=3 and g.eaat12=0 and b.PRODUCT_SL_NO=h.SL_NO "
                              + " and a.DEPT_SL_NO in(select  SL_NO from m4_bill_dept_master where DEPT_CODE in(" + deptin + ")) "+wareshouesql+" "
                              + rightBillsql
                              + " " + rightpchsql + " "+rightcostsql+" "+rightproductsql+" "+datesql+" "+billnosql +" "
                              + " group by a.BILL_NO,a.DEPT_SL_NO,c.DEPT_DESC ,"
                              + "b.type_sl_no,d.TYPE_DESC ||'-'|| d.TYPE_CODE,b.SUB_TYPE_SL_NO,e.SUB_TYPE_DESC  ||'-'|| e.SUB_TYPE_CODE";
                      if (rightpchsql.trim().length() == 0) {
                          qry += " ";
                      } else {
                          qry += " ,b.PCH";
  ...
  f.CC_CODE ||'-'|| g.EATX40 ||'-'|| b.CC_CODE,h.PRODUCT_DESC ||'-'|| h.PRODUCT_CODE
          */
           ...
           // System.out.println(qry);
            ...
            InputStream input = new FileInputStream(new File(path + "/M3BillNoReport.jrxml"));
            ...
            while(result5.next())
            {
                M3BillReportBean bean=new M3BillReportBean();
                bean.setDEPT_SL_NO(result5.getString("DEPT_SL_NO"));
                bean.setDEPT_DESC(result5.getString("DEPT_DESC"));
                bean.setTYPE_SL_NO(result5.getString("TYPE_SL_NO"));
                bean.setTYPE_DESC(result5.getString("TYPE_DESC"));
                bean.setSUB_TYPE_SL_NO(result5.getString("SUB_TYPE_SL_NO"));
                bean.setSUB_TYPE_DESC(result5.getString("SUB_TYPE_DESC"));
                bean.setPCH(result5.getString("PCH"));
                bean.setCC_CODE_DESC(result5.getString("CC_CODE_DESC"));
                bean.setPRODUCT_SL_NO(result5.getString("PRODUCT_SL_NO"));
                bean.setPRODUCT_DESC(result5.getString("PRODUCT_DESC"));
                bean.setPRODUCT_AMOUNT(result5.getDouble("PRODUCT_AMOUNT"));
                bean.setBILL_NO(result5.getString("BILL_NO"));
                
                List Invlist=new ArrayList();
              
                slqry="select distinct inv_no from M4_BILL_ADD_MASTER i ,m4_bill_master a ,m4_bill_detail b where "
                        + " i.bill_sl_no=a.sl_no and a.sl_no=b.bill_sl_no and inv_no is not null and "
                        + " a.DEPT_SL_NO in (select  SL_NO from m4_bill_dept_master where DEPT_CODE in(" + deptin + ")) "
                        + " and bill_no ='"+result5.getString("BILL_NO") +"' ";
                
                if(rightBill==null || (rightBill!=null && rightBill.size()==0))
                {
                   slqry+=rightpchsql+" "+datesql;
                }
                else if (rightBill!=null && rightBill.size()>0 && (rightsubbill==null ||(rightsubbill!=null && rightsubbill.size()==0)))
                {
                    slqry+=rightBillsql+" "+rightpchsql+" "+rightcostsql+" "+datesql;
                }
                else if (rightBill!=null && rightBill.size()>0 && rightsubbill!=null && rightsubbill.size()>0)
                {
                    slqry+= rightBillsql+ " " + rightpchsql + " "+rightcostsql+" "+rightproductsql+" "+datesql;
                }
                
                
                //System.out.println(slqry);
                PreparedStatement stat5=con.prepareStatement(slqry);
                ResultSet rs=stat5.executeQuery();
                String inv_no="";
                M3InvNoBean bean1=new M3InvNoBean();
                while(rs.next())
                {
                    ...
                    {
                        inv_no=inv_no+","+rs.getString("INV_NO");
                    }
                 }
                 bean1.setINV_NO(inv_no);
                 Invlist.add(bean1);
               
                bean.setINV_LIST(Invlist);
                
                DETAIL_LIST.add(bean);
            }
            JRBeanCollectionDataSource resultSetDataSource=new JRBeanCollectionDataSource(DETAIL_LIST);
...
public List<M3BILLBean> getM3costcenterlist() {
    ...
    public List getRightbillno() {
        return rightbillno;
    }

    public void setRightbillno(List rightbillno) {
