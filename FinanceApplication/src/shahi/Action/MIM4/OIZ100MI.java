...
import shahi.Action.MIM4.BaseMI;
...
public class OIZ100MI extends BaseMI {
   ...
   public List<OrderViewBean> LstHeadByStyle(String ITNO) {
    ...
    public List<shahi.Action.PcdEntry.Beans.OrderViewBean> LstHeadByStyleless66(String ITNO, List<shahi.Action.PcdEntry.Beans.OrderViewBean> list1) {
        ...
        if(ITNO!=null && ITNO.trim().length()>0 && ITNO.trim().length()>3)
        {
            ...
            if (recFlag > 0) {
              //  CRMLogHelper.logMessage(HelperConstantsFnl.SYSTEM_EXCEPTION, getClass().getName(), "Returned from applicationserver " + identity + javaMI.mvxGetLastError(), CRMLogHelper.STR_ERROR_PRIORITY);
                ...
                while (javaMI.mvxMore()) {
                    String st=javaMI.mvxGetField("ORSL");
                    //if(st!=null && Integer.parseInt(st)<66)
                    //{
...
public List<OrderLineBean> LstLine(String ORNO) 
{
        ...
        OrderLineBean olb = new OrderLineBean();
        int recFlag;
...
//                String ORQT = javaMI.mvxGetField("ORQT");
                ...
                olb = new OrderLineBean(PONR, ORNO, ITNO, ORQT, WHLO, SAPR, DIP1, DWDT, ITDS, ADID, ORST, TEDS,ALQT,NEPR,LNAM,UCOS,MODL,"","","No","",TEDL,SACD,CUCD);
...
public Map<String,String> LstLineMap(String ORNO) 
{
        ...
        if (recFlag > 0) {
            ...
            while (javaMI.mvxMore()) {
                ...
                listlinemap.put(ORNO+"-"+PONR,DWDT);
...
String status;
  ...
  public List<OrderPcdBean> ListFreeFieldIndataApprovalNew(String HDPR) {
   ...
   public Map<String ,List> ListFreeFieldIndataApprovalNewMAP(String HDPR) {
        ...
        if (recFlag > 0) {
            ...
            while (javaMI.mvxMore()) {
                List list1 = new ArrayList();
                 ...
                 String ponr1=javaMI.mvxGetField("PONR");
                 String exfacdate=javaMI.mvxGetField("CFD4");
                 String orno=javaMI.mvxGetField("ORNO");
                 if(date!=null && date.length()>0 && !date.equals("0"))
                {
                 date=date.substring(6, 8) +"-" + date.substring(4, 6) + "-" +date.substring(0, 4);
                }else{
                ...
                try{
                   if(Maplist1.get(orno)==null)
                   { 
                       Maplist1.put(orno, list1);
                   }else{
                       List ls=(List)Maplist1.get(orno);
                       
                       ls.add(list1);
                     Maplist1.put(orno,ls);
                   }
