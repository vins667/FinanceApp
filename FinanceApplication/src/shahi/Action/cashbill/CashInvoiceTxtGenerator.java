...
import static shahi.Action.cashbill.CashBillTxtGenerator.aResourcBundle;
 ...
 * @author Vivek
 */
public class CashInvoiceTxtGenerator {
    ...
    public CashInvoiceTxtGenerator() {
    ...
    public String txtGenerator(List<String> srNo, String userId) {
        ...
        try {
            ...
            String MINTE = null;
            String DIVI = null;
            String REALFILENAME = "/home/tommy/apache-tomcat-8.0.24/webapps/ShahiApplication/shahiwebpages/cashinvoice/txt/";//
            BufferedWriter bw = null;
            stat = conn.prepareStatement("SELECT M4_invoice_GLS_MASTER_SQ.NEXTVAL FROM DUAL");
            ...
            for (int i = 0; i < srNo.size(); i++) {
                ...
                while (resultSet.next()) {
                    ...
                    bw.write(StringUtils.rightPad(resultSet.getString("SUPPLIER_CODE"), 10, " "));
                    bw.write(StringUtils.rightPad(resultSet.getString("BILL_NO"), 10, " "));
                    bw.write(StringUtils.rightPad(resultSet.getString("BILL_DATE"), 8, " "));
                    bw.write(StringUtils.rightPad(resultSet.getString("BILL_DATE"), 8, " "));
                    if (resultSet.getString("CASH_TYPE") != null && resultSet.getString("CASH_TYPE").equalsIgnoreCase("CR")) {
                        bw.write(StringUtils.leftPad("-" + resultSet.getString("BILL_AMOUNT"), 17, " "));
                    } else {
                        bw.write(StringUtils.leftPad(resultSet.getString("BILL_AMOUNT"), 17, " "));
                    }
                    bw.write(StringUtils.rightPad("", 19, " "));
                    bw.write(resultSet.getString("CURRENCY"));
                    bw.write(StringUtils.rightPad("1", 2, " "));
                    String CONV = "1";
                    if(resultSet.getString("CURRENCY")!=null && resultSet.getString("CURRENCY").length()>0 && !resultSet.getString("CURRENCY").equals("INR")){
                        stat1 = connhris.prepareStatement("SELECT CUARAT FROM  prodbi.CCURRA C"
                                    +" WHERE c.cucutd = (select max(y.cucutd) from prodbi.ccurra y"
                                    +" where c.cucono = y.cucono  and c.cucucd = y.cucucd"
                                    +" and  c.cucrtp = y.cucrtp and c.culocd = y.culocd and y.cucutd <= ? )"
                                    +" and c.cucono = 111 and c.cucucd = ? and cucrtp = 1"
                                    +" and culocd = 'INR'");
                        stat1.setString(1, resultSet.getString("VCH_DATE"));
                        stat1.setString(2, resultSet.getString("CURRENCY").trim());
                        resultSet1 = stat1.executeQuery();
                        if(resultSet1.next()){
                            CONV = resultSet1.getString("CUARAT");
                        }
                        if(resultSet1!=null) resultSet1.close();
                        if(stat1!=null) stat1.close();
                    }
                    String GL_CODE="15102";
                    if(resultSet.getString("CURRENCY")!=null && resultSet.getString("CURRENCY").length()>0 && !resultSet.getString("CURRENCY").equals("INR")){
                        GL_CODE="15100";
                    }
                    bw.write(StringUtils.leftPad(CONV, 13, " "));
                    bw.write(StringUtils.rightPad(resultSet.getString("VCH_DATE"), 8, " "));
                    bw.write(StringUtils.rightPad("", 40, " "));
                    bw.write(StringUtils.rightPad(GL_CODE, 8, " "));
                    bw.write(StringUtils.rightPad("COR", 8, " "));
                    if(resultSet.getString("BILL_WHLO")!=null && resultSet.getString("BILL_WHLO").equals("100")){
                        bw.write(StringUtils.rightPad("HR000000", 8, " "));
                    } else if(resultSet.getString("BILL_WHLO")!=null && resultSet.getString("BILL_WHLO").equals("200")){
                        bw.write(StringUtils.rightPad("KA000000", 8, " "));
                    } else{
                        bw.write(StringUtils.rightPad("00000000", 8, " "));
                    }
                    bw.write(StringUtils.rightPad("", 8, " "));
                    bw.write(StringUtils.rightPad(resultSet.getString("SUPPLIER_CODE"), 8, " "));
                    bw.write(StringUtils.rightPad("", 8, " "));
                    bw.write(StringUtils.leftPad("0", 15, " "));
                    bw.write(StringUtils.rightPad("BT", 2, " "));
                    bw.write(StringUtils.rightPad("BTR", 3, " "));
                    bw.write(StringUtils.rightPad("008", 3, " "));
                    bw.write(StringUtils.rightPad(resultSet.getString("REPORT_NO"), 8, " "));
                    bw.write("\n");
                    stat1 = conn.prepareStatement("SELECT SL_NO, BILL_SL_NO, CC_CODE, TYPE_SL_NO, SUB_TYPE_SL_NO, PRODUCT_SL_NO, PRODUCT_AMOUNT, TDATE, USER_ID, PCH, BILL_DATE1, BILL_DATE2, REMARKS, TAXABLE, NON_GST_AMOUNT, HSN_CODE, VOUCHER_DESC, NVL(ORDER_NO,' ') ORDER_NO, QUANTITY, NVL(PRODUCT_QUANTITY,'0') PRODUCT_QUANTITY,NVL(UOM,' ') UOM FROM M4_INVOICE_DETAIL WHERE BILL_SL_NO=?");
                    ...
                    while (resultSet1.next()) {
                        ...
                        stat2 = connhris.prepareStatement("SELECT EAAITM FROM SEPLWEB.M4_CC_MASTER@IBM.WORLD@IBM A,PRODBI.FCHACC B WHERE EACONO=111 AND A.CC_CODE=B.EAAITM AND EAAITP=3 AND DEPT_SL_NO='94' AND EAAT12=0 AND SL_NO=?");
                        ...
                        bw.write(StringUtils.rightPad(resultSet.getString("SUPPLIER_CODE"), 8, " "));
                        bw.write(StringUtils.rightPad(resultSet1.getString("ORDER_NO"), 8, " "));
                        bw.write(StringUtils.leftPad(resultSet1.getDouble("PRODUCT_QUANTITY") + "", 15, " "));
                        bw.write(StringUtils.rightPad("BT", 2, " "));
                        bw.write(StringUtils.rightPad("BTR", 3, " "));
                        bw.write(StringUtils.rightPad("008", 3, " "));
                        bw.write(StringUtils.rightPad(resultSet.getString("REPORT_NO"), 8, " "));
                        bw.write(StringUtils.rightPad("", 44, " "));
                        ...
                        if(resultSet.getString("CURRENCY")!=null && resultSet.getString("CURRENCY").length()>0 && resultSet.getString("CURRENCY").equals("INR")){
                            stat2 = conn.prepareStatement("SELECT SL_NO, COST_ELEMENT, AMT, FORM_TYPE, TDATE, USER_ID, BILL_SL_NO, FORM_WHLO, GST_PER, TAX_CODE, LINE_SL_NO"
                                    + " FROM M4_INVOICE_BREAKUP_DETAIL WHERE BILL_SL_NO=? AND LINE_SL_NO=?");
                            ...
                            while (resultSet2.next()) {
                                ...
                                stat3 = connhris.prepareStatement("SELECT EAAITM FROM SEPLWEB.M4_CC_MASTER@IBM.WORLD@IBM A,PRODBI.FCHACC B WHERE EACONO=111 AND A.CC_CODE=B.EAAITM AND EAAITP=3 AND DEPT_SL_NO='94' AND EAAT12=0 AND SL_NO=?");
                                ...
                                if (resultSet3.next()) {
                                    ...
                                    bw.write(StringUtils.rightPad("", 10, " "));
                                }
                                if (resultSet3 != null) {
                                    resultSet3.close();
                                }
                                ...
                                bw.write(StringUtils.leftPad("", 15, " "));
                                bw.write(StringUtils.rightPad(resultSet1.getString("UOM"), 3, " "));
                                bw.write("\n");
            ...
            subjecttitle = "Cash Bill (M4) :-" + FILE_NAME;
    ...
    public static void main(String[] args) {
        CashInvoiceTxtGenerator cashInvoiceTxtGenerator = new CashInvoiceTxtGenerator();
        List list = new ArrayList<String>();
        list.add("2");
        cashInvoiceTxtGenerator.txtGenerator(list, "237519");
    }
}
