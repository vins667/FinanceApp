 ...
 * @author Vivek
 */
public class CashInvoiceEntryAction extends ActionSupport {
    ...
    private List<M3BILLBean> CURRENCY_LIST;
    ...
    private List<String> OPERATION_DESC;
    ...
    public String mksess() throws SQLException, Exception {
        ...
        if (usrid != null) {
            ...
            try {
                ...
                System.out.println("CashInvoiceEntryAction + mksess() " + e);
                return SUCCESS;
            } finally {
                if (result != null) {
                    result.close();
    ...
    public String execute() throws Exception {
        ...
        try {
            ...
            if(ACT_FLAG!=null && ACT_FLAG.equalsIgnoreCase("CONTROL") && CONTROL_NO_LIST!=null && CONTROL_NO_LIST.size()>0 && !CONTROL_NO_LIST.get(0).equalsIgnoreCase("false")){
                String ctno = "0";
                stat = conn.prepareStatement("SELECT M4_INVOICE_CASH_MASTER_CTN_SQ.NEXTVAL FROM DUAL");
                ...
                for (int i = 0; i < CONTROL_NO_LIST.size(); i++) {
                    stat = conn.prepareStatement("UPDATE M4_INVOICE_MASTER SET REPORT_NO=?  WHERE SL_NO=?");
            ...
            if(ACT_FLAG!=null && ACT_FLAG.equalsIgnoreCase("CONTROL") && DEL_CONTROL_NO_LIST!=null && DEL_CONTROL_NO_LIST.size()>0 && !DEL_CONTROL_NO_LIST.get(0).equalsIgnoreCase("false")){
                for (int i = 0; i < DEL_CONTROL_NO_LIST.size(); i++) {
                    stat = conn.prepareStatement("UPDATE M4_INVOICE_MASTER SET REPORT_NO=null  WHERE SL_NO=?");
            ...
            if(ACT_FLAG!=null && ACT_FLAG.equalsIgnoreCase("FORWARD") && FORWARD_USER_LIST!=null && FORWARD_USER_LIST.size()>0 && !FORWARD_USER_LIST.get(0).equalsIgnoreCase("false")){
                for (int i = 0; i < FORWARD_USER_LIST.size(); i++) {
                    stat = conn.prepareStatement("UPDATE M4_INVOICE_MASTER SET FORWARD_USER=? ,FORWARD_DATE=sysdate  WHERE SL_NO=?");
            ...
            if(ACT_FLAG!=null && ACT_FLAG.equalsIgnoreCase("APPROVE") && UNAPPROVE_BILL_LIST!=null && UNAPPROVE_BILL_LIST.size()>0 && !UNAPPROVE_BILL_LIST.get(0).equalsIgnoreCase("false")){
                for (int i = 0; i < UNAPPROVE_BILL_LIST.size(); i++) {
                    stat = conn.prepareStatement("UPDATE M4_INVOICE_MASTER SET FORWARD_USER=null ,FORWARD_DATE=null  WHERE SL_NO=?");
            ...
            if(ACT_FLAG!=null && ACT_FLAG.equalsIgnoreCase("APPROVE") && APPROVE_BILL_LIST!=null && APPROVE_BILL_LIST.size()>0 && !APPROVE_BILL_LIST.get(0).equalsIgnoreCase("false")){
                for (int i = 0; i < APPROVE_BILL_LIST.size(); i++) {
                    stat = conn.prepareStatement("UPDATE  M4_INVOICE_MASTER SET ACCOUNT_USER=? ,ACCOUNT_DATE=SYSDATE,VCH_DATE=to_date(?,'dd/MM/yyyy')  WHERE SL_NO=?");
                    stat.setString(1, usrid);
                ...
                new CashInvoiceTxtGenerator().txtGenerator(APPROVE_BILL_LIST,usrid);
            ...
            if(ACT_FLAG!=null && ACT_FLAG.equalsIgnoreCase("DELETE") && DELETE_BILL_LIST!=null && DELETE_BILL_LIST.size()>0 && !DELETE_BILL_LIST.get(0).equalsIgnoreCase("false")){
                for (int i = 0; i < DELETE_BILL_LIST.size(); i++) {
                    stat = conn.prepareStatement("DELETE FROM M4_INVOICE_DETAIL WHERE BILL_SL_NO=?");
                    stat.setString(1, DELETE_BILL_LIST.get(i).toString());
                    stat.executeUpdate();

                    stat = conn.prepareStatement("DELETE FROM M4_INVOICE_BREAKUP_DETAIL WHERE BILL_SL_NO=?");
                    stat.setString(1, DELETE_BILL_LIST.get(i).toString());
                    stat.executeUpdate();

                    stat = conn.prepareStatement("DELETE FROM M4_INVOICE_MASTER WHERE SL_NO=?");
                    stat.setString(1, DELETE_BILL_LIST.get(i).toString());
                    stat.executeUpdate();
                }
            ...
            if (FIN_STATUS != null && FIN_STATUS.equalsIgnoreCase("YES")) {
                ...
                stat = conn.prepareStatement("SELECT A.SL_NO,DEPT_SL_NO,BILL_NO,TO_CHAR(BILL_DATE,'dd/mm/yyyy') BILL_DATE,SUPPLIER_CODE,BILL_AMOUNT,GROSS_AMOUNT,b.DEPT_DESC DEPT_DESC ,to_char(a.FORWARD_DATE,'dd/mm/yyyy') FORWARD_DATE,A.SACONM IDSUNM, FORWARD_USER,BILL_WHLO,REPORT_NO ,REVERSE_SRVTAX,SRVTAX_GL_CODE,BILL_YEAR,A.ACCOUNT_USER, TO_CHAR(A.ACCOUNT_DATE,'dd/mm/yyyy') ACCOUNT_DATE FROM M4_INVOICE_MASTER a,M4_BILL_DEPT_MASTER b where a.DEPT_SL_NO=b.SL_NO AND DEPT_SL_NO in(select SL_NO from M4_BILL_DEPT_MASTER  WHERE  DEPT_CODE in (select DEPT_SL_NO from  m4_bill_user_master where emp_code=? and flag='Y')) AND DEPT_SL_NO='94' " + para + " order by SL_NO");
            ...
            System.out.println("CashInvoiceEntryAction " + se);
            addActionError("CashInvoiceEntryAction " + se);
        } finally {
            if (conn != null) {
                try {
                    conn.close();
                } catch (SQLException ex) {
                    Logger.getLogger(CashInvoiceEntryAction.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }
        return SUCCESS;
    ...
    public String newpage() {
        ...
        CURRENCY_LIST = new ArrayList<M3BILLBean>();
        ...
        try{
            ...
            CURRENCY_LIST = getCurr(connhris);
    ...
    public List getWarehouse(Connection conn, String USER_ID){
        ...
        try{
            stat = conn.prepareStatement("SELECT WAREHOUSE FROM M4_BILL_USER_MASTER WHERE EMP_CODE=? AND DEPT_SL_NO='75'");
    ...
    public List getBillType(Connection conn, String USER_ID){
        ...
        ResultSet result = null;
        try{
            stat = conn.prepareStatement("SELECT SL_NO,DEPT_SL_NO,TYPE_CODE,TYPE_DESC,FLAG from M4_BILL_TYPE_MASTER where DEPT_SL_NO='94' and FLAG='Y' AND TYPE_CODE in(SELECT BILL_TYPE FROM M4_BILL_USER_MASTER WHERE EMP_CODE=?)  ORDER BY  TYPE_DESC");
            stat.setString(1, USER_ID);
            result = stat.executeQuery();
            while(result.next()){
    ...
    public List getCurr(Connection conn){
        List<M3BILLBean> currlist = new ArrayList<M3BILLBean>();
        PreparedStatement stat = null;
        ResultSet result = null;
        try{
            stat = conn.prepareStatement("select ctstky, cttx15 from prodbi.csytab where CTCONO='111' and ctstco='CUCD' order by cttx15");
            result = stat.executeQuery();
            while(result.next()){
                currlist.add(new M3BILLBean(result.getString(1), result.getString(2)));
            }
        } catch(SQLException se){
            System.out.println("CashInvoiceEntryAction "+ se);
        } finally{
    ...
    public List getBillCC(Connection conn, String srNo){
        ...
        ResultSet result = null;
        try{
            stat = conn.prepareStatement("SELECT SL_NO,EAAITM,EATX40 FROM SEPLWEB.M4_CC_MASTER@IBM.WORLD@IBM A,PRODBI.FCHACC B WHERE EACONO=111 AND A.CC_CODE=B.EAAITM AND EAAITP=3 AND DEPT_SL_NO='94' AND TYPE_SL_NO=? AND EAAT12=0 ORDER BY 2");
            stat.setString(1, srNo);
            result = stat.executeQuery();
            while(result.next()){
    ...
    public static List<SupplierBean> getSupplier(String CONO, String query) throws SQLException {
        ...
        if (query != null && query.length() > 0) {
            ...
            try {
                ...
                stat = conn.prepareStatement("SELECT OKCUNO,OKCUNM,OKCUCD FROM OCUSMA WHERE OKCONO=? AND (OKCUNO LIKE ? OR OKCUNM LIKE ?) AND OKSTAT='20'");
                ...
                while (resultSet.next()) {
                    String GSTF = "";
                    String GSTD = "";
                    List<SupplierAddressBean> supplierAddressMasters = CustomerAddressMaster.getCustomerAddressWithoutConnection(conn, CONO, resultSet.getString("OKCUNO"), "1");
                    for (SupplierAddressBean supplierAddressBean : supplierAddressMasters) {
                        supplierBeans.add(new SupplierBean(resultSet.getString("OKCUNO"), resultSet.getString("OKCUNM"), GSTF, GSTD,resultSet.getString("OKCUCD"), supplierAddressBean));
                    }
                }
    ...
    public String gstcalc() throws SQLException{
        ...
        if(WAREHOUSE!=null && WAREHOUSE.length()>0 && CASH_TYPE!=null && CASH_TYPE.length()>0){
            Connection conn = null;
            PreparedStatement stat = null;
            ResultSet resultSet = null;
            try {
                ...
                if(CASH_TYPE!=null && CASH_TYPE.equals("DR")){
                    stat = conn.prepareStatement("select HSN_CODE,DN_GL_CODE GL_CODE from M4_BILL_HSN_GL where flag='Y' and REV_GL_CODE is null ");
                }else{
                    stat = conn.prepareStatement("select HSN_CODE,CN_GL_CODE GL_CODE from M4_BILL_HSN_GL where flag='Y' and REV_GL_CODE is null ");
                }
                resultSet= stat.executeQuery();
                while (resultSet.next()) {
    ...
    public String save() {
        ...
        try {
            ...
            int flag = 0;
            if (BILL_AMOUNT_BREAKUP != null && BILL_AMOUNT_BREAKUP.size() > 0) {
                ...
                if (UPD_MAST != null && UPD_MAST.length() > 0 && MAST_SL_NO != null && MAST_SL_NO.length() > 0) {
                    stat = conn.prepareStatement("UPDATE M4_INVOICE_MASTER SET BILL_AMOUNT=?,REVERSE_SRVTAX=?,CASH_TYPE=? WHERE SL_NO=?");
                    stat.setString(1, BILL_AMOUNT);
                    stat.setString(2, REVERSE_SRVTAX == null ? "0" : REVERSE_SRVTAX);//REVERSE_SRVTAX
                    stat.setString(3, CASH_TYPE);
                    stat.setString(4, MAST_SL_NO);
                    ...
                    for (int i = 0; i < BILL_TYPE_SLNO.size(); i++) {
                        if (BILL_TYPE_SLNO.get(i).toString() != null && BILL_TYPE_SLNO.get(i).toString().length() > 0 && BILL_AMOUNT_BREAKUP.get(i).toString() != null && BILL_AMOUNT_BREAKUP.get(i).toString().length() > 0 && Double.parseDouble(BILL_AMOUNT_BREAKUP.get(i).toString()) > 0) {
                            ...
                            stat = conn.prepareStatement("select M4_INVOICE_DETAIL_SQ.nextval from dual");
                            resultSet = stat.executeQuery();
                            if (resultSet.next()) {
                            ...
                            stat = conn.prepareStatement("INSERT INTO M4_INVOICE_DETAIL(SL_NO,BILL_SL_NO,CC_CODE,TYPE_SL_NO,SUB_TYPE_SL_NO,PRODUCT_SL_NO,PRODUCT_AMOUNT,NON_GST_AMOUNT,TDATE,USER_ID,PCH,BILL_DATE1,BILL_DATE2,REMARKS,TAXABLE,HSN_CODE,VOUCHER_DESC,ORDER_NO,PRODUCT_QUANTITY,UOM) values(?,?,?,?,?,?,?,?,sysdate,?,?,?,?,?,?,?,?,?,?,?)");
                            ...
                            stat.setString(17, OPERATION_DESC.get(i).toUpperCase().trim());
                            stat.setString(18, BILL_AMOUNT_QUANTITY.get(i));
                            stat.setString(19, BILL_AMOUNT_UOM.get(i));
                            flag = stat.executeUpdate();
                            if (stat != null) {
                                stat.close();
                            }
                            if (BREAKUP_COUNT.get(i) != null && Integer.parseInt(BREAKUP_COUNT.get(i)) > 0) {
                                for (int j = 0; j < Integer.parseInt(BREAKUP_COUNT.get(i)); j++) {
                                    String SUB_SL_NO = "";
                                    stat = conn.prepareStatement("SELECT M4_INVOICE_BREAKUP_DETAIL_SQ.NEXTVAL FROM DUAL");
                                    resultSet = stat.executeQuery();
                                    if (resultSet.next()) {
                                    ...
                                    PreparedStatement statb = conn.prepareStatement("INSERT INTO M4_INVOICE_BREAKUP_DETAIL(SL_NO, COST_ELEMENT, AMT, TDATE, USER_ID, BILL_SL_NO, GST_PER, TAX_CODE,LINE_SL_NO) VALUES(?,?,?,SYSDATE,?,?,?,?,?)");
                    ...
                    PreparedStatement stats = conn.prepareStatement("SELECT * FROM M4_INVOICE_MASTER WHERE BILL_NO=? AND SUPPLIER_CODE=? AND BILL_YEAR=?");
                    ...
                    if (resultSets.next()) {
                        ...
                        BILL_NO="";  														//changed on 03/05/2018 by saurav LPAD(ILANO,5,'0')
                        stat = conn.prepareStatement("SELECT IBKTP||ISTCD||SUBSTR(IYEAR,3,4)||LPAD(ILANO,4,'0') BILL_NO FROM M4_INVOICE_SERIES WHERE IWHLO=? AND ITYPE=? AND IYEAR=? AND FLAG='Y' AND ILANO<=IEDNO");
                        stat.setString(1, WAREHOUSE);
                        stat.setString(2, CASH_TYPE);
                        stat.setString(3, BILL_YEAR);
                        resultSet = stat.executeQuery();
                        if(resultSet.next()){
                            BILL_NO = resultSet.getString("BILL_NO");
                            stat1 = conn.prepareStatement("UPDATE M4_INVOICE_SERIES SET ILANO=ILANO+1 WHERE IWHLO=? AND ITYPE=? AND IYEAR=? AND FLAG='Y'");
                            stat1.setString(1, WAREHOUSE);
                            stat1.setString(2, CASH_TYPE);
                            stat1.setString(3, BILL_YEAR);
                            stat1.executeUpdate();
                        }
                        else{
                            addActionError("Number Series is not defined/exceed.");
                            conn.commit();
                            return "newpage";
                        }
                        conn.commit();
                        ...
                        if (MAST_SL_NO == null || (MAST_SL_NO!=null && MAST_SL_NO.length()==0)) {
                            stat = conn.prepareStatement("select M4_INVOICE_MASTER_SQ.nextval from dual");
                            resultSet = stat.executeQuery();
                            if (resultSet.next()) {
                        ...
                        stat = conn.prepareStatement("INSERT INTO M4_INVOICE_MASTER(SL_NO,DEPT_SL_NO,BILL_NO,BILL_DATE,SUPPLIER_CODE,BILL_AMOUNT,TDATE,USER_ID,GROSS_AMOUNT,BILL_WHLO,BILL_YEAR,remarks,REVERSE_SRVTAX,REVERSE_SRVTAX_RATE,SRVTAX_GL_CODE,REVERSE_SRVTAX_CODE,NON_SRVTAX_AMOUNT,SASUNM,SAADR1,SAADR2,SAADR3,SAADR4,SATOWN,SAECAR,SAPONO,SACSCD,HSN_CODE,NON_GST_AMOUNT,SACONM,SAADID,SASTDT,CASH_TYPE,CURRENCY) values(?,?,?,to_date(?,'dd/mm/yyyy'),?,?,sysdate,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,upper(?),?,?,?,?,?,?)");
                        stat.setString(1, MAST_SL_NO);
                        stat.setString(2, "94");//DEPT_SL_NO//cash sr no
                        stat.setString(3, BILL_NO);
                        ...
                        stat.setString(31, CASH_TYPE);
                        stat.setString(32, CURRENCY);
                        flag = stat.executeUpdate();
                        if (stat != null) {
                            stat.close();
                        }
            ...
            System.out.println("CashInvoiceEntryAction " + e.getMessage());
            try {
                conn.rollback();
            } catch (SQLException ex) {
                Logger.getLogger(CashInvoiceEntryAction.class.getName()).log(Level.SEVERE, null, ex);
        ...
        try {
            ...
            if (MAST_SL_NO != null && MAST_SL_NO.length() > 0) {                
                ...
                if(resultSet.next()){
                    ...
                    CASH_TYPE = resultSet.getString("CASH_TYPE");
                ...
                stat = conn.prepareStatement("SELECT A.SL_NO, A.BILL_SL_NO, A.CC_CODE, A.TYPE_SL_NO, A.SUB_TYPE_SL_NO, A.PRODUCT_SL_NO, A.PRODUCT_AMOUNT, A.TDATE, A.USER_ID,"
                            ...
                            +" C.SUB_TYPE_DESC||'-'||SUB_TYPE_CODE SUB_TYPE_SL_NO_DESC,E.EAAITM ||'-'|| E.EATX40 CC_CODE_DESC,PRODUCT_DESC||'-'||PRODUCT_CODE PRODUCT_SL_NO_DESC,A.VOUCHER_DESC,A.ORDER_NO,"
                            +" A.PRODUCT_QUANTITY,A.UOM"
                            +" FROM M4_INVOICE_DETAIL A, M4_BILL_TYPE_MASTER B,M4_BILL_SUB_TYPE_MASTER C,M4_CC_MASTER D,PRODBI.FCHACC@AMS E,M4_BILL_PRODUCT_MASTER F"
                            +" WHERE B.DEPT_SL_NO='94' AND A.TYPE_SL_NO=B.SL_NO AND A.SUB_TYPE_SL_NO=C.SL_NO AND E.EACONO=111"
                            +" AND D.CC_CODE=E.EAAITM AND E.EAAITP=3 AND D.DEPT_SL_NO='94' AND A.CC_CODE=D.SL_NO AND A.PRODUCT_SL_NO=F.SL_NO AND"
                ...
                while(resultSet.next()){
                    ...
                    bean.setOPERATION_DESC(resultSet.getString("ORDER_NO"));
                    ...
                    if(CASH_TYPE!=null && CASH_TYPE.equalsIgnoreCase("DR")){
                        stat1 = conn.prepareStatement("SELECT A.SL_NO, A.COST_ELEMENT, A.AMT, A.FORM_TYPE, A.TDATE, A.USER_ID, A.BILL_SL_NO, A.FORM_WHLO, A.GST_PER, A.TAX_CODE, A.LINE_SL_NO,B.HSN_CODE"
                            +" FROM M4_INVOICE_BREAKUP_DETAIL A,M4_BILL_HSN_GL B WHERE A.COST_ELEMENT=B.DN_GL_CODE AND A.BILL_SL_NO=? AND A.LINE_SL_NO=? AND  B.flag='Y' and B.REV_GL_CODE is null");
                    } else{
                        stat1 = conn.prepareStatement("SELECT A.SL_NO, A.COST_ELEMENT, A.AMT, A.FORM_TYPE, A.TDATE, A.USER_ID, A.BILL_SL_NO, A.FORM_WHLO, A.GST_PER, A.TAX_CODE, A.LINE_SL_NO,B.HSN_CODE"
                            +" FROM M4_INVOICE_BREAKUP_DETAIL A,M4_BILL_HSN_GL B WHERE A.COST_ELEMENT=B.CN_GL_CODE AND A.BILL_SL_NO=? AND A.LINE_SL_NO=? AND  B.flag='Y' and B.REV_GL_CODE is null");
                    }
    ...
    public List<String> getOPERATION_DESC() {
        return OPERATION_DESC;
    }

    public void setOPERATION_DESC(List<String> OPERATION_DESC) {
    ...
    public List<M3BILLBean> getCURRENCY_LIST() {
        return CURRENCY_LIST;
    }

    public void setCURRENCY_LIST(List<M3BILLBean> CURRENCY_LIST) {
