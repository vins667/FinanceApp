...
* @author Shilpa
*/
public class GnrtrefnoSMAHdfcActionNM extends ActionSupport{
	
	private String LOC;
	private String LETTER_TYPE;
        private String COMPID="700";
	...
	private List<GnrtrefnoFbadiciciBeanNM> gnrtrefnoFbadiciciBeansList;
	  ...
	  public String mksess() throws SQLException, ParseException{
          ...
          if (usrid != null){
              ...
              try {
                  ...
                  System.out.println("GnrtrefnoSMAHdfcActionNM " + e);
              } finally {
                  if (conn != null) {
                      conn.close();
                  }
	  ...
	  public void refno() throws SQLException{
	   ...
	   try
	   {
		...
		System.out.println(COMPID+LOC)	;
			stat=con.prepareStatement("select max(ifnull(crefno,0))+1 crefno from mvxcdtshah.chqref where ccmpid=? and cloccd=?");
			stat.setString(1,COMPID);
                        stat.setString(2,unitcode);
			resultset=stat.executeQuery();
			
			if(REF_NO!=null)
			{
				stat1=con.prepareStatement("select cvser,cvono from mvxcdtshah.chqref where ccmpid=? and crefno=? and cloccd=?");
				stat1.setString(1,COMPID);
                                stat1.setString(2,REF_NO);
				stat1.setString(3,unitcode);
	  ...
	  public String search() throws Exception
	  {
		...
		{
			System.out.println("GnrtrefnoSMAHdfcNM-2" +e);
		}
		finally
		{
			if(con!=null)
	  ...
	  public String save() throws Exception
	  {
		...
		//	  }
				  ...
				  if(YEAR.get(i).toString()!=null && YEAR.get(i).toString().length()>0 && BANK.get(i).toString()!=null && BANK.get(i).toString().length()>0 && CHQ_NO.get(i).toString()!=null && CHQ_NO.get(i).toString().length()>0){
					 ...
					 /*if(attachfile.get(i)!=null){		            	
						 ...
						 FileUtils.copyFile(attachfile.get(i), fileToCreate);
                                      ...
                                      System.out.println(COMPID+XYEAR+XBANK+LOC+"-2");   
		  ...
		  catch(SQLException se)
		  {
			  ...
			  System.out.println("GnrtrefnoFbadHdfcException" +se);
		  }
		  catch(Exception e)
		  {
			  con.rollback();
