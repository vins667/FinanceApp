...
public class DataTexPurchaseGstWPDAction extends ActionSupport {
	/**
	 * 
	 */
	...
	public DataTexPurchaseGstWPDAction() {
	...
	public String execute() throws Exception {
		...
		try {
			...
			stat = conn.prepareStatement("select a.companycode,a.divisioncode,a.finyear,a.accountingdate,trim(a.charcol1) INVOICENO,a.invoicedate,trim(a.customersuppliercode) customersuppliercode,a.invamt,b.glamt,nvl"
					...
					+" from movexposting a where a.companycode=100 and a.divisioncode=? "+qry+" and (fromstatecode is not null or TOSTATECODE is not null) and a.eventcode=4 and flag=1 and a.charcol1 is not null  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0 and a.glcode is null"
					...
					+" from movexposting   where companycode=100 and divisioncode=? "+qry+" and (fromstatecode is not null or TOSTATECODE is not null) and eventcode=4 and flag=1 and charcol1 is not null  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0 and glcode is not null"
			...
			while(result.next()){
				...
				dataTexSflPurchaseBean.setRowId(i++);
		...
		if(chkdel!=null && chkdel.size()>0){
			...
			try{
				...
				if(DIVI!=null && DIVI.equals("101")){
					LOCT="800";
				}else if(DIVI!=null && DIVI.equals("105")){
					LOCT="500";
				}
				else{
					LOCT="900";
				...
				for(int i=0;i<chkdel.size();i++){		
					...
					if(chkdel.get(i).toString()!=null && chkdel.get(i).toString().length()>0){
						...
						if(arr.length==4){
							...
							SUPINV=arr[1];
							FINYEAR=arr[2];
							rowid=Integer.parseInt(arr[3]);
						}
						//System.out.println(SUPCODE+" "+SUPINV+" "+FINYEAR);
					}
					//System.out.println("Non Tds Amount"+getDetaillist().get(i+chkdel.size()).getNontdsAmount());
					if(SUPCODE!=null && SUPINV!=null && FINYEAR!=null){
						...
						stat8 = conn.prepareStatement("DELETE FROM SHAHIWEB.MOVEXPOSTGST where COMPANYCODE='100' AND DIVISIONCODE=? AND FINYEAR=? AND trim(CUSTOMERSUPPLIERCODE)=?  AND trim(CHARCOL1)=?  AND EVENTCODE=4 and Flag=2 ");
						stat8.setString(1, DIVI);
						stat8.setString(2, FINYEAR);
						stat8.setString(3, SUPCODE);
						stat8.setString(4, SUPINV);
						stat8.executeUpdate(); 
						...
						while(resultSet5.next()){
							...
							stat.setString(	9	,	resultSet5.getString("CHARCOL1").trim());
							...
							stat.setString(	41	,	resultSet5.getString("CHARCOL1").trim());
							...
							if(rowid!=-1 && getDetaillist().get(rowid).getNontdsAmount()!=null && getDetaillist().get(rowid).getNontdsAmount().length()>1){
								stat.setString(	44	,	getDetaillist().get(rowid).getNontdsAmount());
							}else{
								stat.setString(	44,"0");
							}
						...
						stat1=conn.prepareStatement("select a.companycode,a.divisioncode,a.finyear,TO_CHAR(a.accountingdate,'yyyyMMdd') accountingdate,trim(a.charcol1) INVOICENO,TO_CHAR(a.invoicedate,'yyyyMMdd') invoicedate,trim(a.customersuppliercode) customersuppliercode,a.invamt,b.glamt,nvl"
								...
								+" (select companycode,divisioncode,finyear,accountingdate,charcol1,invoicedate,customersuppliercode,currencycode,"
								+" currencyconversionrate,sum(QUANTITY) qty,decode(currencycode,'INR',sum(INVAMOUTINR),sum(CURRENCYAMOUNT)) invamt,max(duedate) duedate,max(taxform) taxform,max(narration) narration,max(numcol3) numcol3"
								+" from movexpostgst a where a.companycode=100 and a.divisioncode=? and a.finyear=? and trim(a.EXPENSECODE)<>'9995' and trim(a.customersuppliercode)=? and trim(a.charcol1)=?  and a.eventcode=4 and flag=1  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0 and a.glcode is null"
								+" group by companycode,divisioncode,finyear,accountingdate,charcol1,invoicedate,customersuppliercode,currencycode,"
								+" currencyconversionrate) a,"
								+" (select companycode,divisioncode,finyear,accountingdate,charcol1,invoicedate,customersuppliercode,sum(glamount) glamt"
								+" from movexpostgst   where companycode=100 and divisioncode=? and finyear=?  and trim(customersuppliercode)=? and trim(charcol1)=? and eventcode=4 and flag=1  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0 and glcode is not null"
								+" group by companycode,divisioncode,finyear,accountingdate,charcol1,invoicedate,customersuppliercode) b"
						...
						while(resultSet.next()){								
							...
							if(resultSet1.next() && (CHKTDS.get(i)!=null && CHKTDS.get(i).equals("Y"))){	                            	
								if (resultSet1.getString("iisers")!=null && !resultSet1.getString("iisers").equals("17"))
								{
									...
									filetext=filetext+ RightPad("I",20);
								}		                            
							}
							else
							{
							...
							if(amttds>0){
								...
								filetext=filetext+RightPad("T",1)+RightPad(VTDX,19);
								filetext=filetext+LeftPad(" ",32)+RightPad(resultSet.getString("narration"),40)+LeftPad("0.00",19)+LeftPad(" ",8)+LeftPad(" ",15)+RightPad("CHZ",3)+LeftPad(" ",8);
							...
							stat2=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'500'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(charcol1,24)||"
									...
									+" lpad(currencyconversionrate,13)||to_char(max(accountingdate),'YYYYMMDD')||rpad('AUDITSHMG',10) KK,rpad(EXPENSECODE,8)||rpad(decode(plantcode,'P21','SWD','P22','SPD','999'),8)||"  //rpad(PROFITCENTER,8)
									...
									+" from movexpostgst a where a.companycode=100 and a.divisioncode=? and a.finyear=? and trim(a.EXPENSECODE)<>'9995'and trim(a.customersuppliercode)=? and trim(a.charcol1)=? and a.eventcode=4 and flag=1  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0"
									+" and a.glcode is null"
									+" group by  accountingdate,customersuppliercode,charcol1,currencycode,currencyconversionrate,EXPENSECODE,decode(plantcode,'P21','SWD','P22','SPD','999'),COSTCENTERCODE,PRODUCTGROUPCODE,CONO"); //PROFITCENTER
							...
							while(resultSet2.next()){
								...
								String vatcnt ="";
								stat4=conn.prepareStatement("select count(*) kcnt from movexpostgst a where a.companycode=100 and a.divisioncode=? and a.finyear=? and trim(a.EXPENSECODE)<>'9995'and trim(a.customersuppliercode)=? and trim(a.charcol1)=? and a.eventcode=4 and flag=1  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0"
							...
							stat3=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'500'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(charcol1,24)||"
									...
									+" lpad(currencyconversionrate,13)||to_char(max(accountingdate),'YYYYMMDD')||rpad('AUDITSHMG',10)||rpad('S',1)||rpad('"+newtaxform+"',9)||lpad('"+taxtp+"',3)||rpad(' ',7)||rpad(glcode,8)||rpad(decode(plantcode,'P21','SWD','P22','SPD','999'),8)||"
									...
									+" from movexpostgst a where a.companycode=100 and a.divisioncode=? and a.finyear=? and trim(a.customersuppliercode)=? and trim(a.charcol1)=? and a.eventcode=4 and flag=1  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0"
									+" and a.glcode is not null"
									+" group by  accountingdate,customersuppliercode,charcol1,currencycode,currencyconversionrate,glcode,decode(plantcode,'P21','SWD','P22','SPD','999'),COSTCENTERCODE,PRODUCTGROUPCODE,CONO,HSNCODE,FROMSTATECODE,TAXTEMPLATE order by HSNCODE"); //PROFITCENTER
				...
				if (ftpout.equals("OK")) {
					...
					MI.connect();
					String status = MI.DataUpload("111", "500", "AP-BILL-101", "dinv"+DIVI+".txt", "GLS850MI", "0");
					MI.destroyMI();
					MI = null;
					...
					for(int i=0;i<chkdel.size();i++){  
						...
						if(SUPCODE!=null && SUPINV!=null && FINYEAR!=null){
							...
							statupd=conn.prepareStatement("UPDATE SHAHIWEB.MOVEXPOSTGST SET FLAG=2 WHERE COMPANYCODE='100' and instr(invoiceno,'-DN')=0 AND DIVISIONCODE=? and finyear=? and trim(customersuppliercode)=? and trim(charcol1)=? AND EVENTCODE=4 and flag=1");
							statupd.setString(1, DIVI);
							statupd.setString(2, FINYEAR);
							statupd.setString(3, SUPCODE);
							statupd.setString(4, SUPINV);
