 ...
 * @author Vivek
 */
public class CustomerOutwardReportAction {
        ...
        if (dateFrom != null && dateTo != null) {
            try {
                ...
                stmt = conn.prepareStatement("SELECT OKCUNM,OKCFC3,T3IVNO,T3IVDT,T3CUNO,T3CSNO,MMITDS,T3UNMS,T3ZQTY,OPVRNO,TXVAL,T3TAR3,T3TXA3,T3TAR1,T3TXA1,T3TAR2,T3TXA2,T3VTAM,GSTN,"
                        +" POS,CSTAT,XSSBNO,XSSBDT,XSEXIV,XSREM2,DOCTP,ESACDT,ESYEA4,ESVSER,ESVONO,XSCUCD,XSACAM,sum(TXVAL) over (partition by"
                        +" OKCUNM,OKCFC3,T3CUNO,T3CSNO,OPVRNO,GSTN) INVVAL,ROW_NUMBER () OVER (PARTITION BY XSEXIV ORDER BY XSEXIV) SEQ_NO"
                        +" from ("
                        ...
                        +" SUM(B.T3VTAM-(B.T3TXA1+B.T3TXA2+B.T3TXA3)) TXVAL, MAX(B.T3TAR3) T3TAR3, SUM(B.T3TXA3) T3TXA3, MAX(B.T3TAR1) T3TAR1,"
                        +" SUM(B.T3TXA1) T3TXA1, MAX(B.T3TAR2) T3TAR2, SUM(B.T3TXA2) T3TXA2,"
                        ...
                        +" B.T3CONO=F.M4CONO AND B.T3DIVI=F.M4DIVI AND B.T3GEOF=F.M4GEOC AND F.M4GEOC IS NOT NULL AND F.GSTN IS NOT NULL AND"
                        +" B.T3CONO=D.OPCONO(+) AND B.T3CUNO=D.OPCUNO(+) AND B.T3GEOT=D.OPGEOC(+) AND"
                        +" B.T3CONO=XSCONO(+) AND B.T3DIVI=XSDIVI(+) AND B.T3YEA4=XSYEA4(+) AND B.T3CUNO=XSCUNO(+) AND NVL(B.T3EXIN,B.T3IVNO)=XSIVNO(+) AND"
                        ...
                        +" B.T3CUNO,F.GSTN,SUBSTR(F.GSTN,0,2),SUBSTR(B.T3GEOT,6,2),SUBSTR(A.T3CSNO,0,INSTR(A.T3CSNO,'-')-1),D.OPVRNO,"
                ...
                while (resultSet.next()) {
                    ...
                    if(resultSet.getString("XSCUCD")==null || (resultSet.getString("XSCUCD")!=null && resultSet.getString("XSCUCD").trim().equalsIgnoreCase("INR"))){
                        bean.setINVOICEVALUE(resultSet.getDouble("T3VTAM"));//49 
                ...
                System.out.println("CustomerOutwardReportAction.class " + e.getMessage());
            } finally {
                if (conn != null) {
                    try {
                        conn.close();
                    } catch (SQLException ex) {
                        Logger.getLogger(CustomerOutwardReportAction.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        }
