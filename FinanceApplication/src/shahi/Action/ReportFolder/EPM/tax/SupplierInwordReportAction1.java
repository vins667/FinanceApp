 ...
 * @author Vivek
 */
public class SupplierInwordReportAction {
        ...
        if(dateFrom != null && dateTo !=null){
            try{
                ...
                stmt = conn.prepareStatement("SELECT T4CONO, T4DIVI, T4SUNO, T4SINO, T4INYR, T4IVDT, T4GEOT, T4GEOF, T4CSNO, NETVAL, T4TAR1, T4TXA1, T4TAR2, T4TXA2,"
                        ...
                        +" GSTN, POS, SSTAT, EGACDT, T4ZNO1,sum(NETVAL+T4TXA1+T4TXA2+T4TXA3) over (partition by"
                        ...
                        +" E.EGCONO=? AND E.EGDIVI=? AND E.EGACDT BETWEEN ? AND ?"+query+" AND B.T4STAT='01'"
                ...
                while(resultSet.next()){
                   ...
                   // bean.setORIGINAL_DOCUMENT_DATE(resultSet.getString("EPIVDT"));//16
                    ...
                    bean.setPORT_CODE(""); //PENDING KULDEEP SIR[Import Bill]//24
                    bean.setBILL_REPORT_NUMBER(""); //PENDING KULDEEP SIR[Import Bill]//25
                    bean.setBILL_REPORT_DATE(""); //PENDING KULDEEP SIR[Import Bill]//26
                    ...
                    if(resultSet.getString("MMUNMS")!=null && resultSet.getString("MMUNMS").length()>0){
                        bean.setUNIT_OF_MEASUREMENT(resultSet.getString("MMUNMS"));//33 
                    }else{
                ...
                if(conn!=null){
                    try {
                        ...
                        Logger.getLogger(SupplierInwordReportAction.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        }
