 ...
 * @author Vivek
 */
public class CustomerOutwardReportActionFC {
        ...
        if (dateFrom != null && dateTo != null) {
            try {
                ...
                stmt = conn.prepareStatement("SELECT OKCUNM,OKCFC3,T3IVNO,T3IVDT,T3CUNO,T3CSNO,MMITDS,T3UNMS,T3ZQTY,OPVRNO,TXVAL,T3TAR3,T3TXA3,T3TAR1,T3TXA1,T3TAR2,T3TXA2,T3VTAM,GSTN,"
                        +" POS,CSTAT,XSSBNO,XSSBDT,XSEXIV,case when xsrem2 is null then xsrem2 else 'IN'||trim(XSREM2) end XSREM2,DOCTP,ESACDT,ESYEA4,ESVSER,ESVONO,XSCUCD,XSACAM,sum(T3VTAM+(T3TXA1+T3TXA2+T3TXA3)) over (partition by"
                        +" T3CUNO,NVL(T3IVNO,XSEXIV)) INVVAL,ROW_NUMBER () OVER (PARTITION BY T3CUNO,NVL(T3IVNO,XSEXIV) ORDER BY T3CUNO,NVL(T3IVNO,XSEXIV)) SEQ_NO"
                        +" from ("
                ...
                while (resultSet.next()) {
                    ...
                    if(resultSet.getString("XSCUCD")==null || (resultSet.getString("XSCUCD")!=null && resultSet.getString("XSCUCD").trim().equalsIgnoreCase("INR"))){
                        ...
                        if(resultSet.getDouble("XSACAM")==0){
                            ...
                            bean.setTAXABLEVALUE(resultSet.getDouble("TXVAL"));//38 XSACAM removed on 11/12/17
                        }
                    }
                ...
                System.out.println("CustomerOutwardReportActionFC.class " + e.getMessage());
            } finally {
                if (conn != null) {
                    try {
                        conn.close();
                    } catch (SQLException ex) {
                        Logger.getLogger(CustomerOutwardReportActionFC.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        }
