...
public class DataTexSaleGstSflAction extends ActionSupport {
	...
	public DataTexSaleGstSflAction() {
	...
	public String execute() throws Exception {
		...
		try {
			...
			stat = conn.prepareStatement("select a.ABSUNIQUEID,a.companycode,a.divisioncode,a.finyear,VARCHAR_FORMAT(a.accountingdate,'DD-MM-YYYY') accountingdate,a.invoiceno,VARCHAR_FORMAT(a.invoicedate,'DD-MM-YYYY') invoicedate,a.customersuppliercode,a.taxform,a.invamt,b.glamt,nvl(a.invamt,0)+nvl(b.glamt,0) netamt,a.qty,VARCHAR_FORMAT(a.duedate,'DD-MM-YYYY') duedate,a.currencycode,a.currencyconversionrate,a.narration  from"
					...
					+" from movexposting a where  trim(a.companycode)='100' and trim(a.divisioncode)=? "+qry+" and (fromstatecode is not null or fromstatecode is not null) and a.eventcode=5 and a.glcode is null and flag=1"
					...
					+" from movexposting   where trim(companycode)='100' and trim(divisioncode)=? "+qry+" and (fromstatecode is not null or fromstatecode is not null) and eventcode=5 and glcode is not null and flag=1"
		...
		if(chkdel!=null && chkdel.size()>0){
			...
			try{
				...
				for(int i=0;i<chkdel.size();i++){
					 ...
					 */
					stat1=conn.prepareStatement("select 'I1'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'550'||rpad(a.customersuppliercode,10)||rpad(a.customersuppliercode,10)||rpad(substr(a.invoiceno,1,15),15)||"  //rpad(substr(a.invoiceno,4,10),10)
							...
							+" lpad(CURRENCYCONVERSIONRATE,13)||to_char(a.ACCOUNTINGDATE,'YYYYMMDD')||rpad(trim(a.CUSTOMERSUPPLIERCODE)||'  '||substr(a.invoiceno,1,15),40)||rpad('15102',8)||rpad('SPD',8)||rpad('UPK70000',8)||rpad(' ',8)||"
					...
					stat2=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'550'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr(INVOICENO,1,15),15)||to_char(INVOICEDATE,'YYYYMMDD')||"
					...
					//lpad('0',15)
					stat3=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'550'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr(INVOICENO,1,15),15)||to_char(INVOICEDATE,'YYYYMMDD')||"
				...
				if (ftpout.equals("OK")) {
					...
					String status = MI.DataUpload("111", "550", "AR-SALE-101", "sale"+DIVI+".txt", "GLS850MI", "1"); 
					MI.destroyMI();
					MI = null;
              ...
              //  tomail[1] = "rameshk.chauhan@shahi.co.in";
                ...
                subjecttitle = "Sale Voucher Posted From DataTex (SFL) on dated: " +format.format(new java.util.Date());
				...
				conn.rollback();
				System.out.println("1.DataTexSaleGstSflAction-save() " + e);
				addActionError(e.getMessage());
			} finally {
				if (conn != null) {
					conn.close();
