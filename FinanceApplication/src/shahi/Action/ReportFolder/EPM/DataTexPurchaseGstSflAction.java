...
public class DataTexPurchaseGstSflAction extends ActionSupport {
	/**
	 * 
	 */
	...
	public DataTexPurchaseGstSflAction() {
	...
	public String execute() throws Exception {
		...
		try {
			...
			stat = conn.prepareStatement("select a.companycode,a.divisioncode,a.finyear,a.accountingdate,trim(a.charcol1) INVOICENO,a.invoicedate,trim(a.customersuppliercode) customersuppliercode,a.invamt,b.glamt,nvl"
					...
					+" from movexposting a where a.companycode=100 and a.divisioncode=? "+qry+" and (fromstatecode is not null or fromstatecode is not null) and a.eventcode=4 and flag=1 and a.charcol1 is not null  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0 and a.glcode is null"
					...
					+" from movexposting   where companycode=100 and divisioncode=? "+qry+" and (fromstatecode is not null or fromstatecode is not null) and eventcode=4 and flag=1 and charcol1 is not null  and instr(invoiceno,'-DN')=0 and nvl(numcol2,0)=0 and glcode is not null"
		...
		if(chkdel!=null && chkdel.size()>0){
			...
			try{
				...
				//String path1 = getValue("reportPdfPath") + "PayChq"; 
				   String path1 = "D:/ReportFolder/EPM/PayChq";
				...
				for(int i=0;i<chkdel.size();i++){		
					...
					if(SUPCODE!=null && SUPINV!=null && FINYEAR!=null){
						...
						while(resultSet.next()){								
							...
							String newtaxform ="";
							String taxtp =" ";
							stat2=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'550'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(charcol1,24)||"
							...
							stat3=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'550'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(charcol1,24)||"
				...
				if (ftpout.equals("OK")) {
					...
					MI.connect();
					String status = MI.DataUpload("111", "550", "AP-BILL-101", "dinv"+DIVI+".txt", "GLS850MI", "0");
					MI.destroyMI();
					MI = null;
