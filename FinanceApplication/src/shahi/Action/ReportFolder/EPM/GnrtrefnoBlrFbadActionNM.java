...
import shahi.Action.ReportFolder.EPM.beans.GnrtrefnoBlrFbadBeanNM;
...
* @author Shilpa
*/
public class GnrtrefnoBlrFbadActionNM extends ActionSupport{
	...
	private List<GnrtrefnoBlrFbadBeanNM> gnrtrefnoBlrFbadBeansList;

    public GnrtrefnoBlrFbadActionNM() {
        super();
    }
	  ...
	  public String mksess() throws SQLException, ParseException{
          ...
          if (usrid != null){
              ...
              try {
                  ...
                  System.out.println("GnrtrefnoBlrFbadActionNM " + e);
              } finally {
                  if (conn != null) {
                      conn.close();
                  }
	  ...
	  public void refno() throws SQLException{
	   ...
	   try
	   {
		   con=new connectiondb2().getConnection();
			...
			stat=con.prepareStatement("select max(ifnull(crefno,0))+1 crefno from mvxcdtshah.chqref where cloccd=?");
			stat.setString(1,unitcode);
			resultset=stat.executeQuery();
			
			if(REF_NO!=null)
			{
				stat1=con.prepareStatement("select cvser,cvono from mvxcdtshah.chqref where crefno=? and cloccd=?");
	  ...
	  public String search() throws Exception
	  {
		...
		if((XYEAR !=null && XYEAR.length()>0)&&(XBANK !=null && XBANK.length()>0)&&(XCHQ_NO !=null && XCHQ_NO.length()>0))
		{
			...
			stat=con.prepareStatement("select ckdivi,ckyea4,ckbkid,ckchkn,ckvser,ckvono,ckstts,ckpycu,xpxtda,0-bankamt bankamt,bankcrg,ckspyn"
					+ " from MVXCDTSHAH.FAPMAST_NEW where ckcono=111 and ckdivi=? and ckyea4=? and ckbkid=? and ckchkn=?");
			...
			if(resultset.next())
			{
				...
				T_SUPPLIER=resultset.getString("ckspyn");
                                stat4=con.prepareStatement("select substr(ZICFR7,1,4) pymt from cinfdbprd.zidmas where zicono=111 and zisuno=?");
				stat4.setString(1,T_SUPPLIER.trim());
				...
				if(resultset.getString("ckstts")!=null && resultset.getString("ckstts").equals("5")){
					...
					stat1=con.prepareStatement("select cvser,cvono,crefno from mvxcdtshah.chqref where ccmpid=? and cyear=? and cbank=? and cchqno=?");
					stat1.setString(1,resultset.getString("ckdivi"));
                                        stat1.setString(2,resultset.getString("ckyea4"));
					stat1.setString(3,resultset.getString("ckbkid"));
					stat1.setString(4,resultset.getString("ckchkn"));
					resultset1=stat1.executeQuery();
					if(resultset.getString("ckstts")!=null && resultset.getString("ckstts").equals("1"))
					{
						...
						stat2=con.prepareStatement("SELECT ZICFR2 THTX40,ZICFR3 THKFLD  FROM cinfdbprd.zidmas WHERE  zicono=111 AND zisuno=?");
						...
						if(a==1 && LETTER_TYPE=="R")
						{
							...
							T_STAUS="1";
								...
								stat3=con.prepareStatement("SELECT ZICFR7 IDVRNO,ZICFR6 IDSUCM FROM CINFDBPRD.ZIDMAS WHERE ZICONO='111' AND "
				...
				ERRORMSG="Record Not Found!!!-1";
			}
		}
		}
		catch(Exception e)
		{
			System.out.println("GnrtrefnoBlrFbadNM" +e);
		}
		finally
		{
			if(con!=null)
	  ...
	  public String save() throws Exception
	  {
		...
		//	  }
			  ...
			  for(int i=0; i<YEAR.size(); i++)
			  {
				  if(YEAR.get(i).toString()!=null && YEAR.get(i).toString().length()>0 && BANK.get(i).toString()!=null && BANK.get(i).toString().length()>0 && CHQ_NO.get(i).toString()!=null && CHQ_NO.get(i).toString().length()>0){
					 ...
					 /*if(attachfile.get(i)!=null){		            	
				      ...
				      stat=con.prepareStatement("insert into mvxcdtshah.chqref(CYEAR,CBANK,CCHQNO,CVSER,CVONO,CSTTS,CREFNO,CCHID,CRGDT,CRGTM,CSUNO,CREFTP,CLOCCD,CFILE,CPYMT,CCMPID) "
		  ...
		  catch(SQLException se)
		  {
			  ...
			  System.out.println("GnrtrefnoException" +se);
		  }
		  catch(Exception e)
		  {
			  con.rollback();
			  addActionError("Record Not Saved!!!-3");
	...
	public List<GnrtrefnoBlrFbadBeanNM> getGnrtrefnoBlrFbadBeansList() {
		return gnrtrefnoBlrFbadBeansList;
	}


	public void setGnrtrefnoBlrFbadBeansList(
			List<GnrtrefnoBlrFbadBeanNM> gnrtrefnoBlrFbadBeansList) {
		this.gnrtrefnoBlrFbadBeansList = gnrtrefnoBlrFbadBeansList;
	}
