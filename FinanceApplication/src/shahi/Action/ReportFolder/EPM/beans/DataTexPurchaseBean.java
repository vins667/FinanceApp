package shahi.Action.ReportFolder.EPM.beans;

public class DataTexPurchaseBean {
	private String ABSUNIQUEID;
	private String COMPANYCODE;
	private String DIVISIONCODE;
	private String FINYEAR;
	private String ACCOUNTINGDATE;
	private String INVOICENO;
	private String INVOICEDATE;
	private String CUSTOMERSUPPLIERCODE;
	private String TAXFORM;
	private String INVAMT;
	private String GLAMT;
	private String NETAMT;
	private String QTY;
	private String DUEDATE;
	private String CURRENCYCODE;
	private String CURRENCYCONVERSIONRATE;
	private String NARRATION;
    private String TCHK;
    private String DCHK;
    private String GLCODE;
	public String getABSUNIQUEID() {
		return ABSUNIQUEID;
	}
	public void setABSUNIQUEID(String aBSUNIQUEID) {
		ABSUNIQUEID = aBSUNIQUEID;
	}
	public String getCOMPANYCODE() {
		return COMPANYCODE;
	}
	public void setCOMPANYCODE(String cOMPANYCODE) {
		COMPANYCODE = cOMPANYCODE;
	}
	public String getDIVISIONCODE() {
		return DIVISIONCODE;
	}
	public void setDIVISIONCODE(String dIVISIONCODE) {
		DIVISIONCODE = dIVISIONCODE;
	}
	public String getFINYEAR() {
		return FINYEAR;
	}
	public void setFINYEAR(String fINYEAR) {
		FINYEAR = fINYEAR;
	}
	public String getACCOUNTINGDATE() {
		return ACCOUNTINGDATE;
	}
	public void setACCOUNTINGDATE(String aCCOUNTINGDATE) {
		ACCOUNTINGDATE = aCCOUNTINGDATE;
	}
	public String getINVOICENO() {
		return INVOICENO;
	}
	public void setINVOICENO(String iNVOICENO) {
		INVOICENO = iNVOICENO;
	}
	public String getINVOICEDATE() {
		return INVOICEDATE;
	}
	public void setINVOICEDATE(String iNVOICEDATE) {
		INVOICEDATE = iNVOICEDATE;
	}
	public String getCUSTOMERSUPPLIERCODE() {
		return CUSTOMERSUPPLIERCODE;
	}
	public void setCUSTOMERSUPPLIERCODE(String cUSTOMERSUPPLIERCODE) {
		CUSTOMERSUPPLIERCODE = cUSTOMERSUPPLIERCODE;
	}
	public String getTAXFORM() {
		return TAXFORM;
	}
	public void setTAXFORM(String tAXFORM) {
		TAXFORM = tAXFORM;
	}
	public String getINVAMT() {
		return INVAMT;
	}
	public void setINVAMT(String iNVAMT) {
		INVAMT = iNVAMT;
	}
	public String getGLAMT() {
		return GLAMT;
	}
	public void setGLAMT(String gLAMT) {
		GLAMT = gLAMT;
	}
	public String getNETAMT() {
		return NETAMT;
	}
	public void setNETAMT(String nETAMT) {
		NETAMT = nETAMT;
	}
	public String getQTY() {
		return QTY;
	}
	public void setQTY(String qTY) {
		QTY = qTY;
	}
	public String getDUEDATE() {
		return DUEDATE;
	}
	public void setDUEDATE(String dUEDATE) {
		DUEDATE = dUEDATE;
	}
	public String getCURRENCYCODE() {
		return CURRENCYCODE;
	}
	public String getGLCODE() {
		return GLCODE;
	}
	public void setGLCODE(String gLCODE) {
		GLCODE = gLCODE;
	}
	public void setCURRENCYCODE(String cURRENCYCODE) {
		CURRENCYCODE = cURRENCYCODE;
	}
	public String getCURRENCYCONVERSIONRATE() {
		return CURRENCYCONVERSIONRATE;
	}
	public void setCURRENCYCONVERSIONRATE(String cURRENCYCONVERSIONRATE) {
		CURRENCYCONVERSIONRATE = cURRENCYCONVERSIONRATE;
	}
	public String getNARRATION() {
		return NARRATION;
	}
	public void setNARRATION(String nARRATION) {
		NARRATION = nARRATION;
	}

    public String getTCHK() {
        return TCHK;
    }

    public void setTCHK(String TCHK) {
        this.TCHK = TCHK;
    }

    public String getDCHK() {
        return DCHK;
    }

    public void setDCHK(String DCHK) {
        this.DCHK = DCHK;
    }
	@Override
	public String toString() {
		return "DataTexPurchaseBean [ABSUNIQUEID=" + ABSUNIQUEID + ", COMPANYCODE=" + COMPANYCODE + ", DIVISIONCODE="
				+ DIVISIONCODE + ", FINYEAR=" + FINYEAR + ", ACCOUNTINGDATE=" + ACCOUNTINGDATE + ", INVOICENO="
				+ INVOICENO + ", INVOICEDATE=" + INVOICEDATE + ", CUSTOMERSUPPLIERCODE=" + CUSTOMERSUPPLIERCODE
				+ ", TAXFORM=" + TAXFORM + ", INVAMT=" + INVAMT + ", GLAMT=" + GLAMT + ", NETAMT=" + NETAMT + ", QTY="
				+ QTY + ", DUEDATE=" + DUEDATE + ", CURRENCYCODE=" + CURRENCYCODE + ", CURRENCYCONVERSIONRATE="
				+ CURRENCYCONVERSIONRATE + ", NARRATION=" + NARRATION + ", TCHK=" + TCHK + ", DCHK=" + DCHK
				+ ", GLCODE=" + GLCODE + "]";
	}

    
	
	
}
