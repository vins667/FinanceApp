...
public class DataTexSaleGstWPDAction_1 extends ActionSupport {
	...
	public DataTexSaleGstWPDAction() {
	...
	public String execute() throws Exception {
		...
		try {
			...
			stat = conn.prepareStatement("select a.ABSUNIQUEID,a.companycode,a.divisioncode,a.finyear,VARCHAR_FORMAT(a.accountingdate,'DD-MM-YYYY') accountingdate,a.invoiceno,VARCHAR_FORMAT(a.invoicedate,'DD-MM-YYYY') invoicedate,a.customersuppliercode,a.taxform,a.invamt,b.glamt,nvl(a.invamt,0)+nvl(b.glamt,0) netamt,a.qty,VARCHAR_FORMAT(a.duedate,'DD-MM-YYYY') duedate,a.currencycode,a.currencyconversionrate,a.narration  from"
					...
					+" from movexposting a where  trim(a.companycode)='100' and trim(a.divisioncode)=? "+qry+" and (fromstatecode is not null or tostatecode is not null) and a.eventcode=5 and a.glcode is null and flag=1"
					...
					+" from movexposting   where trim(companycode)='100' and trim(divisioncode)=? "+qry+" and (fromstatecode is not null or tostatecode is not null) and eventcode=5 and glcode is not null and flag=1"
			...
			System.out.println("1.DataTexSaleGstSflAction-execute() " + e);
			addActionError(e.getMessage());
		} finally {
		...
		if(chkdel!=null && chkdel.size()>0){
			...
			try{
				...
				for(int i=0;i<chkdel.size();i++){
					...
					stat8 = conn.prepareStatement("DELETE FROM SHAHIWEB.MOVEXPOSTGST where COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=5  AND ABSUNIQUEID=?  and Flag=2 ");
					stat8.setString(1, DIVI);
					stat8.setString(2, chkdel.get(i).toString());
					stat8.executeUpdate(); 
					 ...
					 */
					stat1=conn.prepareStatement("select 'I1'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(a.customersuppliercode,10)||rpad(a.customersuppliercode,10)||rpad(substr(a.invoiceno,1,15),15)||"  //rpad(substr(a.invoiceno,4,10),10)
							...
							+" lpad(CURRENCYCONVERSIONRATE,13)||to_char(a.ACCOUNTINGDATE,'YYYYMMDD')||rpad(trim(a.CUSTOMERSUPPLIERCODE)||'  '||substr(a.invoiceno,1,15),40)||rpad('15102',8)||rpad('WPD',8)||rpad('KA000000',8)||rpad(' ',8)||"
							+" rpad(a.CUSTOMERSUPPLIERCODE,8)||rpad(' ',8)||lpad('0',15)||'TT'||'TTE'||'003'||rpad(' ',8) kk"
							+" from"
							+" (select ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,taxform,duedate,currencycode,"
							+" currencyconversionrate,narration ,sum(QUANTITY) qty,sum(INVAMOUTINR) invamt"
							+" from movexpostgst a where  a.companycode=100 and a.divisioncode=? and a.ABSUNIQUEID=? and a.eventcode=5  and flag=1 and a.glcode is null"
							+" group by ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,taxform,duedate,currencycode,"
							+" currencyconversionrate,narration ) a,"
							+" (select ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,sum(glamount) glamt"
							+" from movexpostgst   where companycode=100 and divisioncode=? and ABSUNIQUEID=? and eventcode=5 and flag=1 and glcode is not null"
							+" group by ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode) b"
					...
					stat2=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr(INVOICENO,1,15),15)||to_char(INVOICEDATE,'YYYYMMDD')||"
							...
							+" rpad(substr(trim(invoiceno),1,15)||' '||trim(NARRATION),40)||rpad(EXPENSECODE,8)||rpad(nvl(PROFITCENTER,' '),8)||rpad(nvl(COSTCENTERCODE,' '),8)||rpad(nvl(PRODUCTGROUPcode,'001'),8)||rpad(CUSTOMERSUPPLIERCODE,8)||rpad(nvl(substr(cono,2,8),' '),8)||"
							+" lpad(QUANTITY,15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8) kk FROM shahiweb.movexpostgst where companycode=100 and divisioncode=? and eventcode=5 and ABSUNIQUEID=?  and flag=1 and glcode is null");
					...
					//lpad('0',15)
					stat3=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr(INVOICENO,1,15),15)||to_char(INVOICEDATE,'YYYYMMDD')||"
							...
							+" rpad(substr(trim(invoiceno),1,15)||' '||trim(NARRATION),40)||rpad(GLCODE,8)||rpad(nvl(PROFITCENTER,' '),8)||rpad(nvl(COSTCENTERCODE,' '),8)||rpad(nvl(PRODUCTGROUPcode,'001'),8)||rpad(CUSTOMERSUPPLIERCODE,8)||rpad(nvl(substr(cono,2,8),' '),8)||"
							+" lpad(QUANTITY,15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8)||rpad(HSNCODE,16) kk,TOSTATECODE,FROMSTATECODE,TAXTEMPLATE FROM shahiweb.movexpostgst where companycode=100 and divisioncode=? and eventcode=5 and ABSUNIQUEID=? and flag=1 and glcode is not null order by hsncode");
				...
				if (ftpout.equals("OK")) {
					...
					MI.connect();
					//String status = MI.DataUpload("111", "550", "AR-SALE-"+DIVI, "sale"+DIVI+".txt", "GLS850MI", "0");
					String status = MI.DataUpload("111", "500", "AR-SALE-105", "sale"+DIVI+".txt", "GLS850MI", "1"); 
					MI.destroyMI();
					MI = null;
					...
					for(int i=0;i<chkdel.size();i++){
						...
						statupd=conn.prepareStatement("UPDATE SHAHIWEB.MOVEXPOSTGST SET FLAG=2 WHERE ABSUNIQUEID=? AND COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=5");
						statupd.setString(1, chkdel.get(i).toString());
						statupd.setString(2, DIVI);
						statupd.executeUpdate();
						if(statupd!=null){
              ...
              //  tomail[1] = "rameshk.chauhan@shahi.co.in";
                ...
                subjecttitle = "Sale Voucher Posted From DataTex (WPD) on dated: " +format.format(new java.util.Date());
				...
				conn.rollback();
				System.out.println("1.DataTexSaleGstWPDAction-save() " + e);
				addActionError(e.getMessage());
			} finally {
				if (stat != null) {
					stat.close();
