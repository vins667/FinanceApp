...
public class DataTexSaleGstWPDAction extends ActionSupport {
		...
		if(chkdel!=null && chkdel.size()>0){
			...
			try{
				...
				for(int i=0;i<chkdel.size();i++){
					 ...
					 */
					stat1=conn.prepareStatement("select 'I1'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(a.customersuppliercode,10)||rpad(a.customersuppliercode,10)||rpad(substr(a.invoiceno,4,15),15)||"  //rpad(substr(a.invoiceno,4,10),10)
							...
							+" lpad(CURRENCYCONVERSIONRATE,13)||to_char(a.ACCOUNTINGDATE,'YYYYMMDD')||rpad(trim(a.CUSTOMERSUPPLIERCODE)||'  '||substr(a.invoiceno,4,15),40)||rpad('15102',8)||rpad('WPD',8)||rpad('KA000000',8)||rpad(' ',8)||"
					...
					stat2=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr(INVOICENO,4,15),15)||to_char(INVOICEDATE,'YYYYMMDD')||"
							...
							+" rpad(substr(trim(invoiceno),4,15)||' '||trim(NARRATION),40)||rpad(EXPENSECODE,8)||rpad(nvl(PROFITCENTER,' '),8)||rpad(nvl(COSTCENTERCODE,' '),8)||rpad(nvl(PRODUCTGROUPcode,'001'),8)||rpad(CUSTOMERSUPPLIERCODE,8)||rpad(nvl(substr(cono,2,8),' '),8)||"
					...
					//lpad('0',15)
					stat3=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr(INVOICENO,4,15),15)||to_char(INVOICEDATE,'YYYYMMDD')||"
							...
							+" rpad(substr(trim(invoiceno),4,15)||' '||trim(NARRATION),40)||rpad(GLCODE,8)||rpad(nvl(PROFITCENTER,' '),8)||rpad(nvl(COSTCENTERCODE,' '),8)||rpad(nvl(PRODUCTGROUPcode,'001'),8)||rpad(CUSTOMERSUPPLIERCODE,8)||rpad(nvl(substr(cono,2,8),' '),8)||"
