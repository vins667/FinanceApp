...
import shahi.Action.ReportFolder.EPM.beans.GnrtrefnoBlrFbadBean;
...
* @author Shilpa
*/
public class GnrtrefnoBlrFbadAction extends ActionSupport{
	...
	private List<GnrtrefnoBlrFbadBean> gnrtrefnoBlrFbadBeansList;
	  ...
	  public String search() throws Exception
	  {
		...
		{
			stat=con.prepareStatement("select ckyea4,ckbkid,ckchkn,ckvser,ckvono,ckstts,ckpycu,xpxtda,0-bankamt bankamt,bankcrg,ckspyn"
					+ " from MVXCDTSHAH.FAPMAST_NEW where ckyea4=? and ckbkid=? and ckchkn=?");
			stat.setString(1,XYEAR);
			stat.setString(2,XBANK.toUpperCase());
			stat.setString(3,XCHQ_NO);
			resultset = stat.executeQuery();
			if(resultset.next())
			{
				...
				if(resultset.getString("ckstts")!=null && resultset.getString("ckstts").equals("5")){
					...
					stat1=con.prepareStatement("select cvser,cvono,crefno from mvxcdtshah.chqref where cyear=? and cbank=? and cchqno=?");
					...
					if(resultset.getString("ckstts")!=null && resultset.getString("ckstts").equals("1"))
					{
						...
						stat2=con.prepareStatement("SELECT ZICFR2 THTX40,ZICFR3 THKFLD  FROM shacdtprod.zidmas WHERE  zicono=111 AND zisuno=?");
		...
		{
			System.out.println("GnrtrefnoBlrFbad" +e);
		}
		finally
		{
			if(con!=null)
			  ...
			  for(int i=0; i<YEAR.size(); i++)
			  {
				  if(YEAR.get(i).toString()!=null && YEAR.get(i).toString().length()>0 && BANK.get(i).toString()!=null && BANK.get(i).toString().length()>0 && CHQ_NO.get(i).toString()!=null && CHQ_NO.get(i).toString().length()>0){
					 ...
					 if(attachfile.get(i)!=null){		            	
				      ...
				      stat=con.prepareStatement("insert into mvxcdtshah.chqref(CYEAR,CBANK,CCHQNO,CVSER,CVONO,CSTTS,CREFNO,CCHID,CRGDT,CRGTM,CSUNO,CREFTP,CLOCCD,CFILE,CPYMT) "
	...
	public List<GnrtrefnoBlrFbadBean> getGnrtrefnoBlrFbadBeansList() {
		return gnrtrefnoBlrFbadBeansList;
	}


	public void setGnrtrefnoBlrFbadBeansList(
			List<GnrtrefnoBlrFbadBean> gnrtrefnoBlrFbadBeansList) {
