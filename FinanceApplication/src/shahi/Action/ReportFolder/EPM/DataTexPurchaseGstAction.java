...
public class DataTexPurchaseGstAction extends ActionSupport {
	/**
	 * 
	 */
	...
	public DataTexPurchaseGstAction() {
	...
	public String execute() throws Exception {
        ...
        try {
            ...
            System.out.println("1.DataTexPurchaseGstAction-execute() " + e);
            addActionError(e.getMessage());
        } finally {
            if (conn != null) {
                conn.close();
...
stat.setString(	49	,	resultSet5.getString("TAXTEMPLATE"));
							...
							while(resultSet.next()){								
	                           ...
	                           /* stat1 = conn2.prepareStatement("select xasuno,t2xtdc,t1ait1,t1ait2,t2xtdr from mincdtprod.xidven,mincdtprod.xtdsrt,mincdtprod.xtdscd"
								...
								stat3=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'500'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(charcol1,24)||"
											 ...
											 +" lpad(currencyconversionrate,13)||to_char(max(accountingdate),'YYYYMMDD')||rpad('AUDITSHMG',10)||rpad('S',1)||rpad('"+newtaxform+"',9)||lpad('"+taxtp+"',3)||rpad(' ',7)||rpad(glcode,8)||rpad(PROFITCENTER,8)||"
											 +" rpad(COSTCENTERCODE,8)||rpad(customersuppliercode,8)||rpad(max(narration),40)||rpad(' ',19)||rpad(PRODUCTGROUPCODE,8)||lpad(sum(QUANTITY),15)||'CHK'||RPAD(CONO,8)||rpad(HSNCODE,16) KK,FROMSTATECODE,TAXTEMPLATE"
											 ...
											 +" group by  accountingdate,customersuppliercode,charcol1,currencycode,currencyconversionrate,glcode,PROFITCENTER,COSTCENTERCODE,PRODUCTGROUPCODE,CONO,HSNCODE,FROMSTATECODE,TAXTEMPLATE order by HSNCODE");
	        ...
	        Ftpfileupload fileobj = new Ftpfileupload();
                ...
                if (ftpout.equals("OK")) {
                    ...
                    MI.connect();
                    String status = MI.DataUpload("111", "500", "AP-BILL-"+DIVI, "dinv"+DIVI+".txt", "GLS850MI", "1");
                    MI.destroyMI();
                    MI = null;
		...
		MailProvider mailProvider = new MailProvider();
              ...
              //  tomail[1] = "rameshk.chauhan@shahi.co.in";
				...
				conn.rollback();
	            System.out.println("1.DataTexPurchaseGstAction-save() " + e);
	            addActionError(e.getMessage());
	        } finally {
	            if (conn != null) {
	                conn.close();
