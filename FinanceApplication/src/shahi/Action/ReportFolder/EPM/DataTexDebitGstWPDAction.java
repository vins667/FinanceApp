...
public class DataTexDebitGstWPDAction extends ActionSupport {
	/**
	 * 
	 */
	...
	public DataTexDebitGstWPDAction() {
	...
	public String execute() throws Exception {
		...
		try {
			...
			stat = conn.prepareStatement("select a.companycode,a.divisioncode,a.finyear,a.accountingdate,trim(a.INVOICENO) INVOICENO,a.invoicedate,trim(a.customersuppliercode) customersuppliercode,a.invamt,b.glamt,nvl"
					...
					+" from movexposting a where a.companycode=100 and a.divisioncode=? and (fromstatecode is not null or tostatecode is not null) and a.eventcode=18 and flag=1   and nvl(numcol2,0)=0 and a.glcode is not null"
					...
					+" from movexposting   where companycode=100 and divisioncode=? and (fromstatecode is not null or tostatecode is not null) and eventcode=18 and flag=1  and nvl(numcol2,0)=0 and glcode is not null"
			...
			try{
				...
				for(int i=0;i<chkdel.size();i++){					
					...
					if(SUPCODE!=null && SUPINV!=null && FINYEAR!=null){
						...
						stat8 = conn.prepareStatement("DELETE FROM SHAHIWEB.MOVEXPOSTGST where COMPANYCODE='100' AND DIVISIONCODE=? AND FINYEAR=? AND trim(CUSTOMERSUPPLIERCODE)=?  AND trim(CHARCOL1)=?  AND EVENTCODE=18 and Flag=2 ");
						...
						while(resultSet.next()){								
							...
							String newtaxform ="";
							String taxtp =" ";
							stat2=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'500'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(INVOICENO,24)||"
							...
							stat3=conn.prepareStatement("select 'I2'||lpad(max("+SQLUNIQUE+"),9)||rpad('"+ctr+"',8)||'500'||rpad(customersuppliercode,10)||rpad(customersuppliercode,10)||rpad(INVOICENO,24)||"
				...
				if (ftpout.equals("OK")) {
					...
					MI.connect();
					String status = MI.DataUpload("111", "500", "AP-BILL-101", "dninv"+DIVI+".txt", "GLS850MI", "0");
					MI.destroyMI();
					MI = null;
