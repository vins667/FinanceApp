...
import shahi.Action.ReportFolder.EPM.beans.CbnacBBean;
 ...
 * @author Vivek
 */
public class PayrollPaymentsExecuteBAction extends ActionSupport {
   ...
   // private String DIVI = "100";
    ...
    public String view() throws SQLException, Exception {
        ...
        try {
            ...
            System.out.println("1.PayrollPaymentsExecuteBAction " + e);
            addActionError(e.getMessage());
            conn.rollback();
        } finally {
            if (conn != null) {
    ...
    public void getAllBankCode() throws Exception {
        ...
        if (DIVI != null && DIVI.length() > 0) {
            ...
            try {
                stat = connbi.prepareStatement("select distinct BCAIT1 BCBKID,BCBANA,BCBKID BCBKID1 from m3fdbprd.CBANAC where BCCONO=111 and BCDIVI=? and BCBKTP=1 order by 1");
                stat.setString(1, DIVI);
                result = stat.executeQuery();
                while (result.next()) {
                    bankcodelist.add(new CbnacBBean(result.getString("BCBKID"), result.getString("BCBANA"), result.getString("BCBKID1")));
                }
                connbi.commit();
            } catch (Exception e) {
                System.out.println("2.PayrollPaymentsExecuteBAction " + e);
                addActionError(e.getMessage());
                connbi.rollback();
            } finally {
                if (connbi != null) {
    ...
    public String execute() throws Exception {
        ...
        try {
            stat1 = connora.prepareStatement("select chk_upd from AMSNOW.fa_payroll_chq where  CONO=111 AND divi=? and batch_no=? and PYMT_DATE is not null and chk_upd is not null");
            ...
            if(result1.next()){
                ...
                statpayCHK = connora.prepareStatement("select sum(chq_amount) chq_amount from AMSNOW.fa_payroll_chq a "
                ...
                statmvxCHK=conn.prepareStatement("select sum(egacam) egacam from m3fdbprd.fgledg,m3fdbprd.fchacc where egcono=111 and egdivi=? and egyea4=? and egait6=? and egait7<>'' and egcono=eacono and egait1=eaaitm  and eaat12='1' and egreco<>9");
               // and eaat12='1' removed on 20/12/16
                ...
                addActionError("Payroll and Movex Amount not matched for given batch no "+TMPRID+" "+mvxbttot+" "+paybttot);
                    getAllBankCode();
                    return SUCCESS;
                }
                
                stat = conn.prepareStatement("select egcono,egdivi,egyea4,egacdt,egvser,egvono,egait1,egvtxt,egait5,egait6,egait7,egacam from m3fdbprd.fgledg,m3fdbprd.fchacc "
            ...
            System.out.println("3.PayrollPaymentsExecuteBAction " + e);
            addActionError(e.getMessage());
            conn.rollback();
        } finally {
            if (conn != null) {
    ...
    public String save() throws SQLException, Exception {
        ...
        try {
            System.out.println(DIVI+CKBKID+BKID);
            String arr[] = CKBKID.split("\\$\\$\\$");
            ...
            List<String> tempEGAIT1=new ArrayList<String>();
            if(EGAIT1!=null && EGAIT1.size()>0 ){
                 System.out.println(DIVI+KBKID+BKID+YEAR+EGAIT1+EGAITP);
                                  
            //    EGAITP=EGAIT1.get(i).toString();
                for(int i=0;i<EGAIT1.size();i++){
                    if(tempEGAIT1.size()==0 || !(tempEGAIT1.contains(EGAIT1.get(i).toString()))){
                    tempEGAIT1.add(EGAIT1.get(i).toString());
                    stat = conn.prepareStatement("insert into m3fdbprd.fapchk select egcono ckcono,egdivi ckdivi,bcbkid,repeat( '0', 15-cast(length(trim(egait7)) as integer))||trim(egait7)  ckchkn,0 ckckno,'COMM BONUS' ckspyn,egvtxt cksunm,egait7 ckadr1,egait3 ckadr2,'' ckadr3,'' ckadr4, "
                        ...
                        + " egyea4 ckyea4,egvser ckvser,egvono ckvono,0 ckcada,'CHK' ckpyme,'AP20' ckfeid,'704'ckfncn,'' cktx01,'' cktx02,egrgdt ckrgdt,egrgtm ckrgtm,eglmdt cklmdt,0 ckchno,egchid ckchid,eglmts cklmts "
                        + " from m3fdbprd.fgledg,m3fdbprd.cbanac "
                        + " where egcono=111 and egdivi=? and egyea4=? and egait1=? and egait6=? and egait7<>''"
                        + " and egcono=bccono and egdivi=bcdivi and egait1=bcait1 and bcbkid=?"
                        + " and (bcbkid not like '%RT%' and bcbkid not like '%MN%' and bcbkid not like '%HR%' and bcbkid not like '%DD%')");
               
                    stat.setString(1, DIVI);
                    stat.setString(2, YEAR);
                    stat.setString(3, EGAIT1.get(i).toString());
                    stat.setString(4, TMPRID);                    
                    stat.setString(5, BKID.trim());
                    stat.executeUpdate();
                 ...
                 //   stat3.setString(5, CKBKID1);
                    stat3.setString(5, BKID.trim());
                    stat3.executeUpdate();

                    stat5 = conn.prepareStatement("update m3fdbprd.fgledg  set egtrcd='51' where egcono=111 and egdivi=? and egyea4=? and (egcono||egdivi||egyea4||egvono||egvser||egfeid||egfncn) in (select (egcono||egdivi||egyea4||egvono||egvser||egfeid||egfncn) from m3fdbprd.fgledg where egcono=111 and egdivi=? and egyea4=? and egait1=?  and egait6=? and egait7<>'')");
            ...
            stat4 = connora.prepareStatement("update AMSNOW.fa_payroll_chq set chk_upd=1 where  CONO=111 AND divi=? and batch_no=? and PYMT_DATE is not null ");
                ...
                tomail[0]="nagashayana.talkad@shahi.co.in";
               ...
               subjecttitle = "Cheques uploaded in movex M4 of batch No  :-" + TMPRID;
            ...
            System.out.println("4.PayrollPaymentsExecuteBAction " + e);
            addActionError(e.getMessage());
            conn.rollback();
        } finally {
            if (conn != null) {
    ...
    public String getKBKID() {
        return KBKID;
    }

    public void setKBKID(String KBKID) {
        this.KBKID = KBKID;
    }
