...
import shahi.Action.ReportFolder.EPM.beans.DataTexCreditBean;
import shahi.Action.database.ConnectionDataTex;
import shahi.Action.database.ConnectionDataTexWPD;
import shahi.Action.database.connection;
import shahi.Action.database.connectiondb2;

public class DataTexCreditGSTWPDAction extends ActionSupport {
	/**
	 * 
	 */
	...
	public DataTexCreditGSTWPDAction() {
		aResourcBundle = ResourceBundle.getBundle("shahi.Action.database.app");
	}

	private List<DataTexCreditBean> detaillist;
	...
	public String makesession() throws Exception {
		...
		try {
			stat = conn.prepareStatement(
					"select emp_code,EMP_NAME,LOCATION_CODE,UNIT_CODE,DEPT_CODE from EMPLOYEE_VIEW where emp_code=?");
			...
			System.out.println("1.DataTexCreditGSTAction " + e);
			addActionError(e.getMessage());
		} finally {
			if (conn != null) {
				conn.close();
	...
	public String execute() throws Exception {
		detaillist = new ArrayList<DataTexCreditBean>();
		Connection conn = null;
		PreparedStatement stat = null;
		ResultSet result = null;

		try {
			conn = new ConnectionDataTexWPD().getConnection();
			String qry = " ";
			...
			stat = conn.prepareStatement(
					"select a.ABSUNIQUEID,a.companycode,a.divisioncode,a.finyear,VARCHAR_FORMAT(a.accountingdate,'DD-MM-YYYY') accountingdate,a.invoiceno,VARCHAR_FORMAT(a.invoicedate,'DD-MM-YYYY') invoicedate,a.customersuppliercode,a.taxform,a.invamt,b.glamt,nvl(a.invamt,0)+nvl(b.glamt,0) netamt,a.qty,VARCHAR_FORMAT(a.duedate,'DD-MM-YYYY') duedate,a.currencycode,a.currencyconversionrate,a.narration  from"
							...
							+ " from movexposting a where  trim(a.companycode)='100' and trim(a.divisioncode)=? " + qry
							+ " and a.eventcode=19 and a.glcode is null and flag=1"
							...
							+ " from movexposting   where trim(companycode)='100' and trim(divisioncode)=? " + qry
							+ " and eventcode=19 and glcode is not null and flag=1"
							+ " group by ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode) b on a.ABSUNIQUEID=b.ABSUNIQUEID"
							+ " order by a.accountingdate,a.invoiceno");
			stat.setString(1, DIVI);
			stat.setString(2, DIVI);
			result = stat.executeQuery();
			while (result.next()) {
				DataTexCreditBean dataTexCreditBean = new DataTexCreditBean();
				dataTexCreditBean.setABSUNIQUEID(result.getString("ABSUNIQUEID"));
				dataTexCreditBean.setCOMPANYCODE(result.getString("COMPANYCODE"));
				dataTexCreditBean.setDIVISIONCODE(result.getString("DIVISIONCODE"));
				dataTexCreditBean.setFINYEAR(result.getString("FINYEAR"));
				dataTexCreditBean.setACCOUNTINGDATE(result.getString("ACCOUNTINGDATE"));
				dataTexCreditBean.setINVOICENO(result.getString("INVOICENO"));
				dataTexCreditBean.setINVOICEDATE(result.getString("INVOICEDATE"));
				dataTexCreditBean.setCUSTOMERSUPPLIERCODE(result.getString("CUSTOMERSUPPLIERCODE"));
				dataTexCreditBean.setTAXFORM(result.getString("TAXFORM"));
				dataTexCreditBean.setINVAMT(result.getString("INVAMT"));
				dataTexCreditBean.setGLAMT(result.getString("GLAMT"));
				dataTexCreditBean.setNETAMT(result.getString("NETAMT"));
				dataTexCreditBean.setQTY(result.getString("QTY"));
				dataTexCreditBean.setDUEDATE(result.getString("DUEDATE"));
				dataTexCreditBean.setCURRENCYCODE(result.getString("CURRENCYCODE"));
				dataTexCreditBean.setCURRENCYCONVERSIONRATE(result.getString("CURRENCYCONVERSIONRATE"));
				dataTexCreditBean.setNARRATION(result.getString("NARRATION"));
				detaillist.add(dataTexCreditBean);

			}
		} catch (Exception e) {
			System.out.println("1.DataTexCreditGSTAction-execute() " + e);
			addActionError(e.getMessage());
		} finally {
			if (stat != null) {
				stat.close();
	...
	public String save() throws Exception {
		...
		if (chkdel != null && chkdel.size() > 0) {
			...
			try {
				...
				conn1 = new ConnectionDataTexWPD().getConnection();
				conn2 = new connectiondb2().getConnection();
				String path1 = getValue("reportPdfPath") + "PayChq";
				// String path1 = "d:/epm/PayChq";
				String real_filename = path1 + "/" + "cninv" + DIVI + ".txt";
				File aa = new File(path1 + "/" + "cninv" + DIVI + ".txt");
				if (aa.exists()) {
					aa.delete();
				}
				String textfile = "";
				...
				int ctr = 1;
				statsq = conn.prepareStatement("SELECT datatexcredit_SQ.nextval from DUAL");
				...
				for (int i = 0; i < chkdel.size(); i++) {
					...
					stat = conn1.prepareStatement("SELECT * FROM MOVEXPOSTING WHERE COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19 AND FLAG=1 AND ABSUNIQUEID=? ");
					stat.setString(1, DIVI);
					stat.setString(2, chkdel.get(i).toString());

					stat4 = conn1.prepareStatement("delete FROM MOVEXPOSTING WHERE COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19 AND FLAG=2 AND ABSUNIQUEID=? ");
					stat4.setString(1, DIVI);
					stat4.setString(2, chkdel.get(i).toString());
					stat4.executeUpdate(); 
					...
					while (resultSet4.next()) {
						...
						stat = conn.prepareStatement(
								"INSERT INTO SHAHIWEB.MOVEXPOSTGST(COMPANYCODE,ABSUNIQUEID ,ROWINDEXNUM,ROWINDEXSUBNUM ,DIVISIONCODE,PLANTCODE,FINYEAR,ACCOUNTINGDATE,INVOICENO,INVOICEDATE,EXPENSECODE,COSTCENTERCODE, "
						...
						stat.setString(46, resultSet4.getString("FROMSTATECODE"));
						stat.setString(47, resultSet4.getString("TOSTATECODE"));
						stat.setString(48, resultSet4.getString("HSNCODE"));
						stat.setString(49, resultSet4.getString("TAXTEMPLATE"));
					...
					stat = conn1.prepareStatement(
							"SELECT distinct nvl(EXTERNALREFERENCE,INVOICENO) EXTERNALREFERENCE FROM MOVEXPOSTING,SALESDOCUMENT WHERE MOVEXPOSTING.COMPANYCODE='100' AND MOVEXPOSTING.DIVISIONCODE=? AND EVENTCODE=19 AND FLAG=1 AND MOVEXPOSTING.ABSUNIQUEID=? and trim(invoiceno)=nvl(trim(PROVISIONALCODE),trim(invoiceno))");

					String debitnote = null;
					stat.setString(1, DIVI);
					stat.setString(2, chkdel.get(i).toString());
					ResultSet resultSet5 = stat.executeQuery();
					if (resultSet5.next()) {
						debitnote = resultSet5.getString("EXTERNALREFERENCE");
					}
					if (stat != null) {
					...
					/*
					 * stat = conn.
					 * prepareStatement("INSERT INTO SHAHIWEB.MOVEXPOST SELECT * FROM MOVEXPOSTING@NOWPROD WHERE COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=5 AND FLAG=1 AND ABSUNIQUEID=? "
					 * ); stat.setString(1, DIVI); stat.setString(2,
					 * chkdel.get(i).toString()); stat.executeUpdate();
					 * if(stat!=null){ stat.close(); }
					 */
					stat1 = conn.prepareStatement("select 'I1'||rpad('" + SQLUNIQUE + "',9)||rpad('" + ctr
							+ "',8)||'500'||rpad(a.customersuppliercode,10)||rpad(a.customersuppliercode,10)||rpad(substr('"
							+ debitnote + "',1,10),10)||"
							+ " to_char(a.invoicedate,'YYYYMMDD')||to_char(a.duedate,'YYYYMMDD')||lpad(0-(nvl(a.invamt,0)+nvl(b.glamt,0)),17)||lpad(' ',19)||rpad(CURRENCYCODE,3)||rpad('1',2)||"
							+ " lpad(CURRENCYCONVERSIONRATE,13)||to_char(a.ACCOUNTINGDATE,'YYYYMMDD')||rpad(trim(a.CUSTOMERSUPPLIERCODE)||'  '||substr(a.plantinvoicecode,4,10)||' '||substr(a.invoiceno,1,15),40)||rpad('15102',8)||rpad('WPD',8)||rpad('800',8)||rpad(' ',8)||"
							+ " rpad(a.CUSTOMERSUPPLIERCODE,8)||rpad(' ',8)||lpad('0',15)||'TT'||'TTE'||'003'||rpad(' ',8) kk"
							+ " from"
							+ " (select ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,taxform,duedate,currencycode,plantinvoicecode,"
							+ " currencyconversionrate,narration ,sum(QUANTITY) qty,sum(INVAMOUTINR) invamt"
							+ " from movexpostgst a where  a.companycode=100 and a.divisioncode=? and a.ABSUNIQUEID=? and a.eventcode=19  and flag=1 and a.glcode is null"
							+ " group by ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,taxform,duedate,currencycode,plantinvoicecode,"
							+ " currencyconversionrate,narration ) a,"
							+ " (select ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,sum(glamount) glamt"
							+ " from movexpostgst   where companycode=100 and divisioncode=? and ABSUNIQUEID=? and eventcode=19 and flag=1 and glcode is not null"
					...
					stat2 = conn.prepareStatement("select 'I2'||rpad('" + SQLUNIQUE + "',9)||rpad('" + ctr
							+ "',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr('"
							+ debitnote + "',1,10),10)||to_char(INVOICEDATE,'YYYYMMDD')||"
							+ " to_char(DUEDATE,'YYYYMMDD')||lpad(INVAMOUTINR,17)||lpad(' ',19)||rpad(CURRENCYCODE,3)||rpad('1',2)||lpad(CURRENCYCONVERSIONRATE,13)||to_char(ACCOUNTINGDATE,'YYYYMMDD')||"
							+ " rpad(trim(CUSTOMERSUPPLIERCODE)||'  '||substr(plantinvoicecode,4,10)||' '||substr(invoiceno,1,15),40)||rpad(EXPENSECODE,8)||rpad(nvl(PROFITCENTER,' '),8)||rpad(nvl(COSTCENTERCODE,' '),8)||rpad(nvl(PRODUCTGROUPcode,'001'),8)||rpad(CUSTOMERSUPPLIERCODE,8)||rpad(nvl(cono,' '),8)||"
							+ " lpad(QUANTITY,15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8) kk FROM shahiweb.movexpostgst where companycode=100 and divisioncode=? and eventcode=19 and ABSUNIQUEID=?  and flag=1 and glcode is null");
					...
					stat3 = conn.prepareStatement("select 'I2'||rpad('" + SQLUNIQUE + "',9)||rpad('" + ctr
							+ "',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr('"
							+ debitnote + "',1,10),10)||to_char(INVOICEDATE,'YYYYMMDD')||"
							+ " to_char(DUEDATE,'YYYYMMDD')||lpad(GLAMOUNT,17)||lpad(' ',19)||rpad(CURRENCYCODE,3)||rpad('1',2)||lpad(CURRENCYCONVERSIONRATE,13)||to_char(ACCOUNTINGDATE,'YYYYMMDD')||"
							+ " rpad(trim(CUSTOMERSUPPLIERCODE)||'  '||substr(plantinvoicecode,4,10)||' '||substr(invoiceno,1,15),40)||rpad(GLCODE,8)||rpad(nvl(PROFITCENTER,' '),8)||rpad(nvl(COSTCENTERCODE,' '),8)||rpad(nvl(PRODUCTGROUPcode,'001'),8)||rpad(CUSTOMERSUPPLIERCODE,8)||rpad(nvl(cono,' '),8)||"
							+ " lpad('0',15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8) ||rpad(HSNCODE,16) kk,TOSTATECODE,FROMSTATECODE,TAXTEMPLATE FROM shahiweb.movexpostgst where companycode=100 and divisioncode=? and eventcode=19 and ABSUNIQUEID=? and flag=1 and glcode is not null");
					...
					while (resultSet3.next()) {
						...
						// System.out.println(textfile);
						// pw.println(textfile);
						statgeo = conn2.prepareStatement(
								"select distinct T0GEOC from m3fdbprd.CGEOJU where t0cono=111 and t0taj3<>'' and t0taj1=?");
						...
						statgeo = conn.prepareStatement("select distinct GSTCD from  shahiweb.GSTCDMAP where gstcds=?");
						statgeo.setString(1, resultSet3.getString("TAXTEMPLATE").trim());
						resultSetgeo = statgeo.executeQuery();
						if (resultSetgeo.next()) {
							taxc = resultSetgeo.getString("GSTCD");
						}
						textfile = textfile + StringUtils.rightPad(geoc, 10) + taxc + "               ";
						pw.println(textfile);
				...
				String ftpout = fileobj.FtpFileCopy(connst, cuser, cpass, real_filename,
						"/M3BE1512/env/M3BE_15.1_PRD/transfer/GLS850MI/" + "cninv" + DIVI + ".txt");
				if (ftpout.equals("OK")) {
					GLS850MI MI = new GLS850MI();
					MI.connect();
					String status = MI.DataUpload("111", "500", "AR-CN-" + DIVI, "cninv" + DIVI + ".txt", "GLS850MI",
							"0");
					MI.destroyMI();
					MI = null;

					for (int i = 0; i < chkdel.size(); i++) {
						statupd = conn1.prepareStatement(
								"UPDATE MOVEXPOSTING SET FLAG=2 WHERE ABSUNIQUEID=? AND COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19");
						...
						statupd = conn.prepareStatement(
								"UPDATE SHAHIWEB.MOVEXPOSTGST SET FLAG=2 WHERE ABSUNIQUEID=? AND COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19");
						statupd.setString(1, chkdel.get(i).toString());
						statupd.setString(2, DIVI);
						statupd.executeUpdate();
						if (statupd != null) {
				...
				filenames[0] = path1 + "/cninv" + DIVI + ".txt";
				// mail status
				...
				PreparedStatement mailtoStat = conn.prepareStatement(
						"select initcap(USER_NAME) USER_NAME,E_MAIL from seh_web_users where user_id=?");
				...
				subjecttitle = "Credit Note Voucher Posted From DataTex-GST on dated: "
				...
				messageBodyText += "This is to inform you that attached Credit Note Voucher file uploaded in Movex. ";
				...
				conn.rollback();
				System.out.println("1.DataTexCreditGSTAction-save() " + e);
				addActionError(e.getMessage());
			} finally {
				if (stat != null) {
					stat.close();
	...
	public List<DataTexCreditBean> getDetaillist() {
		return detaillist;
	}

	public void setDetaillist(List<DataTexCreditBean> detaillist) {
