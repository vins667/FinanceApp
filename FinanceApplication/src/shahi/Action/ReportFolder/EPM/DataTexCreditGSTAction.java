...
public class DataTexCreditGSTAction extends ActionSupport {
	/**
	 * 
	 */
	...
	Connection conn = null;
        ...
        try {
        	...
        	stat = conn.prepareStatement("select a.ABSUNIQUEID,a.companycode,a.divisioncode,a.finyear,VARCHAR_FORMAT(a.accountingdate,'DD-MM-YYYY') accountingdate,a.invoiceno,VARCHAR_FORMAT(a.invoicedate,'DD-MM-YYYY') invoicedate,a.customersuppliercode,a.taxform,a.invamt,b.glamt,nvl(a.invamt,0)+nvl(b.glamt,0) netamt,a.qty,VARCHAR_FORMAT(a.duedate,'DD-MM-YYYY') duedate,a.currencycode,a.currencyconversionrate,a.narration  from"
											...
											+" from movexposting a where  trim(a.companycode)='100' and trim(a.divisioncode)=? "+qry+" and a.eventcode=19 and a.glcode is null and flag=1"
											...
											+" from movexposting   where trim(companycode)='100' and trim(divisioncode)=? "+qry+" and eventcode=19 and glcode is not null and flag=1"
...
stat.setString(	49	,	resultSet4.getString("TAXTEMPLATE"));
					...
					if(stat!=null){
						...
						stat.close();
                                            ...
                                            stat = conn1.prepareStatement("SELECT distinct EXTERNALREFERENCE FROM MOVEXPOSTING,SALESDOCUMENT WHERE MOVEXPOSTING.COMPANYCODE='100' AND MOVEXPOSTING.DIVISIONCODE=? AND EVENTCODE=19 AND FLAG=1 AND MOVEXPOSTING.ABSUNIQUEID=? and trim(invoiceno)=trim(PROVISIONALCODE)");
				            ...
				            if(resultSet5.next()){
					       debitnote=resultSet5.getString("EXTERNALREFERENCE").trim();
                                            } 
                                            if(stat!=null){
					...
					*/
					stat1=conn.prepareStatement("select 'I1'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(a.customersuppliercode,10)||rpad(a.customersuppliercode,10)||rpad(substr('"+debitnote+"',1,10),10)||"
													...
													+" lpad(CURRENCYCONVERSIONRATE,13)||to_char(a.ACCOUNTINGDATE,'YYYYMMDD')||rpad(trim(a.CUSTOMERSUPPLIERCODE)||'  '||substr(a.plantinvoicecode,4,10)||' '||substr(a.invoiceno,1,15),40)||rpad('15102',8)||rpad('KPD',8)||rpad('800',8)||rpad(' ',8)||"
					...
					stat2=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr('"+debitnote+"',1,10),10)||to_char(INVOICEDATE,'YYYYMMDD')||"
					...
					stat3=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr('"+debitnote+"',4,10),10)||to_char(INVOICEDATE,'YYYYMMDD')||"
													...
													+" lpad('0',15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8) kk FROM shahiweb.movexpostgst where companycode=100 and divisioncode=? and eventcode=19 and ABSUNIQUEID=? and flag=1 and glcode is not null");
                ...
                String ftpout = fileobj.FtpFileCopy(connst, cuser, cpass, real_filename, "/M3BE1512/env/M3BE_15.1_PRD/transfer/GLS850MI/"+"cninv"+DIVI+".txt");
                if (ftpout.equals("OK")) {
                	GLS850MI MI = new GLS850MI();
                    MI.connect();
                    String status = MI.DataUpload("111", "500", "AR-CN-"+DIVI, "cninv"+DIVI+".txt", "GLS850MI", "0");
                    MI.destroyMI();
                    MI = null;
                    
                    for(int i=0;i<chkdel.size();i++){                    
	                    statupd=conn1.prepareStatement("UPDATE MOVEXPOSTING SET FLAG=2 WHERE ABSUNIQUEID=? AND COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19");
	                    ...
	                    statupd=conn.prepareStatement("UPDATE SHAHIWEB.MOVEXPOSTGST SET FLAG=2 WHERE ABSUNIQUEID=? AND COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19");
						statupd.setString(1, chkdel.get(i).toString());
						statupd.setString(2, DIVI);
						statupd.executeUpdate();	
						if(statupd!=null){
		...
		MailProvider mailProvider = new MailProvider();
              ...
              //  tomail[1] = "rameshk.chauhan@shahi.co.in";
                ...
                subjecttitle = "Credit Note Voucher Posted From DataTex-GST on dated: " +format.format(new java.util.Date());
