...
public class DataTexCreditAction extends ActionSupport {
	/**
	 * 
	 */
	...
	public String makesession() throws Exception {		
        ...
        try {
            ...
            System.out.println("1.DataTexCreditAction " + e);
            addActionError(e.getMessage());
        } finally {
            if (conn != null) {
                conn.close();
	...
	Connection conn = null;
        ...
        try {
            ...
            System.out.println("1.DataTexCreditAction-execute() " + e);
            addActionError(e.getMessage());
        } finally {
            if (conn != null) {
                conn.close();
...
stat.setString(	45	,	resultSet4.getString("PLANTINVOICECODE"));
					...
					*/
					stat1=conn.prepareStatement("select 'I1'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(a.customersuppliercode,10)||rpad(a.customersuppliercode,10)||rpad(substr('"+debitnote+"',1,10),10)||"
													...
													+" currencyconversionrate,narration ,sum(QUANTITY) qty,sum(INVAMOUTINR) invamt"
													+" from movexpost a where  a.companycode=100 and a.divisioncode=? and a.ABSUNIQUEID=? and a.eventcode=19  and flag=1 and a.glcode is null"
													...
													+" (select ABSUNIQUEID,companycode,divisioncode,finyear,accountingdate,invoiceno,invoicedate,customersuppliercode,sum(glamount) glamt"
													+" from movexpost   where companycode=100 and divisioncode=? and ABSUNIQUEID=? and eventcode=19 and flag=1 and glcode is not null"
					...
					stat2=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr('"+debitnote+"',1,10),10)||to_char(INVOICEDATE,'YYYYMMDD')||"
													...
													+" lpad(QUANTITY,15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8) kk FROM shahiweb.movexpost where companycode=100 and divisioncode=? and eventcode=19 and ABSUNIQUEID=?  and flag=1 and glcode is null");
					...
					stat3=conn.prepareStatement("select 'I2'||rpad('"+SQLUNIQUE+"',9)||rpad('"+ctr+"',8)||'500'||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(CUSTOMERSUPPLIERCODE,10)||rpad(substr('"+debitnote+"',4,10),10)||to_char(INVOICEDATE,'YYYYMMDD')||"
													...
													+" lpad('0',15)||'TT'||'TTE'||'003'||rpad(nvl(EVENT,' '),8) kk FROM shahiweb.movexpost where companycode=100 and divisioncode=? and eventcode=19 and ABSUNIQUEID=? and flag=1 and glcode is not null");
	        ...
	        Ftpfileupload fileobj = new Ftpfileupload();
                ...
                if (ftpout.equals("OK")) {
                    ...
                    for(int i=0;i<chkdel.size();i++){                    
	                    ...
	                    statupd=conn.prepareStatement("UPDATE SHAHIWEB.MOVEXPOST SET FLAG=2 WHERE ABSUNIQUEID=? AND COMPANYCODE='100' AND DIVISIONCODE=? AND EVENTCODE=19");
						statupd.setString(1, chkdel.get(i).toString());
						statupd.setString(2, DIVI);
						statupd.executeUpdate();	
						if(statupd!=null){
           ...
           /*    MailProvider mailProvider = new MailProvider();
              ...
              //  tomail[1] = "rameshk.chauhan@shahi.co.in";
                ...
                subjecttitle = "Credit Note Voucher Posted From DataTex on dated: " +format.format(new java.util.Date());
                ...
                */
				...
				conn.rollback();
	            System.out.println("1.DataTexCreditAction-save() " + e);
	            addActionError(e.getMessage());
	        } finally {
	            if (conn != null) {
	                conn.close();
