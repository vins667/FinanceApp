...
public class ComplianceCalenderDetailNewAction extends ActionSupport{
	...
	private List chkdel;
        ...
        private String DUE_DATE_ENT;
        private String E_CALENDER_TYPE;
        private String saveflg;
        private String SEARCH_LOCT;
        private String SEARCH_DEPT;
        private String SEARCH_SUB_DEPT;
	...
	public String newmksess() throws SQLException, ParseException{
	...
	public String execute() throws SQLException{
        ...
        try{
        	...
        	if(SEARCH_ACTIVITY!=null && SEARCH_ACTIVITY.length()>0){
        		query+=" AND CC_ACTIVITY_MASTER.ACTIVITY_NAME LIKE '%"+SEARCH_ACTIVITY.toUpperCase()+"%' ";
        	}
               if(SEARCH_LOCT!=null && SEARCH_LOCT.length()>0){
        		query+=" AND CC_ACTIVITY_MASTER.LOCATION_CODE = '"+SEARCH_LOCT+"' ";
        	}
               if(SEARCH_COMP!=null && SEARCH_COMP.length()>0){
        		query+=" AND CC_COMPANY_MASTER.NAME LIKE '%"+SEARCH_COMP.toUpperCase()+"%' ";
        	}
               if(SEARCH_DEPT!=null && SEARCH_DEPT.length()>0){
        		query+=" AND CC_DEPT_MASTER.NAME LIKE '%"+SEARCH_DEPT.toUpperCase()+"%' ";
        	}
               if(SEARCH_SUB_DEPT!=null && SEARCH_SUB_DEPT.length()>0){
        		query+=" AND CC_SUB_DEPT_MASTER.NAME LIKE '%"+SEARCH_SUB_DEPT.toUpperCase()+"%' ";
        	}
              
                
                if(SEARCH_STATUS!=null && (SEARCH_STATUS.equals("P") || SEARCH_STATUS.equals("C")))
                {
                 activitylist=pendingact(query);
        	...
        	stat=conn.prepareStatement("SELECT CC_COMPLIANCE_CALENDER_DETAIL.ID,CC_ACTIVITY_MASTER.ACTIVITY_NAME,CC_COMPLIANCE_CALENDER_DETAIL.COMPLIANCE_ID,to_char(CC_COMPLIANCE_CALENDER_DETAIL.DUE_DATE,'dd/MM/yyyy') DUE_DATE," 
					...
					+" ,CC_COMPANY_MASTER.NAME compname,CC_DEPT_MASTER.NAME deptuser, CC_SUB_DEPT_MASTER.NAME subdeptuser,CC_ACTIVITY_MASTER.ID actid,CC_COMPLIANCE_CALENDER.CALENDER_TYPE,CC_COMPLIANCE_CALENDER.CALENDER_TYPE_CODE,to_char(DUE_DATE,'yyyy') ACTIVITY_YEAR"
                                        +" FROM CC_COMPLIANCE_CALENDER_DETAIL,CC_COMPLIANCE_CALENDER,CC_ACTIVITY_MASTER,CC_COMPANY_MASTER,CC_DEPT_MASTER,CC_SUB_DEPT_MASTER"
					+" WHERE CC_COMPLIANCE_CALENDER_DETAIL.COMPLIANCE_ID=CC_COMPLIANCE_CALENDER.ID AND CC_COMPLIANCE_CALENDER.ACTIVITY_ID=CC_ACTIVITY_MASTER.ID "+query
                                       ...
                                       +" and CC_ACTIVITY_MASTER.ID in(select ACT_ID from CC_ACTIVITY_ACCESS_MASTER where EMP_CODE=? and FLAG='Y') order by ACTIVITY_NAME,LOCATION_CODE,COMPANY_ID,DEPT_ID,SUBDEPT_ID,DUE_DATE desc");
        	stat.setString(1, usrid);
                resultSet=stat.executeQuery();
                List cmptemlist=new ArrayList();
        	while(resultSet.next()){
                    
                   
                    
                    String accflg=null;
                    stat=conn.prepareStatement("select ACC_TYPE from CC_ACTIVITY_ACCESS_MASTER where ACT_ID=? and EMP_CODE=?");
                    stat.setString(1,resultSet.getString("actid"));
                    stat.setString(2,usrid);
                    resultSet1=stat.executeQuery();
                   
                    if(resultSet1.next())
                    {
                    accflg=resultSet1.getString(1);
                    }
                    String editflag="No";
                    if(!cmptemlist.contains(resultSet.getString("ACTIVITY_NAME")+
                            resultSet.getString("LOCATION_CODE")+resultSet.getString("deptuser")+
                            resultSet.getString("subdeptuser")+resultSet.getString("compname")))
                    {
                    editflag="Yes";
                    cmptemlist.add(resultSet.getString("ACTIVITY_NAME")+
                            resultSet.getString("LOCATION_CODE")+resultSet.getString("deptuser")+
                            resultSet.getString("subdeptuser")+resultSet.getString("compname"));
                    }
                    
                    
                    String monthact=null;
                       SimpleDateFormat formatter3 = new SimpleDateFormat("MMM");
                        if(resultSet.getString("CALENDER_TYPE")!=null && resultSet.getString("CALENDER_TYPE").equals("M"))
                        {
                             monthact=formatter3.format(duemonth(resultSet.getString("CALENDER_TYPE"),resultSet.getInt("CALENDER_TYPE_CODE"),resultSet.getInt("ACTIVITY_YEAR")));
                      
                      
                        }
                        if(resultSet.getString("CALENDER_TYPE")!=null && resultSet.getString("CALENDER_TYPE").equals("H"))
                        {
                         monthact="HY-"+resultSet.getString("CALENDER_TYPE_CODE");
                        }
                         if(resultSet.getString("CALENDER_TYPE")!=null && resultSet.getString("CALENDER_TYPE").equals("Q"))
                        {
                         monthact="Q-"+resultSet.getString("CALENDER_TYPE_CODE");
                        }
                         
                          if(resultSet.getString("CALENDER_TYPE")!=null && resultSet.getString("CALENDER_TYPE").equals("Y"))
                        {
                         monthact="Yearly";
                        } 
        		
        	detaillist.add(new ComplianceCalenderDetailBean(resultSet.getLong("ID"),resultSet.getString("ACTIVITY_NAME"),resultSet.getLong("COMPLIANCE_ID"),resultSet.getString("DUE_DATE"),resultSet.getString("SUBMISSION_DATE"),resultSet.getString("ACTUAL_DATE"),resultSet.getString("AMOUNT"),resultSet.getString("CHALLAN_NO"),resultSet.getString("BANK_CODE"),resultSet.getString("REMARKS"),resultSet.getString("USER_ID"),resultSet.getString("TDATE"),resultSet.getString("MUSER_ID"),resultSet.getString("MDATE"),resultSet.getString("LOCATION_CODE"),resultSet.getString("deptuser"),resultSet.getString("subdeptuser"),resultSet.getString("compname"),accflg,editflag,monthact));
        	}
                }
        }
        catch(Exception e){
        ...
        public String newconew() throws SQLException{
        Map session = ActionContext.getContext().getSession();
        String usrid = (String) session.get("sessUserId");
        if(usrid==null){
        	addActionError("Session not valid");
		...
		if(NE_ID!=null && NE_ID.length()>0){
		    ...
		    try{
		    	 stat=conn.prepareStatement("SELECT CC_COMPLIANCE_CALENDER.ID ID,CC_ACTIVITY_MASTER.ACTIVITY_NAME,CC_ACTIVITY_YEAR.ACTIVITY_YEAR,CALENDER_TYPE,"
					+" CALENDER_TYPE_CODE CALENDER_TYPE_CODE,DUE_DAYS,CC_ACTIVITY_MASTER.LOCATION_CODE, CC_ACTIVITY_MASTER.DEPT_ID,"
					...
					+" and CC_ACTIVITY_MASTER.ID in(select ACT_ID from CC_ACTIVITY_ACCESS_MASTER where EMP_CODE=? and FLAG='Y')"
                                        +" and CC_COMPLIANCE_CALENDER.ID=?");
        	        stat.setString(1, usrid);
                        stat.setString(2, NE_ID);
		    	resultSet=stat.executeQuery();
		    	if(resultSet.next()){
		    		E_COMP_CODE=resultSet.getString("id");
		    		E_COMP_NAME=resultSet.getString("ACTIVITY_NAME");
		    		E_DUE_DATE=DUE_DATE_ENT;
                                E_CALENDER_TYPE=resultSet.getString("CALENDER_TYPE");
		    		...
		    		E_COMP_YEAR = resultSet.getString("ACTIVITY_YEAR");
	...
	public String newedit() throws SQLException{
		Map session = ActionContext.getContext().getSession();
        String usrid = (String) session.get("sessUserId");
        if(usrid==null){
        	addActionError("Session not valid");
	...
	String usrid = (String) session.get("sessUserId");
        ...
        Connection conn=null;
		     ...
		     try {
				...
				System.out.println(e.toString());
                     ...
                     if(E_COMP_CODE!=null && E_COMP_CODE.length()>0  && saveflg==null){
			
		    try{
		    	SimpleDateFormat simpleDateFormat=new SimpleDateFormat("dd-MM-yyyy");
				SimpleDateFormat simpleDateFormat1=new SimpleDateFormat("dd-MMM-yyyy");
                                
                                if(E_ACTUAL_DATE!=null && E_ACTUAL_DATE.length()==0)
                                {
                                E_ACTUAL_DATE=null;
                                }
                                if(E_REMARKS!=null)
                                                {
                                                E_REMARKS=E_REMARKS.toUpperCase();
                                                }
		    	stat=conn.prepareStatement("UPDATE CC_COMPLIANCE_CALENDER_DETAIL SET SUBMISSION_DATE=to_date(?,'dd-mm-yyyy'), ACTUAL_DATE=to_date(?,'dd-mm-yyyy'), AMOUNT=?, CHALLAN_NO=?, BANK_CODE=?, REMARKS=?,MUSER_ID=?, MDATE=SYSDATE WHERE ID=?");
		    	stat.setString(1, E_SUBMISSION_DATE);
		    	stat.setString(2, E_ACTUAL_DATE);
        ...
        if(E_COMP_CODE!=null && E_COMP_CODE.length()>0 && saveflg!=null && saveflg.equals("Yes")){
         
        stat=conn.prepareStatement("SELECT * FROM CC_COMPLIANCE_CALENDER_DETAIL WHERE COMPLIANCE_ID=?  AND ACT_FIN_YEAR=?");
	        			stat.setString(1,E_COMP_CODE);
                                        stat.setString(2, E_COMP_YEAR);
	        			resultSet=stat.executeQuery();
	        			if(resultSet.next()){}
	        			else{
	        				long slno=0;
	        				...
	        				stat1=conn.prepareStatement("INSERT INTO CC_COMPLIANCE_CALENDER_DETAIL (ID,COMPLIANCE_ID,DUE_DATE,SUBMISSION_DATE,ACTUAL_DATE,AMOUNT,CHALLAN_NO,BANK_CODE,REMARKS,USER_ID,TDATE,MUSER_ID,MDATE,CALENDER_TYPE,ACT_FIN_YEAR)"
	        						+" VALUES(?,?,?,to_date(?,'dd-mm-yyyy'),to_date(?,'dd-mm-yyyy'),?,?,?,?,?,SYSDATE,?,SYSDATE,?,?)");
	        				stat1.setLong(1, slno);
	        				stat1.setString(2, E_COMP_CODE);
	        				stat1.setString(3, simpleDateFormat1.format(simpleDateFormat.parse(E_DUE_DATE)));
	        				stat1.setString(4, E_SUBMISSION_DATE);
	        				stat1.setString(5, E_ACTUAL_DATE);
	        				stat1.setString(6, E_AMOUNT);
	        				stat1.setString(7,E_CHALLAN_NO);
	        				stat1.setString(8, E_BANK_CODE);
	        				stat1.setString(9, E_REMARKS);
	        				stat1.setString(10, usrid);
	        				stat1.setString(11, usrid);
                                                stat1.setString(12, E_CALENDER_TYPE);
                                                stat1.setString(13, E_COMP_YEAR);
	        				...
	        				stat1=conn.prepareStatement("UPDATE CC_COMPLIANCE_CALENDER SET ACT_FLAG='Y' WHERE ID=?");
	        				stat1.setString(1,E_COMP_CODE);
	        				stat1.executeUpdate();
	        				if(stat1!=null){
	        					stat1.close();
	        				}
	...
	public String rem() throws SQLException, ParseException
	{
	Map session = ActionContext.getContext().getSession();
	String usrid = (String) session.get("sessUserId");
        if(usrid==null){
        ...
        Connection conn=null;
		    ...
		    try{
		    	...
		    	stat=conn.prepareStatement("select REMARKS from CC_COMPLIANCE_CALENDER_DETAIL WHERE ID=?");
                        stat.setString(1, E_ID);
		    	resultSet=stat.executeQuery();
                        while(resultSet.next())
                        {
                        E_REMARKS=resultSet.getString(1);
                        }
	...
	public String delete() throws SQLException{
     ...
     List pendingact(String qry) throws SQLException{
		Map session = ActionContext.getContext().getSession();
        String usrid = (String) session.get("sessUserId");
       List a=new ArrayList();
        ...
        try{
        	...
        	stat=conn.prepareStatement("SELECT min(CC_COMPLIANCE_CALENDER.ID) ID,CC_ACTIVITY_MASTER.ACTIVITY_NAME,CC_ACTIVITY_YEAR.ACTIVITY_YEAR,CALENDER_TYPE,"
					...
					+" WHERE CC_COMPLIANCE_CALENDER.ACTIVITY_ID=CC_ACTIVITY_MASTER.ID AND CC_ACTIVITY_MASTER.ID=CC_ACTIVITY_YEAR.ACTIVITY_ID  AND CC_ACTIVITY_YEAR.FLAG='Y' "+qry
                                        +" AND"
                                        +" (CC_COMPLIANCE_CALENDER.ID, CALENDER_TYPE, CALENDER_TYPE_CODE,ACTIVITY_YEAR) NOT IN ("
                                        +" SELECT CC_COMPLIANCE_CALENDER_DETAIL.COMPLIANCE_ID,CC_COMPLIANCE_CALENDER.CALENDER_TYPE,CC_COMPLIANCE_CALENDER.CALENDER_TYPE_CODE, ACTIVITY_YEAR"
                                        +" FROM CC_COMPLIANCE_CALENDER_DETAIL,CC_COMPLIANCE_CALENDER,CC_ACTIVITY_MASTER,CC_ACTIVITY_YEAR"
                                        +" WHERE CC_COMPLIANCE_CALENDER_DETAIL.COMPLIANCE_ID=CC_COMPLIANCE_CALENDER.ID AND CC_COMPLIANCE_CALENDER.ACTIVITY_ID=CC_ACTIVITY_MASTER.ID  AND "
                                        +"  CC_ACTIVITY_MASTER.ID=CC_ACTIVITY_YEAR.ACTIVITY_ID  AND CC_ACTIVITY_YEAR.ACTIVITY_YEAR=CC_COMPLIANCE_CALENDER_DETAIL.ACT_FIN_YEAR "+qry
                                        +" )"
					+" and CC_ACTIVITY_MASTER.ID in(select ACT_ID from CC_ACTIVITY_ACCESS_MASTER where EMP_CODE=? and FLAG='Y')"
                                        +" and CC_COMPLIANCE_CALENDER.COMPANY_ID=CC_COMPANY_MASTER.ID and CC_ACTIVITY_MASTER.DEPT_ID=CC_DEPT_MASTER.ID and CC_ACTIVITY_MASTER.SUBDEPT_ID=CC_SUB_DEPT_MASTER.ID "
                                        +" GROUP BY CC_ACTIVITY_MASTER.ACTIVITY_NAME,CC_ACTIVITY_YEAR.ACTIVITY_YEAR,CALENDER_TYPE,DUE_DAYS,"
					+" CC_ACTIVITY_MASTER.LOCATION_CODE, CC_ACTIVITY_MASTER.DEPT_ID, CC_ACTIVITY_MASTER.SUBDEPT_ID,CC_COMPLIANCE_CALENDER.COMPANY_ID,CC_COMPANY_MASTER.NAME,CC_DEPT_MASTER.NAME,CC_SUB_DEPT_MASTER.NAME,CC_ACTIVITY_MASTER.ID  order by ACTIVITY_NAME,LOCATION_CODE,COMPANY_ID,DEPT_ID,SUBDEPT_ID");
        	stat.setString(1, usrid);
                resultSet=stat.executeQuery();
        	while(resultSet.next()){
        		...
        		SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
                       ...
                       // cal.add(Calendar.DATE, - resultSet.getInt("DUE_DAYS"));
                    ...
                    if(duedate!=null && Integer.parseInt(formatter1.format(duedate))<=Integer.parseInt(formatter1.format(new Date())))
                    {
                     editflag="Yes";
                    ...
                    if(SEARCH_STATUS!=null && SEARCH_STATUS.equals("C") ){
                        if(editflag.equals("Yes")){
                              a.add(new ComplianceCalenderDtBean(resultSet.getLong("ID"),resultSet.getString("ACTIVITY_NAME"),formatter.format(duedate),resultSet.getString("compname"),resultSet.getString("LOCATION_CODE"),resultSet.getString("deptuser"),resultSet.getString("subdeptuser"),resultSet.getString("CALENDER_TYPE"),accflg,editflag,monthact));
                        }
                    }else{
	...
	public Date duedate(String type,int year,int code,int duedate){
		...
		cal.add(Calendar.DATE, duedate);
                /*if(type!=null && type.equals("M") && duedate>0 && (bb==12 || bb<3)){
	...
	public static Date getLastDateOfMonth(int year, int month) {
        ...
        public Date duemonth(String type,int code,int year) throws ParseException{
		int bb=0;
		if(type!=null && type.equals("M")){
			switch(code){
			case 1 : bb=3;
    ...
    public String getDUE_DATE_ENT() {
        return DUE_DATE_ENT;
    }

    public void setDUE_DATE_ENT(String DUE_DATE_ENT) {
        this.DUE_DATE_ENT = DUE_DATE_ENT;
    }

    public String getE_CALENDER_TYPE() {
        return E_CALENDER_TYPE;
    }

    public void setE_CALENDER_TYPE(String E_CALENDER_TYPE) {
        this.E_CALENDER_TYPE = E_CALENDER_TYPE;
    }
    ...
    public String getSEARCH_SUB_DEPT() {
        return SEARCH_SUB_DEPT;
    }

    public void setSEARCH_SUB_DEPT(String SEARCH_SUB_DEPT) {
        this.SEARCH_SUB_DEPT = SEARCH_SUB_DEPT;
    }

    public String getE_COMP_YEAR() {
        return E_COMP_YEAR;
    }

    public void setE_COMP_YEAR(String E_COMP_YEAR) {
        this.E_COMP_YEAR = E_COMP_YEAR;
    }
