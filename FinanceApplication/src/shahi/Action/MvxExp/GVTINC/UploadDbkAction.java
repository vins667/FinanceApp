...
import shahi.Action.MvxExp.GVTINC.Beans.DBKSCROLLBEAN;


public class UploadDbkAction extends ActionSupport {
...
int flag = 0;
                   ...
                   Cell cell0 = row.getCell(0);
                     ...
                     if(cell0.getCellType()==0){
                        if (DateUtil.isCellDateFormatted(cell0)){
                            ...
                            DATA1=format.format(cell0.getDateCellValue());
                          }
                        }  
        ...
        file.close();                 }
             ...
             {   for(int i=0; i<SB_NO.size(); i++)
                  {   
                   ...
                   if (vsbno.length()>0 && vsbno!=null)
                      {    //Removed on 07-dec-2017 due to multiple sb allow requested by vinod  
                         //    stat1=conn.prepareStatement("select * from eis_dbkchq_upload where sb_no=? and sb_date=?");
                         ...
                         //    {       
                                 if(SB_DATE.get(i).toString()!=null && SB_DATE.get(i).toString().length()>2){
                                    stat = conn.prepareStatement("INSERT INTO eis_dbkchq_upload (TR_DATE,SB_NO,SB_DATE,AMT_CR,TDATE,USER_ID,BANK_CODE,SCROLL_NO,SCROLL_DATE) VALUES (?,?,?,?,sysdate,?,?,?,?)  "  );
                                    stat.setString(1,TR_DATE.get(i).toString());
                                    stat.setString(2,vsbno);
                                    stat.setString(3,vsbdt);
                                    stat.setString(4,AMOUNT.get(i).toString());
                                    stat.setString(5,usrid); 
                                    stat.setString(6,dbkbank);
                                    stat.setString(7,SCROLL_NO.get(i).toString());  
                                    stat.setString(8,SCROLL_DATE.get(i).toString());
                                    result = stat.executeQuery();
                                     if (stat != null) { stat.close(); result.close(); }
                                    }
                                    flag=1;
                                    result1.close(); 
                                    stat1.close();
            ...
            try{   conn.rollback();
                ...
                try
                {
                    ...
                    System.out.print("File Name : UploaddbkAction.java Exception in finally block");
                    addActionError(e.getMessage()+"  UploadDbkAction.java ");
                }
            } /// end Finally Block
    ...
    public List getTR_DATE() {
        return TR_DATE;
    }

    public void setTR_DATE(List TR_DATE) {
