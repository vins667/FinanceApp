...
import shahi.Action.MvxExp.GVTINC.Beans.IOBdbkBean; 
import shahi.Action.database.connection;
import shahi.Action.MvxExp.Admin.EisUtil;
import shahi.Action.MvxExp.GVTINC.Beans.DbkChqUpdBean;
         ...
         try
           { 
             ...
             {  
              ...
              {
                stat1 = conn.prepareStatement(" select to_char(tr_date,'dd-Mon-yyyy') tr_date,a.shp_bill_no,to_char(a.shp_bill_date,'yyyymmdd') sbdt,to_char(a.shp_bill_date,'dd-Mon-yyyy') sbdate,claim_port,a.dbk_admisable,a.str_due,a.str_woff,b.amt_cr,str_not_claim from ei_dbk_mast a,eis_dbkchq_upload b where a.shp_bill_date>='01-jan-2012' and a.shp_bill_no=TRIM(b.sb_no) and upd_date is null and b.bank_code=? and tr_date=? order by 1,2,3 " );
              }else
              { 
                stat1 = conn.prepareStatement(" select to_char(tr_date,'dd-Mon-yyyy') tr_date,a.shp_bill_no,to_char(a.shp_bill_date,'yyyymmdd') sbdt,to_char(a.shp_bill_date,'dd-Mon-yyyy') sbdate,claim_port,a.dbk_admisable,a.str_due,a.str_woff,b.amt_cr,str_not_claim from ei_dbk_mast a,eis_dbkchq_upload b where a.shp_bill_no=TRIM(b.sb_no) and (to_char(a.shp_bill_date,'dd/mm/yyyy')=to_char(b.sb_date,'dd/mm/yyyy') or to_char(a.shp_bill_date,'dd/mm/yyyy')=to_char(b.sb_date,'mm/dd/yyyy')) and upd_date is null and b.bank_code=? and tr_date=? order by 1,2,3 " );
              }
                stat1.setString(1,sbank);
               ...
               {    
                  ...
                  double strdue=result1.getDouble("str_due")-result1.getDouble("str_woff");;
                     if (result1.getString("str_not_claim")==null && strdue>0 && result1.getInt("sbdt")<20170701)
                     {
                       strrecv=strdue;
                     }
                               ...
                               }else
                             {
                                stat=conn.prepareStatement("select count(*) v_cnt from ei_dbk_mast where shp_bill_no=?  and shp_bill_date=?");
                                  ...
                                  result=stat.executeQuery();
                                   ...
                                   stat.close();
                                   result.close(); 
                                            stat1 = conn.prepareStatement(" select a.dbk_admisable,a.str_due,a.str_woff,b.amt_cr,str_not_claim,to_char(b.tr_date,'ddmmyy') chdt,to_char(a.shp_bill_date,'yyyymmdd') sbdt from ei_dbk_mast a,eis_dbkchq_upload b where  a.shp_bill_no=trim(b.sb_no) and upd_date is null and a.shp_bill_no=? and a.shp_bill_date=?  " );
                                   ...
                                   result1.close(); 
                                         ...
                                         stat = conn.prepareStatement("INSERT INTO exports.ei_dbk_chq_dtls (year,chq_no,shp_bill_no,shp_bill_date,amount,bank_cr,str_amt,w_off,seh_user,pay_type,chqdate,chqbank,location,tdate) VALUES (pay_fin_year(?),?,?,?,?,?,?,?,?,'D',?,?,?,sysdate)  "  );
                                        stat.setString(1,va3); 
                                   ...
                                   stat.close();
                                         ...
                                         stat1 = conn.prepareStatement("update eis_dbkchq_upload set upd_date=sysdate where trim(sb_no)=? and sb_date=? and trim(bank_code)=? and upd_date is null   " );
                                         stat1.setString(1,va1.trim());
                                         stat1.setString(2,va2); 
                                         stat1.setString(3,dbkbank.trim());
                                         stat1.executeUpdate();
                                   stat1.close();
