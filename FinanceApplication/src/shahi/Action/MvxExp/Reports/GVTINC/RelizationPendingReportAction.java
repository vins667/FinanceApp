...
public final class RelizationPendingReportAction extends ActionSupport{
		...
		{  
                ...
                if(date1!=null && date1.length()>0 && date2!=null && date2.length()>0)
                 {  
                      ...
                      stat=con.prepareStatement("select distinct COMPANY from ei_endors_mast where company='111'");
                      resultset=stat.executeQuery();
                                while(resultset.next())
                                {
                      ...
                      PreparedStatement stat1=con.prepareStatement("select distinct nvl(F_BANK,0) F_BANK from ei_shipment_mast where g_comp='111' ");
