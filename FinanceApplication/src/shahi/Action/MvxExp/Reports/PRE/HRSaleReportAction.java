...
public final class HRSaleReportAction extends ActionSupport{
	...
	public String execute() throws SQLException
	{
		...
		{  
			...
			if(date1!=null && date1.length()>0 && date2!=null && date2.length()>0)
			{
                                ...
                                stat=con.prepareStatement("select distinct exp_type from ei_endors_mast where t_o_date between to_date(?,'dd/MM/yyyy') and to_date(?,'dd/MM/yyyy') AND LOCATION LIKE ? order by 1");
                                ...
                                stat=con.prepareStatement("select distinct CNTRY_ORIGIN from ei_endors_mast where cntry_origin is not null and  t_o_date between  to_date(?,'dd/MM/yyyy') and to_date(?,'dd/MM/yyyy') AND LOCATION LIKE ? order by 1");
                                stat.setString(1, date1);
                                stat.setString(2, date2);
                                stat.setString(3,LOCT_CODE);
                                resultset=stat.executeQuery();
                                ...
                                stat=con.prepareStatement("select distinct crncy_code from ei_endors_mast where t_o_date between to_date(?,'dd/MM/yyyy') and to_date(?,'dd/MM/yyyy') AND LOCATION LIKE ? order by 1");
                                stat.setString(1, date1);
                                stat.setString(2, date2);
                                stat.setString(3,LOCT_CODE);
                                resultset=stat.executeQuery();
                                ...
                                stat=con.prepareStatement("select distinct mode_of_ship from ei_endors_mast where mode_of_ship is not null and t_o_date between to_date(?,'dd/MM/yyyy') and to_date(?,'dd/MM/yyyy') AND LOCATION LIKE ? order by 1");
                                stat.setString(1, date1);
                                stat.setString(2, date2);
                                stat.setString(3,LOCT_CODE);
