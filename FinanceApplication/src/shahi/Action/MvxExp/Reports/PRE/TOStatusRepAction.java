...
public class TOStatusRepAction extends ActionSupport implements ServletRequestAware,ServletResponseAware {
        ...
        try {
            ...
            try
            {
              ...
              if (printFlag!= null)
              {
                ...
                /// Print Start  
                   ...
                   String v_cond=" and 1=1 ";
                    ...
                    {    SimpleDateFormat formatter = new SimpleDateFormat("yyyy-mm-dd");
                       ...
                       {
                           v_cond=" and t_o_date between '"+formatter1.format(date_from)+"' and '"+formatter1.format(date_to)+"'";
                       }else{
                                v_cond=" and "+basedon+"_date between '"+formatter1.format(date_from)+"' and '"+formatter1.format(date_to)+"'";
                       }
                     }
                    else{ v_cond=" and 1=1 ";} 
                    ...
                    String condsql = null;  
                        condsql = "SELECT decode(SELECT decode("+"'"+ordby+"','M',mode_of_ship,'P',cost_centre,'B',buyer,'A',ac_holder,'L',loading,'C',crncy_code) p_ord,to_char(nvl(a.etd_date,a.t_o_date),'YYYYMM') mnth,to_char(nvl(a.etd_date,a.t_o_date),'MON-YY') mawb,crncy_code,sum(QTY_ENDORS) invqty,sum(QTY_ENDORS * (PRICE_FC+NVL(PRICE_MISC,0)))  FOB FROM EI_ENDORS_MAST A,EI_ENDORS_DTLS B WHERE A.YEAR=B.YEAR AND A.COMPANY=B.COMPANY AND A.INV_NO=B.INV_NO AND "+
                                                " NVL(B.QTY_ENDORS,0)<>0 AND A.T_O_date IS NOT NULL AND NVL(A.SHIP_QTY,0)=0 and doc_send is null and nvl(surrender_yn,'X')<>'Y' and a.location like '"+LOCT_CODE+"' and a.buyer like '"+BUYER+"' and a.ac_holder like '"+ac_holder+"' and a.self_tp like '"+self_tp+"'"+v_cond+
                                                " group by decode("+"'"+ordby+"','M',mode_of_ship,'P',cost_centre,'B',buyer,'A',ac_holder,'L',loading,'C',crncy_code),to_char(nvl(a.etd_date,a.t_o_date),'YYYYMM'),to_char(nvl(a.etd_date,a.t_o_date),'MON-YY'),crncy_code order by 1,2,4";
                      
                 
                        stat=conn.prepareStatement("SELECT decode("+"'"+ordby+"','M',mode_of_ship,'P',cost_centre,'B',buyer,'A',ac_holder,'L',loading,'C',crncy_code) p_ord,to_char(nvl(a.etd_date,a.t_o_date),'YYYYMM') mnth,to_char(nvl(a.etd_date,a.t_o_date),'MON-YY') mawb,crncy_code,sum(QTY_ENDORS) invqty,sum(QTY_ENDORS * (PRICE_FC+NVL(PRICE_MISC,0)))  FOB FROM EI_ENDORS_MAST A,EI_ENDORS_DTLS B WHERE A.YEAR=B.YEAR AND A.COMPANY=B.COMPANY AND A.INV_NO=B.INV_NO AND "+
                                                " NVL(B.QTY_ENDORS,0)<>0 AND A.T_O_date IS NOT NULL AND NVL(A.SHIP_QTY,0)=0 and doc_send is null and nvl(surrender_yn,'X')<>'Y' and trunc(sysdate)-trunc(nvl(etd_date,sysdate))>=? and  a.location like '"+LOCT_CODE+"' and a.buyer like '"+BUYER+"' and a.ac_holder like '"+ac_holder+"' and a.self_tp like '"+self_tp+"'"+v_cond+
                                                " group by decode("+"'"+ordby+"','M',mode_of_ship,'P',cost_centre,'B',buyer,'A',ac_holder,'L',loading,'C',crncy_code),to_char(nvl(a.etd_date,a.t_o_date),'YYYYMM'),to_char(nvl(a.etd_date,a.t_o_date),'MON-YY'),crncy_code order by 1,2,4");
                    stat.setInt(1,etd_days);  
                    result=stat.executeQuery();
                   while (result.next()) 
                   {
                       summarylist.add(new TOSumBean(result.getString("p_ord"),result.getString("mawb"),result.getString("crncy_code"),result.getDouble("invqty"),result.getDouble("FOB"))); 
                 
                      
                   }   
                    
                    parameters.put("summarylist",new JRBeanCollectionDataSource(summarylist)); 
                    ...
                    parameters.put("p_days",etd_days);
                    parameters.put("p_ord",ordby);
               ...
               { acList.add(new GetListBean(result.getString("type_Desc"),result.getString("type_Desc")));       }
                ...
                stat=conn.prepareStatement("select distinct cost_centre from ei_endors_mast where company='111' and awbdate is null order by 1");
               result=stat.executeQuery();
               while(result.next())
               { pchList.add(new GetListBean(result.getString("cost_centre"),result.getString("cost_centre")));       }
                ...
                if(unitparam!=null && unitparam.length()>0){
                     ...
                     unitlist = dao.getBuyeraddListbyName(unitparam);
                }
            ...
            try{
                ...
                try
                {
                    ...
                    falg = 0;
                    System.out.print("File Name : TOStatusRepAction.java Exception in finally block");
                    e.printStackTrace();
                    addActionMessage("File Name : TOStatusRepAction.java Exception in finally block");
                    return ERROR;
                }
             } /// end Finally Block
           }
