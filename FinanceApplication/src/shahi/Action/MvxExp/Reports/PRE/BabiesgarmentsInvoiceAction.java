...
import shahi.Action.MvxExp.Reports.PRE.bean.BabiesgarmentsInvoiceBean;
 ...
 * @author User
 */
public class BabiesgarmentsInvoiceAction extends ActionSupport implements ServletRequestAware, ServletResponseAware{
    ...
    public String execute() throws SQLException {
        ...
        try {
            ...
            conn = new connection().getConnection();
             ...
             List BabiesgarmentsInvoiceBeans=new ArrayList();
               ...
               SimpleDateFormat sdfm = new SimpleDateFormat("dd/MM/yyyy");
                      stat1 = conn.prepareStatement("select a.year,a.company,a.inv_no,a.location,a.excs_inv_no,a.plan_no,to_char(inv_date,'dd/mm/yyyy') inv_date,a.exp_type,nvl(a.self_tp,'N') self_tp,trim(notify) notify,agent,fwd_code,hs_code,TRIM(manuf_code) manuf_code,"
                            + " a.cost_centre,a.mode_of_ship,a.inv_qty,rpad(a.buyer,10,' ') buyer,rpad(a.buyer_addr,6,' ') buyer_addr,a.cons_addr,LOADING_port,trim(LOADING) CLR_port,pre_carriage,upcharge_per,comm_per,payment_term,ship_term,pay_term,place,fwd_custom,mmitgr,"
                    ...
                    if (result1.next()) 
                    {  
                        BabiesgarmentsInvoiceBean bean = new BabiesgarmentsInvoiceBean();
                        ...
                        bean.setINV_NO(result1.getString("inv_no"));
                          ...
                          {  
                            bean.setAD_CODE(result.getString("okagbg"));
                                       ...
                                       String GSTIN=null,STATE=null;
                                       
                                        PreparedStatement stat5=conndb2.prepareStatement("select  OAADK2,OACONM,trim(OAADR1)||' '||trim(OAADR2)||' '||trim(OAADR3)||' '||trim(OAADR4) addr,"
                                                + "OATAXC, OAECAR ,aRXCSN cst,aRXLSN tin,arxlcn gstin,arfre1 statecode  "
                                                + "from m3fdbprd.ciaddr a,minfdbprd.xinddr b where oacono=111 and oaadth=1 "
                                                + "and oaadth=aRADTH and oacono=arcono and oaadk2=aradk2 and oaadk3=aradk3 and TRIM(oaadk3)=? ");
                                         stat5.setString(1, result1.getString("manuf_code"));
                                         ResultSet result5 = stat5.executeQuery();
                                         if (result5.next() == true) {
                                            GSTIN=result5.getString("gstin");
                                            STATE=result5.getString("STATECODE");
                                         } 
                                          bean.setGSTIN_CODE(GSTIN);
                                           bean.setSTATE_CODE(STATE);
                                         if(result5!=null){
                                            result5.close();
                                         }
                                         if(stat5!=null){
                                      ...
                                      /*
                                        ...
                                        {   
                                           ...
                                           bean.setGSTIN_CODE(result.getString("gstin"));
                                           bean.setSTATE_CODE(result.getString("statecode"));
                         ...
                         PreparedStatement   stat4 = conn.prepareStatement("select year,company,inv_no,price_fc+nvl(price_misc,0) inv_rate,a.description inv_desc,a.unit,mrp_rate,qty_endors,sr_no,cal_value,cenvat,edu_cess,sec_hr_edu from ei_endors_dtls a,ei_excise_duty_mast b where nvl(qty_endors,0)>0 and ? between date_from and date_to and a.all_no=?");
                        ...
                        while (result2.next()) {  
                           ...
                           // tfob=tfob+result2.getDouble("fob_amt");
                            ...
                            if (hsn!=null){
                                ...
                                hsn= arr[0];}  
                            InvLineList.add(new InvLineBean1(hsn,result2.getString("description"),result2.getString("iqty"),result2.getString("uom"),rate_inv,result2.getDouble("fob_amt")));
                        }
                      ...
                      tot_inv=roundTwoDecimals(tfob+up+excise_d+tax_a);
                        ...
                        String wordEX_DUTY="";        
                        stat=conn.prepareStatement(" select conv_to_word(floor(?)) aa,conv_to_word(((?-floor(?))*100)) aa1 from dual");
                        stat.setDouble(1,roundTwoDecimals(excise_d));
                        stat.setDouble(2,roundTwoDecimals(excise_d));
                        stat.setDouble(3,roundTwoDecimals(excise_d)); 
                        result=stat.executeQuery();
                        if (result.next())
                       ...
                       if (result.next() == true) {
                        ...
                        BabiesgarmentsInvoiceBeans.add(bean);
                ...
                if (INVNO!=null) {
                    ...
                    JasperPrint print = JasperFillManager.fillReport(rep, param1, new JRBeanCollectionDataSource(BabiesgarmentsInvoiceBeans));
            ...
            {
                response.setHeader("Content-Disposition", "attachment;filename=BabiesgarmentsInvoice.pdf"); //attachment- use open new window and inline- use open in same window
            ...
            else
            {
                        ...
                        exporter.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, "BabiesgarmentsInvoice.xls");
                        exporter.exportReport();
                        
                        bytes = xlsReport.toByteArray();
                        response.setContentType("application/vnd.ms-excel");
                        response.setHeader("Content-Disposition", "attachment; filename=BabiesgarmentsInvoice.xls;");
