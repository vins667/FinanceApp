...
public class BuyerInvAjaxAction extends ActionSupport {
    ...
    public String execute() {
     ...
     try{
      ...
      public String ajaxManuf() throws SQLException {
        String ls = "";
        ...
        if (INVOICE_NO_S != null && INVOICE_NO_S.length() > 0) {
            ...
            try {
                ...
                if (INVOICE_NO_S != null && INVOICE_NO_S.length() > 0) {
                    ...
                    conn = new connection().getConnection();
                    st = conn.prepareStatement("select VEND_CODE,VEND_ADDR from pr_vend_mast where 1=1 and VEND_CODE like ? ");
                    st.setString(1, "%" + INVOICE_NO_S.toUpperCase() + "%");
                    rs = st.executeQuery();
                    while (rs.next()) {
                       
                          ls+=rs.getString("VEND_CODE")+"#"+rs.getString("VEND_ADDR");
