...
public class PreInvoiceMvx_beforeGSTAction extends ActionSupport {
    ...
    public String execute() {
     ...
     try{
        ...
        try { 
         ...
         {    
             if (GETREFRESH == null){
                    stat1 = conn.prepareStatement("select a.year,a.company,a.inv_no,a.location,a.excs_inv_no,a.plan_no,to_char(a.inv_date,'dd-Mon-yyyy') inv_date,a.exp_type,nvl(a.self_tp,'N') self_tp,to_char(a.fwd_custom,'dd/mm/yyyy') fwd_custom,to_char(a.tto_date,'dd/mm/yyyy') tto_date,to_char(a.etd_date,'yyyy-mm-dd') etd_date,notify,trim(agent) agent,trim(fwd_code) fwd_code,hs_code,trim(manuf_code) MANUF_CODE,"
                            ...
                            +" a.transport_cost,facility,CTNS,GRWT,NETWT,DESTI_STATE  from ei_endors_mast a  where  a.excs_inv_no like nvl(?,'%') and  a.plan_no like nvl(?,'%') ");
 ...
 /////////   New Plan to Generate Invoice........      
             ...
             if (GETREFRESH.equals("YES")){
                     plan_no=searchplan;
                     GetDataFlag="YES"; 
                            stat=conn.prepareStatement(" select t_o_date,tto_date,etd_date from ei_endors_mast where plan_no=?");
                            stat.setString(1,searchplan);
                            result = stat.executeQuery();
                            if (result.next() == true) 
                            {
                            ...
                            stat = conn.prepareStatement("select a.company_code,a.location_code,nvl(a.sale_type,'N') sale_type,a.STATE_CODE,GST_STATE_CODE,sum(round((nvl(plan_qnty,0)+nvl(extra_qnty,0))/nvl(conv_factr,1),2)) planqty,to_char(trunc(sysdate),'dd-Mon-yyyy') idate  from  seplweb.pr_ship_plan_master a,seplweb.pr_ship_plan_detail b,, seplvportal.state_master c"
                                    + "                     where a.plan_numb=b.plan_numb and a.state_code=c.state_code and c.COUNTRY_MASTER_ID=35 and a.plan_numb=? group by a.company_code,a.location_code,nvl(a.sale_type,'N'),to_char(trunc(sysdate),'dd-Mon-yyyy'),a.STATE_CODE,GST_STATE_CODE");
                            stat.setString(1, searchplan);
                            result = stat.executeQuery();
                            if (result.next() == true) 
                            {
                    ...
                    // Checking Duplicate Delv No.
                            stat = conn.prepareStatement("select del_numb  from seplweb.pr_ship_plan_detail where inv_no is not null and del_numb in (select distinct del_numb from seplweb.pr_ship_plan_detail where del_numb not in (999999,888888) and plan_numb=? and nvl(plan_qnty,0)+nvl(extra_qnty,0)>0) and plan_numb <> ?");
                            stat.setString(1, searchplan);
    ...
    public String cocheck(String P_PLAN,Connection conn,Connection conndb2)
    {
            /* Adding below code for replacement of procedure*/
                        ...
                        ResultSet rs2=null; String p_error="";
                        String v_buyer="";String v_error="";String v_payterm="";String v_paymethod="";String v_baddr="";String v_crncy="";
                           String v_payterm_desc="";String v_inrconv="";
                           String v_cotype="";
                           String err1="";String v_cons_addr="";String v_desti_desc="";String v_desti_name="";String v_mode="";
                           String v_ship_term="";String v_caddr="";String v_desti_code="";String v_desti_cntry=""; String v_desti_state="";
                             
                        
                          try{
                             
                    
                            String Sql1="select distinct co_numb from seplweb.pr_ship_plan_detail where plan_numb=? and nvl(plan_qnty,0)+nvl(extra_qnty,0)>0";
					...
					+ " a.oacono='111' and  a.oaorno=?";
				       ...
				       +" where a.plan_numb=? and  nvl(plan_qnty,0)+nvl(extra_qnty,0)>0";
                             pst=conn.prepareStatement(Sql4);
                              pst.setString(1, P_PLAN);
                             rs=pst.executeQuery();    
                             while(rs.next())
                             {
                                 String p_cons_addr=rs.getString("DEL_ADD_NO");
                                ...
                                pst1.setString(3, rs.getString("item_numb"));
                                 ...
                                 while(rs1.next()) 
                                 {
                                     ...
                                     String Sql6="select  opedes,opcscd,OPECAR from m3fdbprd.ocusad where opcono=111 and " 
                          ...
                          { 
                              try{
                              if(rs2!=null)rs2.close(); if(rs1!=null)rs1.close(); if(rs!=null)rs.close();
                              if(pst2!=null)pst2.close();  if(pst1!=null)pst1.close();  if(pst!=null)pst.close();
     ...
     public String cocheckline(String P_PLAN,String user_id,Connection conn,Connection conndb2)
    {
                        ...
                        PreparedStatement pst=null;
                        ResultSet rs=null; ResultSet rs1=null;
                        ResultSet rs2=null; String p_error="";String v_agent_comm="";
                        String v_hsncode=""; Double v_igst_per=0.0; Double v_sgst_per=0.0; Double v_cgst_per=0.0; 
                        double v_gr_Amt=0;
                        ResultSet rs3=null;
                            
                        String v_exp_type="";String v_pch="";String v_unms="";String v_comm="";  String v_mmitgr="";
                        
                        String v_auom="";int v_err2=0;
                          ...
                          try{
                                 ...
                                 String Sql0="delete from ei_get_coline where  plan_no=? and userid=? ";
                                  ...
                                  String Sql1="select company_code,co_numb,co_line,item_numb,uom,conv_factr,item_desc,sum(nvl(plan_qnty,0)+nvl(extra_qnty,0)) plan_qnty " 
                                        +" from seplweb.pr_ship_plan_detail a where  a.plan_numb=? and " 
                                        +" nvl(plan_qnty,0)+nvl(extra_qnty,0)>0 group by company_code,co_numb,co_line,item_numb,uom,conv_factr,item_desc";
                            pst=conn.prepareStatement(Sql1);
                            pst.setString(1, P_PLAN);
                            rs=pst.executeQuery();
                            while(rs.next())
                                ...
                                String Sql2="select  oafaci,oaortp,obsapr,obnepr,obdip1,obcofa,obalun,obspun,oacuor bpo,oaoref sty " 
                                 ...
                                 {  
                                  ...
                                  while(rs2.next())
                                  {
                                      ...
                                      l1.add("TRM");l1.add("FAB");l1.add("FTP");l1.add("FFC");l1.add("TTP");l1.add("TFS");
                                   ...
                                   String Sqlfn="select  khpope v_comm from    m3fdbprd.MCPRCO  c "
                                          ...
                                          rs3=pst3.executeQuery();
                                               ...
                                               if (v_pch.equals("LSO")){v_agent_comm=null;}    
                                       ...
                                       String qtyins=roundToDecimals(Double.parseDouble(rs.getString("plan_qnty"))/Double.parseDouble(rs.getString("conv_factr")!=null?rs.getString("conv_factr"):"1"),2);
                                        ...
                                        +" values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate)";    
                                            ...
                                            pst3.setDouble(22,v_igst_per);
                                            pst3.setDouble(23,v_cgst_per);
                                            pst3.setDouble(24,v_sgst_per);
...
System.out.println("Line Start ....."+dateFormat.format(new Date()));     
         if(GetDataFlag!=null && GetDataFlag.equals("YES")){
            ...
            stat.setString(2, usrid); 
            result = stat.executeQuery();
            double fob_fc = 0.0; Double invkgs=0.0; String invcat=""; String invdesc="";String invdbk=""; String invstr=""; 
             String invstrm=""; String invmadefor="" ; String invrosl=""; String invscheme=""; String invhs="";
            BigDecimal miscprice = new BigDecimal("0.0");BigDecimal mhngrcost = new BigDecimal("0.0");BigDecimal mtagcost = new BigDecimal("0.0");
     ...
     public String preexcise() throws SQLException {
        ...
        try { 
          ...
          if(GetDataFlag!=null && GetDataFlag.equals("YES")){
            ...
            double fob_fc = 0.0;
            while (result.next()) {
                Double  vmrp=0.0; String vhscode="";
     ...
     public String saveinv() throws SQLException {
        ...
        try {
          ...
          if(saveFlag!=null && saveFlag.equals("YES"))
          {
            ...
            // Checking data at FWD To Custom 
              
              if (fwd_custom!=null && fwd_custom.equals("Y")){
                                    ...
                                    if ((p_error.length()>0))
                                     ...
                                     if (result.next())
                                      {   ;
                                          ...
                                          {
                                              addActionMessage(" Error :- "+result.getString("aa"));
                                              return  ERROR;
                                          }
                                      }  
                                      
                                /*     RsStat =conn.prepareCall("{call  exports.proc_cocheck_line115(?,?,?)}");
                                      RsStat.setString(1,searchplan);
                                     RsStat.setString(2,usrid);
                                     RsStat.registerOutParameter(3, java.sql.Types.VARCHAR);
                                     RsStat.executeUpdate();
              ...
              //// End check FWD to Custom
              
                
              
              String vliccomp=""; String vtext="" ; String vlic=""; String vlicno=""; String vlicdt=""; String vissue="";      
              ...
              stat = conn.prepareStatement("select * from ei_endors_mast where plan_no=? ");
                          ...
                          location=result.getString("location");
                            stat1=conn.prepareStatement("update ei_endors_mast set buyer=?,buyer_addr=?,cons_addr=?,NOTIFY=?,MODE_OF_SHIP=?,PLACE=?,LOADING =?,DISCHARGE =?,DESTI_CNTRY=?,PAY_TERM=?,INV_QTY=?,ENDORS_QTY =?,T_O_DATE=to_date(?,'yyyy-mm-dd'),AGENT =?,SHIP_DESC=?,COMM_PER=?,SELF_TP=?,EXP_TYPE=?,PRE_CARRIAGE=?,SURRENDER_YN=?,COST_CENTRE=?,MANUF_CODE =?,TAX_TYPE=?,TAX_PERCENT=?,"
                                                     + " FACILITY=?,ship_term=?,DESTI_CODE =?,FWD_CUSTOM=decode(?,'Y',sysdate,'NN',null,fwd_custom),HS_CODE =?,OH_WORK =?,TTO_DATE=decode(?,'Y',sysdate,tto_Date),AC_HOLDER =?,TAX_CODE=?,"
                                                      + "FWD_CODE=?,PAYMENT_TERM=?,TAX_CAL_PER =?,ETD_DATE=to_date(?,'yyyy-mm-dd'),to_inr_conv=?,CRNCY_CODE=?,LOADING_PORT=?,LCNO=?,MERCHANT_NAME=?,PPRQ_DATE=decode(?,'Y',sysdate,pprq_date),UPCHARGE_PER=?,DIS_CNTRY=?,CTNS=?,GRWT=?,transport_cost=?,CONSIGNEE=?,mmitgr=?,netwt=?,desti_state=? where plan_no=?");
                            stat1.setString(1,buyer.trim());
                            stat1.setString(2,buyer_addr.trim());
                            stat1.setString(3,cons_addr.trim());
                             stat1.setString(4,notify);
                            stat1.setString(5,mode_of_ship.trim().toUpperCase());
                            stat1.setString(6,PLACE.trim().toUpperCase());
                            stat1.setString(7,CLR_PORT.trim().toUpperCase());
                            stat1.setString(8,DISCHARGE.trim().toUpperCase());
                            stat1.setString(9,DESTI_CNTRY.trim().toUpperCase());
                            stat1.setString(10,pay_term);
                            stat1.setString(11,inv_qty);
                            stat1.setString(12,inv_qty);
                            if (to_date.length()>8)
                            {  stat1.setString(13,to_date.substring(0,10));}
                            else
                            {  stat1.setString(13,to_date);}
                            
                            stat1.setString(14,agent);
                            stat1.setString(15,SHIP_DESC.toUpperCase());
                            stat1.setString(16,comm_per);
                            stat1.setString(17,self_tp);
                            stat1.setString(18,exp_type);
                            stat1.setString(19,pre_carriage);
                            stat1.setString(20,ship_cancel);
                            stat1.setString(21,cost_centre.trim());
                            stat1.setString(22,MANUF_CODE);
                            stat1.setString(23,TAX_TYPE);
                            stat1.setString(24,TAX_PERCENT);
                            stat1.setString(25,facility);
                            stat1.setString(26,ship_term);
                            stat1.setString(27,DESTI_CODE.trim().toUpperCase());
                            stat1.setString(28,fwd_custom);
                            stat1.setString(29,hs_code.toUpperCase());
                            stat1.setString(30,outhouse);
                            stat1.setString(31,tto_date);
                            stat1.setString(32,ac_holder.toUpperCase());
                            stat1.setString(33,TAX_CODE.toUpperCase());
                        //    stat1.setString(35,CNTRY_ORIGIN.trim().toUpperCase());
                            stat1.setString(34,fwd_code.toUpperCase());
                            stat1.setString(35,payment_term);
                            stat1.setString(36,TAX_CAL_PER);
                          if (etd_date.length()>8)
                            {  stat1.setString(37,etd_date.substring(0,10));}
                            else
                            {  stat1.setString(37,etd_date);}   
                             stat1.setString(38,to_inr_conv);
                            stat1.setString(39,CRNCY_CODE.trim());
                            stat1.setString(40,LOADING_PORT.trim().toUpperCase());
                            stat1.setString(41,lcno);
                            stat1.setString(42,merchant);
                
                            stat1.setString(43,pprq_date);
                            stat1.setString(44,upcharge_per);
                            stat1.setString(45,DIS_CNTRY.trim().toUpperCase());
                            stat1.setString(46,CTNS);
                            stat1.setString(47,GRWT);
                            stat1.setString(48,transport_cost);
                            stat1.setString(49,buyer.trim());
                            stat1.setString(50,mmitgr);
                            stat1.setString(51,NETWT);
                          System.out.println("check desti State "+DESTI_STATE);      
                            stat1.setString(52,DESTI_STATE);
                            stat1.setString(53,plan_no);
                  ...
                  // Delete detail file.    
                   ...
                   // Delete completed... insert start 
                   
                             double linefob=0.0;  int sr=0;
                                ...
                                for(int i=0; i<CO_NO_E.size(); i++)
                                {
                                    ...
                                    {  
                                        ...
                                        stat=conn.prepareStatement(" select * from ei_opc_check where loct_code=? and desti_cntry=? and pch=? and cat_code=? and end_date is null ");
                                         stat.setString(1,LOCATION_CODE);
                                   ...
                                   // stat1.setString(14,PRICE_FC_E.get(i).toString());
                                     ...
                                     //   stat1.setString(15,PRICE_MISC.get(i).toString());
                                       ...
                                       stat1.setString(31,LOCATION_CODE);
                                       ...
                                       stat1.setString(33,facility);
                                        ...
                                        stat1.setString(41,IGST_PER_E.get(i).toString());
                       ...
                       /*    ///  Excise MRP  Rate
                            if(CO_NO_EXCISE!=null  && CO_NO_EXCISE.size()>0 )
                            {
                               
                                for(int i=0; i<CO_NO_EXCISE.size(); i++)
                                {
                                    if(CO_NO_EXCISE.get(i).toString()!=null && CO_NO_EXCISE.get(i).toString().length()>0)
                                    {  
                                       
                                        
                                        stat1=conn.prepareStatement("UPDATE  ei_endors_DTLS SET MRP_RATE=? WHERE YEAR=? AND COMPANY=? AND INV_NO=? AND CO_NO=? AND CO_LINE=?  ");
                                        stat1.setString(1,MRP_RATE.get(i).toString());
                                        stat1.setString(2,YEAR);
                                        stat1.setString(3,COMPANY);
                                        stat1.setString(4,INV_NO);
                                        stat1.setString(5,CO_NO_EXCISE.get(i).toString().trim());
                                        stat1.setString(6,CO_LINE_EXCISE.get(i).toString().trim()); 
                                        stat1.executeUpdate(); 
                                    } 
                                } 
                            }// Excise MRP PRice End
                         */   
                             /// Licence Table
                               ...
                               {     int j=1;  
                                ...
                                { if(REF_NO.get(i).toString()!=null && REF_NO.get(i).toString().length()>0)
                                    {  
                                            ...
                                            else{vissue="Issued by JDGFT (CLA), New Delhi";}
                                            ...
                                            if (LIC_COMP.get(i).toString().trim().equals("111") || LIC_COMP.get(i).toString().trim().equals("SCGA0119") || LIC_COMP.get(i).toString().trim().equals("FD001090") || LIC_COMP.get(i).toString().trim().equals("SEPL"))
                                             {vliccomp="111";}
                                             if(COMPANY.equals(vliccomp)){vtext="THIS IS IN DISCHARGE OF EXPORT OBLIGATION UNDER LICENCE NO: "+vlicno+' '+vissue; }
                                       ...
                                       stat1.setString(13,LOCATION_CODE );
                                       stat1.setInt(14,j);
      ...
      // Insert start           
                      { 
                      /// Excs Invoice Series Start
                           
                                 COMPANY="111";  String newno=""; String subtype=""; String prefix="";String vloct=""; String FINYR="";
                              if (CNTRY_ORIGIN.trim().equals("IN")) 
                               {   if (CRNCY_CODE.trim().equals("INR") &&  EXCISE_UNIT.length()==0 && (facility.equals("200") || facility.equals("210")))
                                       {   
                                              // vloct="200";
                                               vloct=facility;
                                               if (vloct.equals("210"))
                                   ...
                                   else if (CRNCY_CODE.trim().equals("INR") &&  EXCISE_UNIT.length()>0 && (facility.equals("200") || facility.equals("210")))
                                       {   
                                               vloct="200";
                                               if (EXCISE_UNIT.equals("U2")) {subtype="U2";prefix="5";}
                                               if (EXCISE_UNIT.equals("U14")) {subtype="U14";prefix="4";}
		                               if (EXCISE_UNIT.equals("U99")) {subtype="U99";prefix="2";}
                                       }  
                                      else
                                          { vloct=facility;
                                            subtype="INV"; 
                               ...
                               {       
                                   vloct=facility;
                                   subtype="TCS";prefix="8";
                               }    
                                  
                                     stat1=conn.prepareStatement("select vou_numb+1 vno ,lpad(nvl(vou_numb,0)+1,4,'0') newex4,lpad(nvl(vou_numb,0)+1,5,'0') newex5,SUBSTR(FIN_YEAR,3,2) FIN_YEAR from ei_vou_numb_mast where location_code=? and fin_year=pay_fin_year(?) and  VOU_TYPE='EI' AND SUB_TYPE=? for update NOWAIT");
                                   ...
                                   if (result1.next() == true) { 
                                       ...
                                       if (subtype.equals("U23")){excs_inv_no="30"+FINYR+result1.getString("newex5");} 
                                       if (subtype.equals("DOM")){excs_inv_no="31"+FINYR+result1.getString("newex5");}
                                        if (subtype.equals("INV")){
                                             if (vloct.equals("215")){  
                                                     excs_inv_no="22"+FINYR+result1.getString("newex5");
                                               }else
                                                    { 
                                                    excs_inv_no=facility.substring(0,2)+FINYR+result1.getString("newex5");
                                                    }
                                              } 
                                        
                                       if (subtype.equals("TCS")){excs_inv_no=facility.substring(0,2)+FINYR+"8"+result1.getString("newex4");} 
                                       if (subtype.equals("U2")){excs_inv_no="30"+FINYR+"5"+result1.getString("newex4");} 
                                       if (subtype.equals("U14")){excs_inv_no="30"+FINYR+"4"+result1.getString("newex4");} 
                                       if (subtype.equals("U99")){excs_inv_no="30"+FINYR+"2"+result1.getString("newex4");} 
                                   }
                                                  
                                           stat1=conn.prepareStatement("select nvl(vou_numb,0)+1 vno from ei_vou_numb_mast WHERE FIN_YEAR=pay_fin_year(?) AND VOU_TYPE='EI' AND SUB_TYPE='111' for update NOWAIT ");
                         ...
                         // Excs Invoice Series clsoed..
                          ...
                          stat1=conn.prepareStatement("insert into  ei_endors_mast ( YEAR,TYPE,COMPANY,INV_NO,INV_DATE,EXCS_INV_NO,PLAN_NO,BUYER,BUYER_ADDR,CONSIGNEE,CONS_ADDR,NOTIFY,MODE_OF_SHIP,PLACE,LOADING, DISCHARGE,DESTI_CNTRY,PAY_TERM,INV_QTY,ENDORS_QTY,T_O_DATE,AGENT,SHIP_DESC,COMM_PER,LOCATION,SELF_TP,"
                                  ...
                                  + "                  LCNO, MERCHANT_NAME, CO_MOS,PPRQ_DATE,UPCHARGE_PER,DIS_CNTRY,CTNS,GRWT,transport_cost,SEH_USER,TDATE,t_mod,EXCISE_UNIT,mmitgr,netwt,inv_state,desti_state,gstin_state) "
                            ...
                            stat1.setString(24,LOCATION_CODE); 
               ...
               //  Insert into Detail File
                            ...
                            if(CO_NO_E!=null  && CO_NO_E.size()>0 )
                            {
                                ...
                                { 
                                    ...
                                    {    Double linefob=0.00;  Double p1=0.0;  Double p2=0.0;
                                         stat=conn.prepareStatement(" select * from ei_opc_check where loct_code=? and desti_cntry=? and pch=? and cat_code=? and end_date is null ");
                                         stat.setString(1,LOCATION_CODE);
                                         stat.setString(2,DESTI_CNTRY.trim().toUpperCase());
                                         stat.setString(3,cost_centre.trim().toUpperCase());
                                         ...
                                         sr=sr+1;         double san_pr=0.0;   
                                         stat1=conn.prepareStatement("insert into ei_endors_dtls (year,type,company,inv_no,all_no,all_date,sr_no,co_no,co_line,item_no,unit,qty_endors,qty_pcs,qty_kgs,price_fc,price_misc,ADJUST_FC,CURRENCY,COUNTRY,CATEGORY,DESCRIPTION,DBK_SLNO,STR_SLNO,STR_MISC,"
                                               + "                  NET_PRICE,GR_DECL_AMT,PRE_PRINT_NO,TOKEN_NO,GR_DECL_PER,TEMP_CAT,MADE_FOR,location,inr_conv,faci, seh_user,rosl_slno,SCHEME_CODE,HSCODE1,hngr_cost,tag_cost,tdate) values(pay_fin_year(?),'E',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,upper(?),?,?,sysdate)");
                                       stat1.setString(1,inv_date);
                                       stat1.setString(2,COMPANY);
                                       stat1.setString(3,INV_NO);
                                       stat1.setString(4,excs_inv_no);
                            ...
                            if(CO_NO_EXCISE!=null  && CO_NO_EXCISE.size()>0 )
                            {
                                ...
                                {
                                    if(CO_NO_EXCISE.get(i).toString()!=null && CO_NO_EXCISE.get(i).toString().length()>0)
                                    {   stat1=conn.prepareStatement("UPDATE  ei_endors_DTLS SET MRP_RATE=? WHERE YEAR=pay_fin_year(?) AND COMPANY=? AND INV_NO=? AND CO_NO=? AND CO_LINE=?  ");
                                        stat1.setString(1,MRP_RATE.get(i).toString());
                                        stat1.setString(2,inv_date);
                                        stat1.setString(3,COMPANY);
                                        stat1.setString(4,INV_NO);
                            ...
                            // Insert into Licence Table
                              ...
                              { int j=1;
                               ...
                               // Update Vou Num Master
                                     ...
                                     stat1=conn.prepareStatement("update ei_vou_numb_mast set vou_numb=?  where location_code=? and fin_year=pay_fin_year(?) and  VOU_TYPE='EI' AND SUB_TYPE=? ");
                                     ...
                                     stat1=conn.prepareStatement("update ei_vou_numb_mast set vou_numb=?  where fin_year=pay_fin_year(?) and VOU_TYPE='EI' AND SUB_TYPE='111' ");
        ...
        if (flag == 1) {
            ...
            addActionMessage("Records Save !!  Invoice No "+excs_inv_no);
                        searchplan=null;
                        searchinv=null;
                        location = null; 
                        searchloct=null;  
                        ...
                        pay_term = null;  
                        transport_cost=null;  
                        CTNS=null; GRWT=null; NETWT=null;
                        payment_term = null;  
                        LOADING_PORT = null;  
                        CLR_PORT = null;    
                        DESTI_CNTRY = null;  
                        ...
                        MANUF_DESC=null; customfwd_auth=null;
                        YEAR=null; COMPANY=null; INV_NO=null;
            return SUCCESS;
        } else {
     ...
     public String exciseinv() throws SQLException {
        ...
        try {
          ...
          String newno=""; String subtype=""; String prefix="";String vloct=""; String FINYR="";
          ...
          if(exciseFlag!=null && exciseFlag.equals("YES"))
          {
                          subtype=EXCISE_UNIT;
                                   ...
                                   if (result1.next() == true) { 
                                       ...
                                       if (subtype.equals("U2")){ex_inv_slno="30"+FINYR+"5"+result1.getString("newex4");} 
                                       if (subtype.equals("U14")){ex_inv_slno="30"+FINYR+"4"+result1.getString("newex4");} 
                                       if (subtype.equals("U99")){ex_inv_slno="30"+FINYR+"2"+result1.getString("newex4");} 
                                   }
     ...
     public String prelicence() throws SQLException {
       ...
       // List InvLicenceList = new ArrayList<PreInvLicenceBean>();
        ...
        try {
            ...
            int flag=0;
               String expdesc="";  String bedesc="";
               double lic_amt_ob=0;  double lic_qty_ob=0;     double lic_amt_issue=0; double lic_qty_issue=0;  
     ...
     public String prestrslview() throws SQLException{
        ...
        if(INVDATE!=null && INVDATE.length()>0){
           ...
           else{ unitList = new PreInvoiceDao().getPreSTRSL(dbkslnocopy,PARAB,INVDATE);}
