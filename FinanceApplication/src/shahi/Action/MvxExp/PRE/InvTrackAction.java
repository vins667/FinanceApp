...
public class InvTrackAction extends ActionSupport {
               ...
               SimpleDateFormat sdfm = new SimpleDateFormat("dd/MM/yyyy");
                ...
                {  
                    ...
                    { 
                        ...
                        stat2 = conn.prepareStatement("select to_char(tr_date,'dd/mm/yyyy') tr_date,type_desc||' - '||tr_type trdesc,tr_type,remark,a.seh_user from ei_truckout_track a,ei_grup_type_dtls b where a.tr_type=b.type_code and b.grup_type_code='IF' and year=? and company=? and inv_no=? order by a.sr_no");
                        ...
                        while (result2.next()) 
                        {
                         ...
                         stat2 = conn.prepareStatement("select to_char(a.tdate,'dd/mm/yyyy') tr_date,type_desc trdesc,reason_code,a.grup_code,a.seh_user from ei_docs_delay_dtls a,ei_grup_type_dtls b where a.REASON_CODE=b.type_code and A.GRUP_CODE=b.grup_type_code AND A.GRUP_CODE IN ('ADR','EDR') and year=? and company=? and inv_no=? order by 1");
                        stat2.setString(1,year);
                        stat2.setString(2,company);
                        stat2.setString(3,inv_no);
                        result2 = stat2.executeQuery();
                        while (result2.next()) 
                        {
                            PreDRList.add(new InvTrackBean(result2.getString("tr_date"), result2.getString("reason_code"), result2.getString("trdesc"), result2.getString("grup_code"), result2.getString("seh_user")));
                        }  
                        stat2 = conn.prepareStatement("select to_char(a.tdate,'dd/mm/yyyy') tr_date,type_desc trdesc,reason_code,a.grup_code,a.seh_user from ei_docs_delay_dtls a,ei_grup_type_dtls b where a.REASON_CODE=b.type_code and b.grup_type_code='PDR' and year=? and company=? and inv_no=? order by 1");
                        stat2.setString(1,year);
                        stat2.setString(2,company);
                        stat2.setString(3,inv_no);
                        result2 = stat2.executeQuery();
                        while (result2.next()) 
                        {
                            PostDRList.add(new InvTrackBean(result2.getString("tr_date"), result2.getString("reason_code"), result2.getString("trdesc"), result2.getString("grup_code"), result2.getString("seh_user")));
                        }
                      ...
                      if (result.next() == true) 
                      {
                       ...
                       if (result.next() == true) 
                       {
                         ...
                         {  
                          ...
                          if (location.equals("100"))
                         { 
                             stat1 = conn.prepareStatement("select  type_code||' - '||type_desc type_DESC,type_code,grup_type_code from  EI_GRUP_TYPE_DTLS WHERE  grup_type_code='EDR' order by 3,1" );
                         }else{
                              stat1 = conn.prepareStatement("select  type_desc||'-'||type_code||'('||grup_type_code||')' type_DESC,type_code,grup_type_code from  EI_GRUP_TYPE_DTLS WHERE  grup_type_code IN ('EDR','ADR') order by 3,1" );
                           } 
                         result1 = stat1.executeQuery();
                         while(result1.next())
                         {   
                         ...
                         stat1 = conn.prepareStatement("select  type_desc||'-'||type_code type_DESC,type_code from  EI_GRUP_TYPE_DTLS WHERE  grup_type_code='PDR' order by 1" );
                         result1 = stat1.executeQuery();
                         while(result1.next())
                         {  
                            PostList.add(new GetListBean(result1.getString("type_code"),result1.getString("type_desc")));      
                         } 
                     }  
                } // View Flg Close      
                       ...
                       if (stype.equals("P") || stype.equals("S") || stype.equals("D"))
                          {
                            ...
                            {addActionMessage("Check Fwd to Custom not updated !!!");
                             ...
                             if (result1.next())
                              {   if (result1.getString("aa")!=null)
                                  {
                                      addActionMessage(" Error :- "+result1.getString("aa"));
                                        srem=null;  
                                       return  ERROR;
                           ...
                           if (awbdate.length()>0 && (stype.equals("P") || stype.equals("R")))
                           {addActionMessage("Post Shipment Entry Done. !!!");
                                srem=null;  
                                return  ERROR;
                            }
                             if (location.equals("200") && stype.equals("E") && post_docs_sent!=null )
                             {  
                                   
                                    addActionMessage(" E Already Updated.. !!!");
                                     srem=null;  
                                     return  ERROR;
                                     
                             }    
                            if (location.equals("200") && stype.equals("P") )
                            {   
                                if (pre_docs_sent.length()==0) 
                                 ...
                                 if (result1.next())
                                 {
                                   ...
                                   {  stat=conn.prepareStatement("select nvl(count(*),0) vfind from ei_docs_delay_dtls where all_no=? and grup_code='EDR'");
                                      ...
                                      {   if (result.getInt("vfind")==0)
                                        { 
                                            addActionMessage("Enter Delay Reason First....More then 6 Days from FCR Date !!!");
                                            return  ERROR;
                                        }
                                      }
                                   }
                     ...
                     ///// Pre Delay insert Start
                        ...
                        if (pretype!=null && pretype.length()>0) 
                         ...
                         if (posttype!=null && posttype.length()>0) 
                          {
                                stat1=conn.prepareStatement("insert into EI_DOCS_DELAY_DTLS (year,company,inv_no,reason_code,grup_code,all_no,seh_user,tdate) values (?,?,?,?,'PDR',?,?,sysdate)");
                                stat1.setString(1,year);
                                stat1.setString(2,company);
                                stat1.setString(3,inv_no);
                                stat1.setString(4, posttype);
                                stat1.setString(5,searchinv);
                                stat1.setString(6,usrid);
                                stat1.executeUpdate();
                                flag=1;
                          
                          }    //Post Delay Reason closed              
