...
public class InvKgsUpdAction extends ActionSupport {
        ...
        try {
            ...
            try {
             ...
             {LOCATION_CODE=result1.getString("location_code");
                    ...
                    if (getFlag!=null && getFlag.equals("Yes"))
                    {
                  
                         stat2 = conn.prepareStatement("select sum(qty_endors) qty  from ei_endors_dtls  where year=? and company=? and inv_no=? and dbk_slno=? and price_fc=? ");
                          stat2.setString(1,year);
                          stat2.setString(2,company);
                          stat2.setString(3,inv_no);
                          stat2.setString(4,ndbk.trim());
                          stat2.setString(5,nprice);
                          result2 = stat2.executeQuery();
                          if (result2.next()) 
                           {
                               nqty=result2.getDouble("qty");
                           } 
                      for(int i=0;i<SRNO.size();i++)
                        {
                          stat2 = conn.prepareStatement("UPDATE ei_endors_dtls set qty_kgs=round((?/?)*qty_endors,2)  where year=? and company=? and inv_no=? and dbk_slno=? and price_fc=? and sr_no=? ");
                          stat2.setDouble(1,nkgs);
                          stat2.setDouble(2,nqty);
                          stat2.setString(3,year);
                          stat2.setString(4,company);
                          stat2.setString(5,inv_no);
                          stat2.setString(6,ndbk.trim());
                          stat2.setString(7,nprice);
                          stat2.setString(8,SRNO.get(i).toString());
                          stat2.executeUpdate();
                       }
                       ndbk=null;
                       nprice=null;
