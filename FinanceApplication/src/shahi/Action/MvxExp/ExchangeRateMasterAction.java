            ...
            try {
                   ...
                   while(result1.next())
                    ...
                    if (saveFlag!=null)
                    {
                     ...
                     {stat2 = conn.prepareStatement("select b.currency_desc,to_char(begin_date,'yyyymmdd') bdate,a.currency,to_char(a.exp_rate,'999.99') exp_rate,to_char(a.imp_rate,'999.99') imp_rate,to_char(a.begin_date,'dd/mm/yyyy') begin_date,to_char(a.end_date,'dd/mm/yyyy') end_date from ei_exchange_rate_mast a,ei_currency_mast b where a.currency=b.currency and a.currency like ? and begin_date>=to_date(?,'yyyy-mm-dd') and end_date<=to_date(?,'yyyy-mm-dd')  order by 1,2"  );
                      stat2.setString(1,curr_code);
                       start_date1 = start_date1.substring(0, 10);
                        stat2.setString(2, start_date1);
                      end_date1 = end_date1.substring(0, 10);
                       stat2.setString(3, end_date1);
                      result2 = stat2.executeQuery();
